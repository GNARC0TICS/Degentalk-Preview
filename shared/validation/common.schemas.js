...
export function createArraySchema(itemSchema, minItems = 0, maxItems = 100) {
    return z.array(itemSchema).min(minItems).max(maxItems);
}
export function createOptionalWithDefault(schema, defaultValue) {
    return schema.default(defaultValue);
}
export function createEnumSchema(values, errorMessage) {
