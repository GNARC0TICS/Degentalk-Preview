...
export const ForumAccessSchema = z.object({
    ...
    moderate: z.object({
        ...
        actions: z
            .array(z.enum(['pin', 'lock', 'delete', 'move', 'edit']))
            .default(['pin', 'lock', 'delete'])
    })
});
...
// Validation helpers
export function validateFeaturesConfig(config) {
    return FeaturesConfigSchema.parse(config);
}
export function validatePartialFeaturesConfig(config) {
