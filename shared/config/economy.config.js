...
export const getEconomyConfig = () => economyConfig;
// Derive helper for level formula ≥ 11
export const getXpForLevel = (level) => {
    if (level <= 1)
        return 0;
    if (level <= 10) {
        const xp = economyConfig.levelXPMap[level];
        if (xp !== undefined)
            return xp;
    }
...
export const MultiplierStrategy = {
     ...
     */
    additive: (roleMultiplier, forumMultiplier) => {
     ...
     */
    multiplicative: (roleMultiplier, forumMultiplier) => {
     ...
     */
    bestOf: (roleMultiplier, forumMultiplier) => {
     ...
     */
    weightedAverage: (roleMultiplier, forumMultiplier) => {
 ...
 */
export function sanitizeMultiplier(roleMultiplier, forumMultiplier, contextInfo) {
    ...
    if (sanitizedRole > config.maxRoleMultiplier) {
        violations.push(`Role multiplier ${sanitizedRole.toFixed(2)} exceeds cap ${config.maxRoleMultiplier}`);
        sanitizedRole = config.maxRoleMultiplier;
    }
    if (sanitizedForum > config.maxForumMultiplier) {
        violations.push(`Forum multiplier ${sanitizedForum.toFixed(2)} exceeds cap ${config.maxForumMultiplier}`);
        sanitizedForum = config.maxForumMultiplier;
    }
    // Combine multipliers according to stacking rule
    let combinedMultiplier;
    const stackingRule = config.stackingRule;
    switch (stackingRule) {
        case 'additive':
    ...
    if (combinedMultiplier > config.maxTotalMultiplier) {
        violations.push(`Total multiplier ${combinedMultiplier.toFixed(2)} exceeds cap ${config.maxTotalMultiplier}`);
    ...
    if (violations.length > 0 && contextInfo) {
        ...
        const enforcementMode = config.enforcementMode;
 ...
 */
export function getSafeMultiplier(roleMultiplier, forumMultiplier) {
