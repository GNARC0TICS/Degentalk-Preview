...
import { z } from 'zod';
// --- Type Definitions ---
export const FORUM_TYPES = ['featured', 'regular'];
...
const ForumRulesSchema = z.object({
    ...
    accessLevel: z.optional(z.enum(['public', 'registered', 'level_10+', 'vip', 'moderator', 'admin'])),
...
const ForumSchema = z.lazy(() => z.object({
    slug: z.string(),
    name: z.string(),
    ...
    for (const rootForum of forumMap.forums) {
        ...
        for (const parentForum of rootForum.forums) {
            if (parentForum.forums) {
                ...
                if (sub)
                    return { forum: sub, parent: rootForum };
            }
        }
    }
    return null;
