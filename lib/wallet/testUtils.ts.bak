export function logDevEvent(action: string, result: any, setLog: (lines: string[]) => void) {
  setLog((prev) => [...prev, `[${new Date().toLocaleTimeString()}] ${action}: ${JSON.stringify(result)}`]);
}

export async function mockDeposit(userId: string, amount: number) {
...
export async function forceBalanceSync(userId: string) {
