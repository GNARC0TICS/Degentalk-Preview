# @degentalk/monorepo Directory Structure

> Generated on: 2025-07-29

This document provides a visual map of the DegenTalk monorepo. It's automatically generated by the `pnpm generate:tree` script.

## 🗺️ Navigational Index

Jump to a specific top-level directory:

| Directory | Purpose | Go to Line |
| :--- | :--- | :---: |
| **CI/CD `.github/`** | GitHub Actions workflows | `L55` |
| **⚛️ `client/`** | Frontend Application (React/Vite) | `L73` |
| **🗄️ `db/`** | Database schema, migrations, and seeds | `L953` |
| **📚 `docs/`** | Project documentation | `L1363` |
| **🛠️ `scripts/`** | Development & utility scripts | `L1570` |
| **⚙️ `server/`** | Backend Server (Express) | `L1944` |
| **🤝 `shared/`** | Isomorphic code (client & server) | `L2426` |
| **🧪 `tests/`** | End-to-end tests (Playwright) | `L2540` |

##  Legend

| Emoji | Meaning                  |
| :---: | ------------------------ |
| ⚛️    | Frontend Application     |
| ⚙️    | Backend Server           |
| 🤝    | Shared Code              |
| 🗄️    | Database                 |
| 🛠️    | Scripts & Tooling        |
| 📚    | Documentation            |
| 🧪    | Tests                    |
| 🧩    | Reusable UI Components   |
| 🏗️    | Server Business Logic    |

---

```
├─ 📁 .cache/
│  └─ 📄 dev-tokens.json
├─ 📁 .claude/
│  ├─ 📁 audit/
│  │  ├─ 📄 2025-06-29.jsonl
│  │  └─ 📄 2025-06-30.jsonl
│  ├─ 🎣 hooks/ - Custom React hooks
│  │  ├─ ⚛️  client/ - Frontend Application (React/Vite)
│  │  │  └─ 📄 no-duplicate-layout-components.mjs
│  │  ├─ 📁 react-app/
│  │  │  ├─ 📄 check-branded-ids.cjs
│  │  │  ├─ 📄 check-tooltip-usage.cjs
│  │  │  └─ 📄 performance-checks.cjs
│  │  ├─ ⚙️  server/ - Backend Server (Express)
│  │  │  ├─ 📄 enforce-domain-boundaries.js
│  │  │  └─ 📄 enforce-repository-pattern.js
│  │  ├─ 🤝 shared/ - Isomorphic code (client & server)
│  │  │  ├─ 📄 check-api-request-signature.cjs
│  │  │  ├─ 📄 check-id-conversions.cjs
│  │  │  ├─ 📄 cleanup-debug-artifacts.js
│  │  │  ├─ 📄 enforce-test-file-placement.js
│  │  │  ├─ 📄 final-cleanup-reminder.js
│  │  │  ├─ 📄 no-any-types.cjs
│  │  │  ├─ 📄 no-console.cjs
│  │  │  ├─ 📄 prevent-duplicate-directories.js
│  │  │  ├─ 📄 production-readiness.cjs
│  │  │  ├─ 📄 security-checks.cjs
│  │  │  └─ 📄 validate-imports.cjs
│  │  ├─ 📄 branded-id-patterns.json
│  │  └─ 📄 hook-config.json
│  ├─ 📁 subagents/
│  │  ├─ 📄 api-contract-enforcer.md
│  │  ├─ 📄 architecture.md
│  │  ├─ 📄 backend.md
│  │  ├─ 📄 CANONICAL-COORDINATION.md
│  │  ├─ 📄 config-surgeon.md
│  │  ├─ 📄 coordination-protocol.md
│  │  ├─ 📄 forum-specialist.md
│  │  ├─ 📄 frontend.md
│  │  ├─ 📄 import-assassin.md
│  │  ├─ 📄 MCP-AGENT-PROTOCOL.md
│  │  ├─ 📄 README.md
│  │  ├─ 📄 refactor.md
│  │  ├─ 📄 schema-unfucker.md
│  │  ├─ 📄 testing.md
│  │  └─ 📄 type-guard.md
│  ├─ 📄 command-helper.sh
│  ├─ 📄 path-instructions.md
│  ├─ 📄 PHASE-0-READINESS.md
│  ├─ 📄 settings-reference.md
│  ├─ 📄 settings.json
│  └─ 📄 settings.local.json
├─ 📁 .clinerules/
│  └─ 📄 database-cheatsheet.mdc
├─  CI/CD .github/ - GitHub Actions workflows
│  ├─ 📁 PULL_REQUEST_TEMPLATE/
│  │  └─ 📄 migration.md
│  └─ 📁 workflows/
│     ├─ 📄 ci.yml
│     ├─ 📄 migrate.yml
│     ├─ 📄 phase5-validation.yml
│     ├─ 📄 prebuild.yml
│     ├─ 📄 promote-to-prod.yml
│     ├─ 📄 security-checks.yml
│     ├─ 📄 transformer-gate.yml
│     └─ 📄 validate-codebase.yml
├─ 📁 .husky/
│  ├─ 📁 _/
│  │  └─ 📄 husky.sh
│  └─ 📄 pre-commit
├─ 📁 .tscache/
│  └─ 📄 root.tsbuildinfo
├─ ⚛️  client/ - Frontend Application (React/Vite)
│  ├─ 📁 public/
│  │  └─ 📁 assets/
│  │     └─ 📁 frames/
│  │        ├─ 📄 bronze-frame.svg
│  │        ├─ 📄 cyber-circuit-frame.svg
│  │        ├─ 📄 diamond-crown-frame.svg
│  │        ├─ 📄 electric-blue-frame.svg
│  │        ├─ 📄 gold-frame.svg
│  │        ├─ 📄 og-beta-frame.svg
│  │        └─ 📄 silver-frame.svg
│  ├─ 🏗️ src/ - Core application source code
│  │  ├─ 📁 __tests__/
│  │  │  ├─ 🧩 components/ - Reusable UI components
│  │  │  │  ├─ 📄 ErrorBoundaryTest.tsx
│  │  │  │  └─ 📄 RoleTest.tsx
│  │  │  ├─ 📁 services/
│  │  │  │  └─ 📄 error.service.test.ts
│  │  │  ├─ 📁 utils/
│  │  │  │  └─ 📄 renderWithProviders.tsx
│  │  │  └─ 📄 setup.ts
│  │  ├─ 📁 archive/
│  │  │  ├─ 🧩 components/ - Reusable UI components
│  │  │  │  └─ 📁 forum/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 features/
│  │  │  │  ├─ 📁 admin/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 auth/
│  │  │  │     ... (max depth reached)
│  │  │  └─ 🎣 hooks/ - Custom React hooks
│  │  │     └─ 📄 useThreadFeaturedForum.ts
│  │  ├─ 🧩 components/ - Reusable UI components
│  │  │  ├─ 📁 auth/
│  │  │  │  ├─ 📄 AuthRedirectHandler.tsx
│  │  │  │  ├─ 📄 GlobalRouteGuard.tsx
│  │  │  │  ├─ 📄 index.tsx
│  │  │  │  ├─ 📄 ProtectedRoute.tsx
│  │  │  │  ├─ 📄 README-MIGRATION.md
│  │  │  │  ├─ 📄 README.md
│  │  │  │  ├─ 📄 RequireRole.tsx
│  │  │  │  ├─ 📄 RouteGuards.tsx
│  │  │  │  └─ 📄 withRouteProtection.tsx
│  │  │  ├─ 📁 common/
│  │  │  │  ├─ 📄 BackToHomeButton.tsx
│  │  │  │  ├─ 📄 Breadcrumb.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 LoadingCard.tsx
│  │  │  │  ├─ 📄 OnlineIndicator.tsx
│  │  │  │  ├─ 📄 StandardErrorDisplay.tsx
│  │  │  │  └─ 📄 StatsBar.tsx
│  │  │  ├─ 📁 dashboard/
│  │  │  │  ├─ 📄 DailyBonusWidget.tsx
│  │  │  │  ├─ 📄 DailyTasksWidget.tsx
│  │  │  │  ├─ 📄 SimpleDailyTasks.tsx
│  │  │  │  └─ 📄 UpcomingEventsWidget.tsx
│  │  │  ├─ 📁 dev/
│  │  │  │  ├─ 📄 dev-playground-shortcut.tsx
│  │  │  │  └─ 📄 dev-role-switcher.tsx
│  │  │  ├─ 📁 economy/
│  │  │  │  ├─ 📁 shoutbox/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 wallet-display.tsx
│  │  │  │  └─ 📄 WalletDashboard.tsx
│  │  │  ├─ 📁 editor/
│  │  │  │  ├─ 📄 enhanced-gif-picker.tsx
│  │  │  │  ├─ 📄 gif-picker.tsx
│  │  │  │  ├─ 📄 rich-text-editor.tsx
│  │  │  │  └─ 📄 suggestion.ts
│  │  │  ├─ 📁 errors/
│  │  │  │  ├─ 📄 AdminErrorBoundary.tsx
│  │  │  │  ├─ 📄 ErrorBoundary.tsx
│  │  │  │  ├─ 📄 ForumNotFound.tsx
│  │  │  │  ├─ 📄 GlobalErrorBoundary.tsx
│  │  │  │  ├─ 📄 NetworkError.tsx
│  │  │  │  ├─ 📄 ThreadNotFound.tsx
│  │  │  │  └─ 📄 UserNotFound.tsx
│  │  │  ├─ 📁 fixtures/
│  │  │  │  ├─ 📄 fixture-builder.tsx
│  │  │  │  └─ 📄 fixture-preview.tsx
│  │  │  ├─ 📁 footer/
│  │  │  │  ├─ 📄 FooterBrand.tsx
│  │  │  │  ├─ 📄 FooterSection.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 LiveStats.tsx
│  │  │  │  ├─ 📄 RandomTagline.tsx
│  │  │  │  └─ 📄 SiteFooter.tsx
│  │  │  ├─ 📁 forum/
│  │  │  │  ├─ 📁 enhanced/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 layouts/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 sidebar/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 breadcrumb-nav.tsx
│  │  │  │  ├─ 📄 category-card.tsx
│  │  │  │  ├─ 📄 ConfigurableFeaturedForumCard.tsx
│  │  │  │  ├─ 📄 CreateThreadButton.tsx
│  │  │  │  ├─ 📄 DESIGN_SYSTEM.md
│  │  │  │  ├─ 📄 featured-forum-group.tsx
│  │  │  │  ├─ 📄 FeaturedForumCard.tsx
│  │  │  │  ├─ 📄 FeaturedForumCarousel.tsx
│  │  │  │  ├─ 📄 FeaturedForumGrid.tsx
│  │  │  │  ├─ 📄 FeaturedForumStats.tsx
│  │  │  │  ├─ 📄 forum-card.tsx
│  │  │  │  ├─ 📄 forum-category-card.tsx
│  │  │  │  ├─ 📄 forum-filters.tsx
│  │  │  │  ├─ 📄 forum-guidelines.tsx
│  │  │  │  ├─ 📄 ForumErrorBoundary.tsx
│  │  │  │  ├─ 📄 ForumPage.tsx
│  │  │  │  ├─ 📄 HotTopics.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 ModeratorActions.tsx
│  │  │  │  ├─ 📄 ModernForumList.tsx
│  │  │  │  ├─ 📄 ModernQuickStats.tsx
│  │  │  │  ├─ 📄 ModernThreadList.tsx
│  │  │  │  ├─ 📄 MyBBActiveMembers.tsx
│  │  │  │  ├─ 📄 MyBBBreadcrumb.tsx
│  │  │  │  ├─ 📄 MyBBCategoryFilter.tsx
│  │  │  │  ├─ 📄 MyBBFeaturedNav.tsx
│  │  │  │  ├─ 📄 MyBBForumGuidelines.tsx
│  │  │  │  ├─ 📄 MyBBForumList.tsx
│  │  │  │  ├─ 📄 MyBBPostCard.tsx
│  │  │  │  ├─ 📄 MyBBQuickStats.tsx
│  │  │  │  ├─ 📄 MyBBReplyForm.tsx
│  │  │  │  ├─ 📄 MyBBStats.tsx
│  │  │  │  ├─ 📄 MyBBThreadList.tsx
│  │  │  │  ├─ 📄 OnlineUsersDisplay.tsx
│  │  │  │  ├─ 📄 OriginForumPill.tsx
│  │  │  │  ├─ 📄 prefix-badge.tsx
│  │  │  │  ├─ 📄 QuickReplyInput.tsx
│  │  │  │  ├─ 📄 QuickStats.tsx
│  │  │  │  ├─ 📄 ReactionBar.tsx
│  │  │  │  ├─ 📄 RecentActivity.tsx
│  │  │  │  ├─ 📄 ShareButton.tsx
│  │  │  │  ├─ 📄 ShopCard.tsx
│  │  │  │  ├─ 📄 SignatureRenderer.tsx
│  │  │  │  ├─ 📄 SolveBadge.tsx
│  │  │  │  ├─ 📄 StickyBackButton.tsx
│  │  │  │  ├─ 📄 tag-input.tsx
│  │  │  │  ├─ 📄 ThreadAuthor.tsx
│  │  │  │  ├─ 📄 ThreadCard.tsx
│  │  │  │  ├─ 📄 ThreadFilters.tsx
│  │  │  │  ├─ 📄 ThreadPagination.tsx
│  │  │  │  ├─ 📄 ThreadRow.tsx
│  │  │  │  ├─ 📄 ThreadSidebar.tsx
│  │  │  │  ├─ 📄 ThreadStats.tsx
│  │  │  │  ├─ 📄 UserLevelDisplay.tsx
│  │  │  │  └─ 📄 XpBoostBadge.tsx
│  │  │  ├─ 📁 gamification/
│  │  │  │  ├─ 📄 achievement-card.tsx
│  │  │  │  ├─ 📄 achievement-grid.tsx
│  │  │  │  ├─ 📄 achievement-unlock-modal.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 leaderboard.tsx
│  │  │  │  ├─ 📄 level-display.tsx
│  │  │  │  ├─ 📄 level-up-modal.tsx
│  │  │  │  ├─ 📄 profile-gamification-widget.tsx
│  │  │  │  ├─ 📄 progression-card.tsx
│  │  │  │  └─ 📄 ProgressPath.tsx
│  │  │  ├─ 📁 header/
│  │  │  │  ├─ 📄 AdminButton.tsx
│  │  │  │  ├─ 📄 AuthButtons.tsx
│  │  │  │  ├─ 📄 HeaderContext.tsx
│  │  │  │  ├─ 📄 HeaderPluginSlot.tsx
│  │  │  │  ├─ 📄 HeaderThemeWrapper.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 Logo.tsx
│  │  │  │  ├─ 📄 MobileNav.tsx
│  │  │  │  ├─ 📄 NavLink.tsx
│  │  │  │  ├─ 📄 NotificationButton.tsx
│  │  │  │  ├─ 📄 PrimaryNav.tsx
│  │  │  │  ├─ 📄 SearchBox.tsx
│  │  │  │  ├─ 📄 SiteHeader.tsx
│  │  │  │  ├─ 📄 UserMenu.tsx
│  │  │  │  └─ 📄 WalletButton.tsx
│  │  │  ├─ 📁 icons/
│  │  │  │  ├─ 📁 custom/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 icon-usage-snapshot.txt
│  │  │  │  ├─ 📄 iconLoader.ts
│  │  │  │  ├─ 📄 iconMap.config.ts
│  │  │  │  ├─ 📄 iconRenderer.tsx
│  │  │  │  ├─ 📄 README.md
│  │  │  │  └─ 📄 types.ts
│  │  │  ├─ 📁 identity/
│  │  │  │  ├─ 📄 AvatarFrame.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 LevelBadge.tsx
│  │  │  │  ├─ 📄 README.md
│  │  │  │  ├─ 📄 RoleBadge.tsx
│  │  │  │  └─ 📄 UserName.tsx
│  │  │  ├─ 📁 layout/
│  │  │  │  ├─ 📄 announcement-ticker.css
│  │  │  │  ├─ 📄 announcement-ticker.tsx
│  │  │  │  ├─ 📄 AppSidebar.tsx
│  │  │  │  ├─ 📄 hero-section.tsx
│  │  │  │  ├─ 📄 LayoutRenderer.tsx
│  │  │  │  ├─ 📄 ProfileBackground.tsx
│  │  │  │  ├─ 📄 ResponsiveLayoutWrapper.tsx
│  │  │  │  ├─ 📄 sidebar.tsx
│  │  │  │  ├─ 📄 SidebarNavigation.tsx
│  │  │  │  ├─ 📄 site-layout-wrapper.tsx
│  │  │  │  ├─ 📄 SlotRenderer.tsx
│  │  │  │  ├─ 📄 WidgetFrame.tsx
│  │  │  │  └─ 📄 WidgetGallery.tsx
│  │  │  ├─ 📁 leaderboard/
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 leaderboard-theme.ts
│  │  │  │  └─ 📄 LeaderboardWidget.tsx
│  │  │  ├─ 📁 media/
│  │  │  │  └─ 📄 MediaAsset.tsx
│  │  │  ├─ 📁 mentions/
│  │  │  │  ├─ 📄 MentionAutocomplete.tsx
│  │  │  │  ├─ 📄 MentionContentItem.tsx
│  │  │  │  └─ 📄 MentionRenderer.tsx
│  │  │  ├─ 📁 messages/
│  │  │  │  ├─ 📁 icons/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 WhisperButton.tsx
│  │  │  │  ├─ 📄 WhisperModal.tsx
│  │  │  │  └─ 📄 WhispersInbox.tsx
│  │  │  ├─ 📁 mod/
│  │  │  │  ├─ 📄 mod-layout.tsx
│  │  │  │  └─ 📄 mod-sidebar.tsx
│  │  │  ├─ 📁 modals/
│  │  │  │  ├─ 📄 DeleteConfirmModal.tsx
│  │  │  │  ├─ 📄 QuotePostModal.tsx
│  │  │  │  └─ 📄 TipPostModal.tsx
│  │  │  ├─ 📁 navigation/
│  │  │  │  ├─ 📄 ForumBreadcrumbs.tsx
│  │  │  │  ├─ 📄 mobile-nav-bar.tsx
│  │  │  │  └─ 📄 nav-item.tsx
│  │  │  ├─ 📁 notifications/
│  │  │  │  ├─ 📄 MentionsNotifications.tsx
│  │  │  │  └─ 📄 NotificationPanel.tsx
│  │  │  ├─ 📁 payment/
│  │  │  │  ├─ 📄 PaymentForm.tsx
│  │  │  │  └─ 📄 StripeElementsWrapper.tsx
│  │  │  ├─ 📁 platform-energy/
│  │  │  │  ├─ 📁 featured-threads/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 hot-threads/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 leaderboards/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 recent-posts/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 stats/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📄 index.ts
│  │  │  ├─ 📁 preferences/
│  │  │  │  ├─ 📄 account-preferences.tsx
│  │  │  │  ├─ 📄 display-preferences.tsx
│  │  │  │  ├─ 📄 notification-preferences.tsx
│  │  │  │  ├─ 📄 PreferencesCard.tsx
│  │  │  │  ├─ 📄 PreferencesGroup.tsx
│  │  │  │  ├─ 📄 PreferencesInput.tsx
│  │  │  │  ├─ 📄 PreferencesSelect.tsx
│  │  │  │  ├─ 📄 PreferencesTextarea.tsx
│  │  │  │  ├─ 📄 PreferencesToggle.tsx
│  │  │  │  ├─ 📄 profile-preferences.tsx
│  │  │  │  ├─ 📄 referral-preferences.tsx
│  │  │  │  ├─ 📄 session-preferences.tsx
│  │  │  │  └─ 📄 social-preferences.tsx
│  │  │  ├─ 📁 profile/
│  │  │  │  ├─ 📁 widgets/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 AchievementsTab.tsx
│  │  │  │  ├─ 📄 CosmeticControlPanel.tsx
│  │  │  │  ├─ 📄 FriendsTab.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 InventoryTab.tsx
│  │  │  │  ├─ 📄 OverviewTab.tsx
│  │  │  │  ├─ 📄 ProfileDashboard.tsx
│  │  │  │  ├─ 📄 ProfileEditor.tsx
│  │  │  │  ├─ 📄 ProfileNavigation.tsx
│  │  │  │  ├─ 📄 ProfileSidebar.tsx
│  │  │  │  ├─ 📄 ProfileSkeleton.tsx
│  │  │  │  ├─ 📄 rarityUtils.ts
│  │  │  │  ├─ 📄 README.md
│  │  │  │  ├─ 📄 SignatureRenderer.tsx
│  │  │  │  ├─ 📄 StatCard.tsx
│  │  │  │  ├─ 📄 UnifiedProfileCard.tsx
│  │  │  │  ├─ 📄 UserBadges.tsx
│  │  │  │  ├─ 📄 UserProfileRenderer.tsx
│  │  │  │  ├─ 📄 UserTitles.tsx
│  │  │  │  ├─ 📄 WhaleWatchTab.tsx
│  │  │  │  ├─ 📄 XpLogView.tsx
│  │  │  │  ├─ 📄 XPProfileSection.tsx
│  │  │  │  └─ 📄 XPProgressBar.tsx
│  │  │  ├─ 🤝 shared/ - Isomorphic code (client & server)
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  └─ 📄 ProfileCard.tsx
│  │  │  ├─ 📁 shop/
│  │  │  │  ├─ 📄 purchase-modal.tsx
│  │  │  │  ├─ 📄 shop-item-card.tsx
│  │  │  │  └─ 📄 ShopItem.tsx
│  │  │  ├─ 📁 shoutbox/
│  │  │  │  ├─ 📄 EmojiPanel.tsx
│  │  │  │  ├─ 📄 EnhancedShoutbox.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 integration-example.tsx
│  │  │  │  ├─ 📄 positioned-shoutbox.tsx
│  │  │  │  ├─ 📄 RainAnimation.tsx
│  │  │  │  ├─ 📄 README.md
│  │  │  │  ├─ 📄 shoutbox-help-command.tsx
│  │  │  │  ├─ 📄 shoutbox-message-styles.tsx
│  │  │  │  ├─ 📄 shoutbox-position-selector.tsx
│  │  │  │  ├─ 📄 shoutbox-rain-notification.tsx
│  │  │  │  ├─ 📄 shoutbox-widget.tsx
│  │  │  │  ├─ 📄 ShoutboxContainer.tsx
│  │  │  │  └─ 📄 ShoutboxInput.tsx
│  │  │  ├─ 📁 sidebar/
│  │  │  │  ├─ 📄 leaderboard-widget.tsx
│  │  │  │  ├─ 📄 navigation-widget.tsx
│  │  │  │  ├─ 📄 related-content-widget.tsx
│  │  │  │  └─ 📄 wallet-summary-widget.tsx
│  │  │  ├─ 📁 skeletons/
│  │  │  │  ├─ 📄 FeaturedForumCardSkeleton.tsx
│  │  │  │  └─ 📄 HomePageSkeleton.tsx
│  │  │  ├─ 📁 social/
│  │  │  │  ├─ 📄 FollowButton.tsx
│  │  │  │  ├─ 📄 FollowingList.tsx
│  │  │  │  ├─ 📄 FriendsManager.tsx
│  │  │  │  ├─ 📄 WhaleWatchDashboard.tsx
│  │  │  │  └─ 📄 WhaleWatchDisplay.tsx
│  │  │  ├─ 📁 ui/
│  │  │  │  ├─ 📄 accessibility-enhancements.tsx
│  │  │  │  ├─ 📄 accordion.tsx
│  │  │  │  ├─ 📄 alert-dialog.tsx
│  │  │  │  ├─ 📄 alert.tsx
│  │  │  │  ├─ 📄 animated-logo.tsx
│  │  │  │  ├─ 📄 aspect-ratio.tsx
│  │  │  │  ├─ 📄 avatar.tsx
│  │  │  │  ├─ 📄 badge.tsx
│  │  │  │  ├─ 📄 bookmark-button.tsx
│  │  │  │  ├─ 📄 breadcrumb.tsx
│  │  │  │  ├─ 📄 button.tsx
│  │  │  │  ├─ 📄 calendar.tsx
│  │  │  │  ├─ 📄 candlestick-menu.tsx
│  │  │  │  ├─ 📄 card.tsx
│  │  │  │  ├─ 📄 carousel.tsx
│  │  │  │  ├─ 📄 chart.tsx
│  │  │  │  ├─ 📄 checkbox.tsx
│  │  │  │  ├─ 📄 collapsible.tsx
│  │  │  │  ├─ 📄 command.tsx
│  │  │  │  ├─ 📄 Container.tsx
│  │  │  │  ├─ 📄 CONTENT_FEED_SUMMARY.md
│  │  │  │  ├─ 📄 content-area.tsx
│  │  │  │  ├─ 📄 content-feed-demo.tsx
│  │  │  │  ├─ 📄 content-feed.tsx
│  │  │  │  ├─ 📄 context-menu.tsx
│  │  │  │  ├─ 📄 dialog.tsx
│  │  │  │  ├─ 📄 drawer.tsx
│  │  │  │  ├─ 📄 dropdown-menu.tsx
│  │  │  │  ├─ 📄 enhanced-button.tsx
│  │  │  │  ├─ 📄 enhanced-loading-states.tsx
│  │  │  │  ├─ 📄 enhanced-notifications.tsx
│  │  │  │  ├─ 📄 error-display.tsx
│  │  │  │  ├─ 📄 feature-gate.tsx
│  │  │  │  ├─ 📄 feed-filters.tsx
│  │  │  │  ├─ 📄 file-drop-zone.tsx
│  │  │  │  ├─ 📄 form.tsx
│  │  │  │  ├─ 📄 frost-card.tsx
│  │  │  │  ├─ 📄 hamburger.tsx
│  │  │  │  ├─ 📄 hover-card.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 input-otp.tsx
│  │  │  │  ├─ 📄 input.tsx
│  │  │  │  ├─ 📄 label.tsx
│  │  │  │  ├─ 📄 loader.tsx
│  │  │  │  ├─ 📄 LoadingIndicator.tsx
│  │  │  │  ├─ 📄 menu.tsx
│  │  │  │  ├─ 📄 menubar.tsx
│  │  │  │  ├─ 📄 mobile-forum-nav.tsx
│  │  │  │  ├─ 📄 navigation-menu.tsx
│  │  │  │  ├─ 📄 pagination.tsx
│  │  │  │  ├─ 📄 performance-monitor.tsx
│  │  │  │  ├─ 📄 popover.tsx
│  │  │  │  ├─ 📄 progress.tsx
│  │  │  │  ├─ 📄 radio-group.tsx
│  │  │  │  ├─ 📄 reactions-bar.tsx
│  │  │  │  ├─ 📄 resizable.tsx
│  │  │  │  ├─ 📄 safe-image.tsx
│  │  │  │  ├─ 📄 scroll-area.tsx
│  │  │  │  ├─ 📄 select.tsx
│  │  │  │  ├─ 📄 seo-head.tsx
│  │  │  │  ├─ 📄 separator.tsx
│  │  │  │  ├─ 📄 sheet.tsx
│  │  │  │  ├─ 📄 sidebar.tsx
│  │  │  │  ├─ 📄 skeleton.tsx
│  │  │  │  ├─ 📄 slider.tsx
│  │  │  │  ├─ 📄 smart-thread-filters.tsx
│  │  │  │  ├─ 📄 stat-chip.tsx
│  │  │  │  ├─ 📄 switch.tsx
│  │  │  │  ├─ 📄 tab-switcher.tsx
│  │  │  │  ├─ 📄 table.tsx
│  │  │  │  ├─ 📄 tabs.tsx
│  │  │  │  ├─ 📄 tag-badge.tsx
│  │  │  │  ├─ 📄 textarea.tsx
│  │  │  │  ├─ 📄 thread-skeleton.tsx
│  │  │  │  ├─ 📄 toast.tsx
│  │  │  │  ├─ 📄 toaster.tsx
│  │  │  │  ├─ 📄 toggle-group.tsx
│  │  │  │  ├─ 📄 toggle.tsx
│  │  │  │  ├─ 📄 tooltip-utils.tsx
│  │  │  │  ├─ 📄 tooltip.tsx
│  │  │  │  ├─ 📄 user-badge.tsx
│  │  │  │  ├─ 📄 user-title.tsx
│  │  │  │  ├─ 📄 virtualized-content-feed.tsx
│  │  │  │  └─ 📄 widget-skeleton.tsx
│  │  │  ├─ 📁 uiverse/
│  │  │  │  ├─ 📄 AnimatedCheckbox.css
│  │  │  │  ├─ 📄 AnimatedCheckbox.tsx
│  │  │  │  ├─ 📄 BrutalistCard.css
│  │  │  │  ├─ 📄 BrutalistCard.tsx
│  │  │  │  ├─ 📄 CopeButton.css
│  │  │  │  ├─ 📄 CopeButton.tsx
│  │  │  │  ├─ 📄 CTAButton.css
│  │  │  │  ├─ 📄 CTAButton.tsx
│  │  │  │  ├─ 📄 DegenLoader.css
│  │  │  │  ├─ 📄 DegenLoader.tsx
│  │  │  │  ├─ 📄 DeleteButton.css
│  │  │  │  ├─ 📄 DeleteButton.tsx
│  │  │  │  ├─ 📄 DiscordButton.css
│  │  │  │  ├─ 📄 DiscordButton.tsx
│  │  │  │  ├─ 📄 GridBackground.css
│  │  │  │  ├─ 📄 GridBackground.tsx
│  │  │  │  ├─ 📄 InfiniteMarquee.css
│  │  │  │  ├─ 📄 InfiniteMarquee.tsx
│  │  │  │  ├─ 📄 LikeButton.css
│  │  │  │  ├─ 📄 LikeButton.tsx
│  │  │  │  ├─ 📄 MacTerminal.css
│  │  │  │  ├─ 📄 MacTerminal.tsx
│  │  │  │  ├─ 📄 PumpButton.css
│  │  │  │  ├─ 📄 PumpButton.tsx
│  │  │  │  ├─ 📄 RadarLoader.css
│  │  │  │  ├─ 📄 RadarLoader.tsx
│  │  │  │  ├─ 📄 Red3DButton.css
│  │  │  │  ├─ 📄 Red3DButton.tsx
│  │  │  │  ├─ 📄 RevenueWidget.css
│  │  │  │  ├─ 📄 RevenueWidget.tsx
│  │  │  │  ├─ 📄 ShareButton.css
│  │  │  │  ├─ 📄 ShareButton.tsx
│  │  │  │  ├─ 📄 ShopCard3D.css
│  │  │  │  ├─ 📄 ShopCard3D.tsx
│  │  │  │  ├─ 📄 SleepyLoader.css
│  │  │  │  ├─ 📄 SleepyLoader.tsx
│  │  │  │  ├─ 📄 SubscribeInput.css
│  │  │  │  └─ 📄 SubscribeInput.tsx
│  │  │  ├─ 📁 uiverse-clones/
│  │  │  │  ├─ 📁 buttons/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 loaders/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📄 index.ts
│  │  │  ├─ 📁 users/
│  │  │  │  ├─ 📄 ActiveMembersWidget.tsx
│  │  │  │  ├─ 📄 framed-avatar.tsx
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 UserAvatar.tsx
│  │  │  │  ├─ 📄 UserCard.tsx
│  │  │  │  ├─ 📄 UserDirectoryTable.tsx
│  │  │  │  ├─ 📄 UserFilters.tsx
│  │  │  │  └─ 📄 Username.tsx
│  │  │  ├─ 📁 widgets/
│  │  │  │  ├─ 📄 ProfileCardWidget.tsx
│  │  │  │  ├─ 📄 README.md
│  │  │  │  └─ 📄 WalletSummaryWidgetWrapper.tsx
│  │  │  └─ 📁 xp/
│  │  │     ├─ 📄 LevelUpModal.tsx
│  │  │     ├─ 📄 tracks.ts
│  │  │     ├─ 📄 XPBarsContainer.tsx
│  │  │     ├─ 📄 XPBarTrack.tsx
│  │  │     └─ 📄 XpToast.tsx
│  │  ├─ 🔧 config/ - Shared build/tooling configuration
│  │  │  ├─ 📄 admin.config.ts
│  │  │  ├─ 📄 animation.config.ts
│  │  │  ├─ 📄 brand.config.ts
│  │  │  ├─ 📄 easter-eggs.config.ts
│  │  │  ├─ 📄 featureFlags.ts
│  │  │  ├─ 📄 fonts.config.ts
│  │  │  ├─ 📄 footer-navigation.ts
│  │  │  ├─ 📄 forum-layouts.config.ts
│  │  │  ├─ 📄 forumMap.ts
│  │  │  ├─ 📄 frames.config.ts
│  │  │  ├─ 📄 index.ts
│  │  │  ├─ 📄 mybb-theme.config.ts
│  │  │  ├─ 📄 navigation.ts
│  │  │  ├─ 📄 pageSlotMap.ts
│  │  │  ├─ 📄 pagination.config.ts
│  │  │  ├─ 📄 publicConfig.ts
│  │  │  ├─ 📄 rarity.config.ts
│  │  │  ├─ 📄 README.md
│  │  │  ├─ 📄 shop-items.config.js
│  │  │  ├─ 📄 shop-items.config.ts
│  │  │  ├─ 📄 sidebarWidgets.config.ts
│  │  │  ├─ 📄 tags.config.ts
│  │  │  ├─ 📄 THEME_CONFIG_README.md
│  │  │  ├─ 📄 theme.config.example.tsx
│  │  │  ├─ 📄 theme.config.ts
│  │  │  ├─ 📄 themeConstants.ts
│  │  │  ├─ 📄 themeFallbacks.ts
│  │  │  ├─ 📄 thread.config.ts
│  │  │  ├─ 📄 ui.config.ts
│  │  │  └─ 📄 widgetRegistry.ts
│  │  ├─ 📁 constants/
│  │  │  ├─ 📄 apiRoutes.ts
│  │  │  ├─ 📄 env.ts
│  │  │  ├─ 📄 routes.ts
│  │  │  └─ 📄 websocket-disabled.ts
│  │  ├─ 🔄 contexts/ - React context providers
│  │  │  ├─ 📄 AdminSidebarContext.tsx
│  │  │  ├─ 📄 content-feed-context.tsx
│  │  │  ├─ 📄 ForumViewThemeContext.tsx
│  │  │  ├─ 📄 LevelUpContext.tsx
│  │  │  ├─ 📄 mock-shoutbox-context.tsx
│  │  │  ├─ 📄 MotionContext.tsx
│  │  │  ├─ 📄 ProfileCardContext.tsx
│  │  │  ├─ 📄 safe-shoutbox-provider.tsx
│  │  │  ├─ 📄 shoutbox-context.tsx
│  │  │  ├─ 📄 UIConfigContext.tsx
│  │  │  ├─ 📄 UiverseConfigContext.tsx
│  │  │  ├─ 📄 wallet-context.tsx
│  │  │  └─ 📄 XpToastContext.tsx
│  │  ├─ 📁 core/
│  │  │  ├─ 📄 api.ts
│  │  │  ├─ 📄 constants.ts
│  │  │  ├─ 📄 queryClient.ts
│  │  │  └─ 📄 router.tsx
│  │  ├─ 📁 design/
│  │  │  └─ 📁 tokens/
│  │  │     ├─ 📄 colors.json
│  │  │     ├─ 📄 spacing.json
│  │  │     └─ 📄 typography.json
│  │  ├─ 📚 docs/ - Project documentation
│  │  │  └─ 📄 MOTION_PREFERENCES.md
│  │  ├─ 📁 features/
│  │  │  ├─ 📁 activity/
│  │  │  │  ├─ 🧩 components/ - Reusable UI components
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🎣 hooks/ - Custom React hooks
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 types/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 admin/
│  │  │  │  ├─ 📁 __tests__/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 analytics/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 api/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 clout/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🧩 components/ - Reusable UI components
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 cosmetics/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 dashboard/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 economy/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 effects/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 forum/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🎣 hooks/ - Custom React hooks
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 layout/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 lib/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 media/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 reports/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 shop/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 styles/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 users/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 views/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 wallet/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 xp/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  └─ 📄 README.md
│  │  │  ├─ 📁 auth/
│  │  │  ├─ 📁 dictionary/
│  │  │  │  ├─ 🧩 components/ - Reusable UI components
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 services/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 forum/
│  │  │  │  ├─ 🧩 components/ - Reusable UI components
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🔄 contexts/ - React context providers
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🎣 hooks/ - Custom React hooks
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 services/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 gamification/
│  │  │  │  ├─ 🧩 components/ - Reusable UI components
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  └─ 📄 README.md
│  │  │  ├─ 📁 users/
│  │  │  │  ├─ 🎣 hooks/ - Custom React hooks
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 services/
│  │  │  │     ... (max depth reached)
│  │  │  └─ 📁 wallet/
│  │  │     ├─ 🧩 components/ - Reusable UI components
│  │  │     │  ... (max depth reached)
│  │  │     ├─ 📁 services/
│  │  │     │  ... (max depth reached)
│  │  │     └─ 📄 index.ts
│  │  ├─ 🎣 hooks/ - Custom React hooks
│  │  │  ├─ 📁 preferences/
│  │  │  │  ├─ 📄 useUpdateUserSettings.ts
│  │  │  │  └─ 📄 useUserSettings.ts
│  │  │  ├─ 📄 index.ts
│  │  │  ├─ 📄 README.md
│  │  │  ├─ 📄 use-achievements.ts
│  │  │  ├─ 📄 use-admin-modules.ts
│  │  │  ├─ 📄 use-async-button.tsx
│  │  │  ├─ 📄 use-auth-navigation.ts
│  │  │  ├─ 📄 use-auth.tsx
│  │  │  ├─ 📄 use-avatar-upload.ts
│  │  │  ├─ 📄 use-content-mentions.ts
│  │  │  ├─ 📄 use-content.ts
│  │  │  ├─ 📄 use-debounce.ts
│  │  │  ├─ 📄 use-dgt-packages.ts
│  │  │  ├─ 📄 use-draft.ts
│  │  │  ├─ 📄 use-gamification.tsx
│  │  │  ├─ 📄 use-image-upload.ts
│  │  │  ├─ 📄 use-infinite-content.ts
│  │  │  ├─ 📄 use-infinite-scroll.ts
│  │  │  ├─ 📄 use-local-storage.ts
│  │  │  ├─ 📄 use-lucia-auth.tsx
│  │  │  ├─ 📄 use-media-query.ts
│  │  │  ├─ 📄 use-mentions.ts
│  │  │  ├─ 📄 use-messages.tsx
│  │  │  ├─ 📄 use-mobile.tsx
│  │  │  ├─ 📄 use-notifications.ts
│  │  │  ├─ 📄 use-pending-transactions.ts
│  │  │  ├─ 📄 use-purchase-modal.tsx
│  │  │  ├─ 📄 use-rain-notifications.ts
│  │  │  ├─ 📄 use-rain.ts
│  │  │  ├─ 📄 use-shop-items.tsx
│  │  │  ├─ 📄 use-shop-ownership.tsx
│  │  │  ├─ 📄 use-tip.ts
│  │  │  ├─ 📄 use-toast.ts
│  │  │  ├─ 📄 use-user-by-username.ts
│  │  │  ├─ 📄 use-vault-items.tsx
│  │  │  ├─ 📄 use-wallet.ts
│  │  │  ├─ 📄 use-websocket-status.ts
│  │  │  ├─ 📄 useAuthToken.ts
│  │  │  ├─ 📄 useConfig.ts
│  │  │  ├─ 📄 useCrudMutation.ts
│  │  │  ├─ 📄 useCurrentForum.ts
│  │  │  ├─ 📄 useDailyBonus.ts
│  │  │  ├─ 📄 useDailyTasks.ts
│  │  │  ├─ 📄 useDgtPurchase.ts
│  │  │  ├─ 📄 useFeaturedForumStats.ts
│  │  │  ├─ 📄 useFeatureGates.ts
│  │  │  ├─ 📄 useFollowing.ts
│  │  │  ├─ 📄 useForumFilters.ts
│  │  │  ├─ 📄 useFriends.ts
│  │  │  ├─ 📄 useIdentityDisplay.ts
│  │  │  ├─ 📄 useJsonConfig.ts
│  │  │  ├─ 📄 useMediaQuery.ts
│  │  │  ├─ 📄 usePermission.ts
│  │  │  ├─ 📄 useProfileEngagement.ts
│  │  │  ├─ 📄 useProfileStats.ts
│  │  │  ├─ 📄 useRouteProtection.ts
│  │  │  ├─ 📄 useSearchParams.ts
│  │  │  ├─ 📄 useShowHotRibbon.ts
│  │  │  ├─ 📄 useTheme.ts
│  │  │  ├─ 📄 useUserCosmetics.ts
│  │  │  ├─ 📄 useUserInventory.ts
│  │  │  ├─ 📄 useUserXP.ts
│  │  │  ├─ 📄 useWebSocket.tsx
│  │  │  ├─ 📄 useXP.ts
│  │  │  ├─ 📄 useXpEvents.ts
│  │  │  └─ 📄 widgetData.ts
│  │  ├─ 📁 layout/
│  │  │  └─ 📁 primitives/
│  │  │     ├─ 📄 ClampPadding.tsx
│  │  │     ├─ 📄 FullBleedSection.tsx
│  │  │     ├─ 📄 index.tsx
│  │  │     ├─ 📄 PageBackdrop.tsx
│  │  │     ├─ 📄 Prose.tsx
│  │  │     ├─ 📄 StickyRegion.tsx
│  │  │     └─ 📄 Wide.tsx
│  │  ├─ 📁 layouts/
│  │  │  ├─ 📄 AdminLayout.tsx
│  │  │  ├─ 📄 ModLayout.tsx
│  │  │  └─ 📄 RootLayout.tsx
│  │  ├─ 📁 lib/
│  │  │  ├─ 📁 forum/
│  │  │  │  ├─ 📄 canUserPost.ts
│  │  │  │  └─ 📄 getForumRules.ts
│  │  │  ├─ 📁 utils/
│  │  │  │  ├─ 📄 animateNumber.ts
│  │  │  │  ├─ 📄 api-helpers.ts
│  │  │  │  ├─ 📄 category.ts
│  │  │  │  └─ 📄 generateSlug.ts
│  │  │  ├─ 📄 admin-module-registry.ts
│  │  │  ├─ 📄 admin-module-transformer.ts
│  │  │  ├─ 📄 api.ts
│  │  │  ├─ 📄 app-init.ts
│  │  │  ├─ 📄 logger.ts
│  │  │  └─ 📄 sentry.ts
│  │  ├─ 📁 navigation/
│  │  │  └─ 📄 forumNav.ts
│  │  ├─ 📄 pages/ - Page components & routing
│  │  │  ├─ 📁 _archived/
│  │  │  │  └─ 📁 missions/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 about/
│  │  │  │  └─ 📄 index.tsx
│  │  │  ├─ 📁 admin/
│  │  │  │  ├─ 📁 activity/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 announcements/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🔧 config/ - Shared build/tooling configuration
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 dictionary/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 features/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 missions/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 reputation/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 ui/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 users/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 xp/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 brand-config.tsx
│  │  │  │  ├─ 📄 emojis.tsx
│  │  │  │  ├─ 📄 feature-flags.tsx
│  │  │  │  ├─ 📄 live-database.tsx
│  │  │  │  ├─ 📄 social-config.tsx
│  │  │  │  ├─ 📄 ui-components-config.tsx
│  │  │  │  └─ 📄 ui-config.tsx
│  │  │  ├─ 📁 announcements/
│  │  │  │  └─ 📄 index.tsx
│  │  │  ├─ 📁 dev/
│  │  │  │  ├─ 📁 profiles/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 ControlsDrawer.tsx
│  │  │  │  ├─ 📄 DemoCard.tsx
│  │  │  │  ├─ 📄 DevPlaygroundLayout.tsx
│  │  │  │  └─ 📄 index.ts
│  │  │  ├─ 📁 dictionary/
│  │  │  │  ├─ 📄 [slug].tsx
│  │  │  │  └─ 📄 index.tsx
│  │  │  ├─ 📁 forums/
│  │  │  │  ├─ 📄 [forumSlug].tsx
│  │  │  │  ├─ 📄 index.tsx
│  │  │  │  └─ 📄 search.tsx
│  │  │  ├─ 📁 invite/
│  │  │  │  └─ 📄 [code].tsx
│  │  │  ├─ 📁 mod/
│  │  │  │  ├─ 📄 activity.tsx
│  │  │  │  ├─ 📄 index.tsx
│  │  │  │  ├─ 📄 reports.tsx
│  │  │  │  ├─ 📄 shoutbox.tsx
│  │  │  │  └─ 📄 users.tsx
│  │  │  ├─ 📁 preferences/
│  │  │  │  └─ 📄 index.tsx
│  │  │  ├─ 📁 profile/
│  │  │  │  ├─ 📄 activity.tsx
│  │  │  │  └─ 📄 xp.tsx
│  │  │  ├─ 📁 referrals/
│  │  │  │  └─ 📄 index.tsx
│  │  │  ├─ 📁 search/
│  │  │  │  └─ 📄 index.tsx
│  │  │  ├─ 📁 shop/
│  │  │  │  └─ 📄 avatar-frames.tsx
│  │  │  ├─ 📁 shop-management/
│  │  │  │  ├─ 📄 dgt-purchase.tsx
│  │  │  │  └─ 📄 purchase-success.tsx
│  │  │  ├─ 📁 threads/
│  │  │  │  ├─ 📄 [slug].tsx
│  │  │  │  └─ 📄 create.tsx
│  │  │  ├─ 📁 ui-playground/
│  │  │  │  ├─ 📁 animations/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 sections/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📄 demo-data.ts
│  │  │  ├─ 📄 auth.tsx
│  │  │  ├─ 📄 degen-index.tsx
│  │  │  ├─ 📄 fixtures-dashboard.tsx
│  │  │  ├─ 📄 forum-rules.tsx
│  │  │  ├─ 📄 home.tsx
│  │  │  ├─ 📄 leaderboard.tsx
│  │  │  ├─ 📄 not-found.tsx
│  │  │  ├─ 📄 profile.tsx
│  │  │  ├─ 📄 progress.tsx
│  │  │  ├─ 📄 RouteProtectionDemo.tsx
│  │  │  ├─ 📄 shop.tsx
│  │  │  ├─ 📄 ui-playground.tsx
│  │  │  ├─ 📄 uiverse-showcase.tsx
│  │  │  ├─ 📄 wallet.tsx
│  │  │  └─ 📄 whispers.tsx
│  │  ├─ 📁 payments/
│  │  │  ├─ 📁 ccpayment/
│  │  │  │  ├─ 📄 ccpayment-client.ts
│  │  │  │  ├─ 📄 deposit.ts
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 README.md
│  │  │  │  ├─ 📄 swap.ts
│  │  │  │  ├─ 📄 types.ts
│  │  │  │  ├─ 📄 utils.ts
│  │  │  │  └─ 📄 withdraw.ts
│  │  │  ├─ 🤝 shared/ - Isomorphic code (client & server)
│  │  │  │  └─ 📄 index.ts
│  │  │  └─ 📄 index.ts
│  │  ├─ 📁 providers/
│  │  │  ├─ 📄 app-provider.tsx
│  │  │  └─ 📄 root-provider.tsx
│  │  ├─ 📁 schemas/
│  │  │  ├─ 📁 api/
│  │  │  │  ├─ 📄 auth.schema.ts
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 shop.schema.ts
│  │  │  │  └─ 📄 wallet.schema.ts
│  │  │  ├─ 🤝 shared/ - Isomorphic code (client & server)
│  │  │  │  ├─ 📄 branded-ids.ts
│  │  │  │  ├─ 📄 helpers.ts
│  │  │  │  └─ 📄 response.schema.ts
│  │  │  ├─ 📄 assertValid.ts
│  │  │  ├─ 📄 brand.schema.ts
│  │  │  ├─ 📄 featureFlags.schema.ts
│  │  │  ├─ 📄 index.ts
│  │  │  └─ 📄 uiQuotes.schema.ts
│  │  ├─ 📁 services/
│  │  │  └─ 📄 error.service.ts
│  │  ├─ 📁 stores/
│  │  │  ├─ 📄 useLayoutStore.ts
│  │  │  └─ 📄 usePlaygroundControls.ts
│  │  ├─ 📁 styles/
│  │  │  ├─ 📁 tokens/
│  │  │  │  └─ 📄 animations.css
│  │  │  ├─ 📄 accessibility.css
│  │  │  ├─ 📄 animations.css
│  │  │  ├─ 📄 avatar-frames.css
│  │  │  ├─ 📄 content-feed.css
│  │  │  ├─ 📄 cssVariables.ts
│  │  │  ├─ 📄 design-system.css
│  │  │  ├─ 📄 globals.css
│  │  │  ├─ 📄 layout.css
│  │  │  ├─ 📄 mybb-classic.css
│  │  │  ├─ 📄 titles.css
│  │  │  ├─ 📄 typography.css
│  │  │  ├─ 📄 underline-links.css
│  │  │  └─ 📄 utilities.css
│  │  ├─ 📁 types/
│  │  │  ├─ 📄 admin-title.types.ts
│  │  │  ├─ 📄 admin-vault.types.ts
│  │  │  ├─ 📄 admin.types.ts
│  │  │  ├─ 📄 canonical.types.js
│  │  │  ├─ 📄 core.types.ts
│  │  │  ├─ 📄 forum.ts
│  │  │  ├─ 📄 gamification.types.ts
│  │  │  ├─ 📄 ids.ts
│  │  │  ├─ 📄 inventory.ts
│  │  │  ├─ 📄 notifications.ts
│  │  │  ├─ 📄 payment.types.ts
│  │  │  ├─ 📄 preferences.types.ts
│  │  │  ├─ 📄 profile.ts
│  │  │  ├─ 📄 thread.types.ts
│  │  │  ├─ 📄 user.ts
│  │  │  ├─ 📄 vault.types.ts
│  │  │  └─ 📄 wallet.ts
│  │  ├─ 📁 utils/
│  │  │  ├─ 📁 api/
│  │  │  │  └─ 📄 achievements.ts
│  │  │  ├─ 📁 dev/
│  │  │  │  └─ 📄 mockProfile.ts
│  │  │  ├─ 📁 forum/
│  │  │  │  ├─ 📄 breadcrumbs.ts
│  │  │  │  ├─ 📄 formatting.ts
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 routing.ts
│  │  │  │  ├─ 📄 sidebarUtils.ts
│  │  │  │  ├─ 📄 stats.ts
│  │  │  │  └─ 📄 urls.ts
│  │  │  ├─ 📄 adaptiveSpacing.ts
│  │  │  ├─ 📄 animateNumber.ts
│  │  │  ├─ 📄 api-client.ts
│  │  │  ├─ 📄 api-request.ts
│  │  │  ├─ 📄 api-response.ts
│  │  │  ├─ 📄 api.ts
│  │  │  ├─ 📄 applyPluginRewards.ts
│  │  │  ├─ 📄 auth-token.ts
│  │  │  ├─ 📄 avatar.ts
│  │  │  ├─ 📄 card-constants.ts
│  │  │  ├─ 📄 debug-widgets.ts
│  │  │  ├─ 📄 format-date.ts
│  │  │  ├─ 📄 format.ts
│  │  │  ├─ 📄 formatters.ts
│  │  │  ├─ 📄 generateSlug.ts
│  │  │  ├─ 📄 queryClient.ts
│  │  │  ├─ 📄 rare-items-vault.ts
│  │  │  ├─ 📄 react-query-helpers.ts
│  │  │  ├─ 📄 README-MIGRATION.md
│  │  │  ├─ 📄 roles.ts
│  │  │  ├─ 📄 routeConfig.ts
│  │  │  ├─ 📄 safeWebSocket.ts
│  │  │  ├─ 📄 spacing-constants.ts
│  │  │  └─ 📄 utils.ts
│  │  ├─ 📄 App.tsx
│  │  ├─ 📄 AUTH_ANALYSIS.md
│  │  ├─ 📄 index.css
│  │  ├─ 📄 main.tsx
│  │  ├─ 📄 Router.tsx
│  ├─ 📄 .env.example
│  ├─ 📄 .eslintignore
│  ├─ 📄 components.json
│  ├─ 📄 index.html
│  ├─ 📄 package.json
│  ├─ 📄 postcss.config.cjs
│  ├─ 📄 README.md
│  ├─ 📄 style-dictionary.config.js
│  ├─ 📄 tailwind.config.js
│  ├─ 📄 tsconfig.eslint.json
│  ├─ 📄 tsconfig.json
│  ├─ 📄 uiverse-components.config.ts
│  ├─ 📄 vite.config.ts
│  └─ 📄 vitest.config.ts
├─ 🗄️ db/ - Database schema, migrations, and seeds
│  ├─ 📁 .tscache/
│  │  └─ 📄 .tsbuildinfo
│  ├─ 📁 enums/
│  │  ├─ 📄 index.ts
│  │  ├─ 📄 user.js
│  │  └─ 📄 user.ts
│  ├─ 📜 migrations/ - Database migration files
│  │  ├─ 📁 2025-06-24_admin_performance_indices/
│  │  │  ├─ 📄 down.sql
│  │  │  ├─ 📄 metadata.json
│  │  │  └─ 📄 migration.sql
│  │  ├─ 📁 2025-06-24_level_visual_fields/
│  │  │  ├─ 📄 down.sql
│  │  │  ├─ 📄 metadata.json
│  │  │  └─ 📄 migration.sql
│  │  ├─ 📁 archive/
│  │  │  ├─ 📄 0007_romantic_colossus.sql
│  │  │  ├─ 📄 0007_smooth_sphinx.sql
│  │  │  └─ 📄 canonical-zones-schema-update.ts
│  │  ├─ 📁 meta/
│  │  │  ├─ 📄 _journal.json
│  │  │  ├─ 📄 0006_snapshot.json
│  │  │  └─ 📄 0007_snapshot.json
│  │  ├─ 📁 postgres/
│  │  │  ├─ 📁 meta/
│  │  │  │  ├─ 📄 _journal.json
│  │  │  │  ├─ 📄 0000_snapshot.json
│  │  │  │  └─ 📄 0001_snapshot.json
│  │  │  ├─ 📄 0000_glamorous_marvex.sql
│  │  │  ├─ 📄 0000_silky_drax.sql
│  │  │  ├─ 📄 0001_happy_vulture.sql
│  │  │  ├─ 📄 0001_jazzy_red_hulk.sql
│  │  │  ├─ 📄 0001_premium_magik.sql
│  │  │  ├─ 📄 0002_closed_romulus.sql
│  │  │  ├─ 📄 0003_fluffy_terror.sql
│  │  │  ├─ 📄 0004_enhanced_profiles.sql
│  │  │  ├─ 📄 0004_high_magneto.sql
│  │  │  ├─ 📄 0005_sparkling_sphinx.sql
│  │  │  ├─ 📄 0006_funny_speedball.sql
│  │  │  ├─ 📄 0007_shiny_stepford_cuckoos.sql
│  │  │  ├─ 📄 0008_round_loners.sql
│  │  │  ├─ 📄 0009_nappy_skreet.sql
│  │  │  ├─ 📄 0010_tiny_vin_gonzales.sql
│  │  │  ├─ 📄 0011_motionless_malice.sql
│  │  │  ├─ 📄 0012_bright_sleeper.sql
│  │  │  ├─ 📄 0013_fantastic_hex.sql
│  │  │  ├─ 📄 0014_dazzling_silhouette.sql
│  │  │  ├─ 📄 0014_dev_uuid_schema_fixes.sql
│  │  │  ├─ 📄 0014_fix_uuid_foreign_keys.sql
│  │  │  ├─ 📄 0015_add_foreign_key_indexes.sql
│  │  │  ├─ 📄 0100_drop_mission_tables.sql
│  │  │  ├─ 📄 relations.ts
│  │  │  └─ 📄 schema.snapshot.ts
│  │  ├─ 📄 0008_add_users_profile_fields.sql
│  │  ├─ 📄 0009_create_xp_logs_table.sql
│  │  ├─ 📄 0010_add_x_account_fields.sql
│  │  ├─ 📄 0011_create_x_shares_table.sql
│  │  ├─ 📄 0030_add_announcements.sql
│  │  ├─ 📄 2025-06-24_backup_restore_system.sql
│  │  ├─ 📄 2025-06-24_email_templates.sql
│  │  ├─ 📄 2025-06-24_sticker_system.sql
│  │  ├─ 📄 20250115_performance_indexes.sql
│  │  ├─ 📄 20250128_enhanced_achievements_system.sql
│  │  ├─ 📄 20250701_fix_wallet_columns.sql
│  │  ├─ 📄 20250701_uuid_migration.sql
│  │  ├─ 📄 20250703_shoutbox_user_id_to_uuid.sql
│  │  ├─ 📄 add-is-featured-to-forum-structure.sql
│  │  └─ 📄 TODO-2025-07-09-user-fields.sql
│  ├─ 📝 schema/ - Database table schemas
│  │  ├─ 📁 admin/
│  │  │  ├─ 📄 announcements.js
│  │  │  ├─ 📄 announcements.ts
│  │  │  ├─ 📄 auditLogs.js
│  │  │  ├─ 📄 auditLogs.ts
│  │  │  ├─ 📄 backups.js
│  │  │  ├─ 📄 backups.ts
│  │  │  ├─ 📄 brandConfig.js
│  │  │  ├─ 📄 brandConfig.ts
│  │  │  ├─ 📄 emailTemplates.js
│  │  │  ├─ 📄 emailTemplates.ts
│  │  │  ├─ 📄 featureFlags.js
│  │  │  ├─ 📄 featureFlags.ts
│  │  │  ├─ 📄 mediaLibrary.js
│  │  │  ├─ 📄 mediaLibrary.ts
│  │  │  ├─ 📄 moderationActions.js
│  │  │  ├─ 📄 moderationActions.ts
│  │  │  ├─ 📄 moderator-notes.js
│  │  │  ├─ 📄 moderator-notes.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 reports.js
│  │  │  ├─ 📄 reports.ts
│  │  │  ├─ 📄 scheduledTasks.js
│  │  │  ├─ 📄 scheduledTasks.ts
│  │  │  ├─ 📄 seoMetadata.js
│  │  │  ├─ 📄 seoMetadata.ts
│  │  │  ├─ 📄 settings.js
│  │  │  ├─ 📄 settings.ts
│  │  │  ├─ 📄 shoutboxConfig.js
│  │  │  ├─ 📄 shoutboxConfig.ts
│  │  │  ├─ 📄 siteSettings.js
│  │  │  ├─ 📄 siteSettings.ts
│  │  │  ├─ 📄 templates.js
│  │  │  ├─ 📄 templates.ts
│  │  │  ├─ 📄 themes.js
│  │  │  ├─ 📄 themes.ts
│  │  │  ├─ 📄 uiConfig.js
│  │  │  ├─ 📄 uiConfig.ts
│  │  │  ├─ 📄 uiThemes.js
│  │  │  └─ 📄 uiThemes.ts
│  │  ├─ 📁 advertising/
│  │  │  ├─ 📄 campaigns.js
│  │  │  ├─ 📄 campaigns.ts
│  │  │  ├─ 📄 payments.js
│  │  │  ├─ 📄 payments.ts
│  │  │  ├─ 📄 performance.js
│  │  │  ├─ 📄 performance.ts
│  │  │  ├─ 📄 placements.js
│  │  │  ├─ 📄 placements.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 targeting.js
│  │  │  ├─ 📄 targeting.ts
│  │  │  ├─ 📄 user-promotions.js
│  │  │  └─ 📄 user-promotions.ts
│  │  ├─ 📁 auth/
│  │  │  ├─ 📄 sessions.js
│  │  │  └─ 📄 sessions.ts
│  │  ├─ 📁 collectibles/
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 stickers.js
│  │  │  └─ 📄 stickers.ts
│  │  ├─ 📁 core/
│  │  │  ├─ 📁 enums/
│  │  │  │  └─ 📄 index.ts
│  │  │  ├─ 📄 enums.js
│  │  │  └─ 📄 enums.ts
│  │  ├─ 📁 dictionary/
│  │  │  ├─ 📄 entries.js
│  │  │  ├─ 📄 entries.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 upvotes.js
│  │  │  └─ 📄 upvotes.ts
│  │  ├─ 📁 economy/
│  │  │  ├─ 📄 airdropRecords.js
│  │  │  ├─ 📄 airdropRecords.ts
│  │  │  ├─ 📄 airdropSettings.js
│  │  │  ├─ 📄 airdropSettings.ts
│  │  │  ├─ 📄 badges.js
│  │  │  ├─ 📄 badges.ts
│  │  │  ├─ 📄 dgtPackages.js
│  │  │  ├─ 📄 dgtPackages.ts
│  │  │  ├─ 📄 dgtPurchaseOrders.js
│  │  │  ├─ 📄 dgtPurchaseOrders.ts
│  │  │  ├─ 📄 levels.js
│  │  │  ├─ 📄 levels.ts
│  │  │  ├─ 📄 postTips.js
│  │  │  ├─ 📄 postTips.ts
│  │  │  ├─ 📄 rainEvents.js
│  │  │  ├─ 📄 rainEvents.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 reputationAchievements.js
│  │  │  ├─ 📄 reputationAchievements.ts
│  │  │  ├─ 📄 settings.js
│  │  │  ├─ 📄 settings.ts
│  │  │  ├─ 📄 titles.js
│  │  │  ├─ 📄 titles.ts
│  │  │  ├─ 📄 transactions.js
│  │  │  ├─ 📄 transactions.ts
│  │  │  ├─ 📄 treasurySettings.js
│  │  │  ├─ 📄 treasurySettings.ts
│  │  │  ├─ 📄 userBadges.js
│  │  │  ├─ 📄 userBadges.ts
│  │  │  ├─ 📄 userCommands.js
│  │  │  ├─ 📄 userCommands.ts
│  │  │  ├─ 📄 userReputationLog.js
│  │  │  ├─ 📄 userReputationLog.ts
│  │  │  ├─ 📄 userTitles.js
│  │  │  ├─ 📄 userTitles.ts
│  │  │  ├─ 📄 vaults.js
│  │  │  ├─ 📄 vaults.ts
│  │  │  ├─ 📄 wallets.js
│  │  │  ├─ 📄 wallets.ts
│  │  │  ├─ 📄 withdrawalRequests.js
│  │  │  ├─ 📄 withdrawalRequests.ts
│  │  │  ├─ 📄 xpActionSettings.js
│  │  │  ├─ 📄 xpActionSettings.ts
│  │  │  ├─ 📄 xpAdjustmentLogs.js
│  │  │  ├─ 📄 xpAdjustmentLogs.ts
│  │  │  ├─ 📄 xpLogs.js
│  │  │  └─ 📄 xpLogs.ts
│  │  ├─ 📁 forum/
│  │  │  ├─ 📄 customEmojis.js
│  │  │  ├─ 📄 customEmojis.ts
│  │  │  ├─ 📄 emojiPackItems.js
│  │  │  ├─ 📄 emojiPackItems.ts
│  │  │  ├─ 📄 emojiPacks.js
│  │  │  ├─ 📄 emojiPacks.ts
│  │  │  ├─ 📄 pollOptions.js
│  │  │  ├─ 📄 pollOptions.ts
│  │  │  ├─ 📄 polls.js
│  │  │  ├─ 📄 polls.ts
│  │  │  ├─ 📄 pollVotes.js
│  │  │  ├─ 📄 pollVotes.ts
│  │  │  ├─ 📄 postDrafts.js
│  │  │  ├─ 📄 postDrafts.ts
│  │  │  ├─ 📄 postLikes.js
│  │  │  ├─ 📄 postLikes.ts
│  │  │  ├─ 📄 postReactions.js
│  │  │  ├─ 📄 postReactions.ts
│  │  │  ├─ 📄 posts.js
│  │  │  ├─ 📄 posts.ts
│  │  │  ├─ 📄 prefixes.js
│  │  │  ├─ 📄 prefixes.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 rules.js
│  │  │  ├─ 📄 rules.ts
│  │  │  ├─ 📄 structure.js
│  │  │  ├─ 📄 structure.ts
│  │  │  ├─ 📄 tags.js
│  │  │  ├─ 📄 tags.ts
│  │  │  ├─ 📄 threadBookmarks.js
│  │  │  ├─ 📄 threadBookmarks.ts
│  │  │  ├─ 📄 threadDrafts.js
│  │  │  ├─ 📄 threadDrafts.ts
│  │  │  ├─ 📄 threadFeaturePermissions.js
│  │  │  ├─ 📄 threadFeaturePermissions.ts
│  │  │  ├─ 📄 threads.js
│  │  │  ├─ 📄 threads.ts
│  │  │  ├─ 📄 threadTags.js
│  │  │  ├─ 📄 threadTags.ts
│  │  │  ├─ 📄 userEmojiPacks.js
│  │  │  ├─ 📄 userEmojiPacks.ts
│  │  │  ├─ 📄 userRuleAgreements.js
│  │  │  └─ 📄 userRuleAgreements.ts
│  │  ├─ 📁 gamification/
│  │  │  ├─ 📄 achievementEvents.js
│  │  │  ├─ 📄 achievementEvents.ts
│  │  │  ├─ 📄 achievements.js
│  │  │  ├─ 📄 achievements.ts
│  │  │  ├─ 📄 leaderboards.js
│  │  │  ├─ 📄 leaderboards.ts
│  │  │  ├─ 📄 platformStats.js
│  │  │  ├─ 📄 platformStats.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 userAchievements.js
│  │  │  └─ 📄 userAchievements.ts
│  │  ├─ 📁 helpers/
│  │  ├─ 📁 messaging/
│  │  │  ├─ 📄 chatRooms.js
│  │  │  ├─ 📄 chatRooms.ts
│  │  │  ├─ 📄 conversationParticipants.js
│  │  │  ├─ 📄 conversationParticipants.ts
│  │  │  ├─ 📄 conversations.js
│  │  │  ├─ 📄 conversations.ts
│  │  │  ├─ 📄 directMessages.js
│  │  │  ├─ 📄 directMessages.ts
│  │  │  ├─ 📄 messageReads.js
│  │  │  ├─ 📄 messageReads.ts
│  │  │  ├─ 📄 messages.js
│  │  │  ├─ 📄 messages.ts
│  │  │  ├─ 📄 onlineUsers.js
│  │  │  ├─ 📄 onlineUsers.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 shoutboxMessages.js
│  │  │  └─ 📄 shoutboxMessages.ts
│  │  ├─ 📜 migrations/ - Database migration files
│  │  │  └─ 📄 performance-indices.ts
│  │  ├─ 📁 shop/
│  │  │  ├─ 📄 animationPackItems.js
│  │  │  ├─ 📄 animationPackItems.ts
│  │  │  ├─ 📄 animationPacks.js
│  │  │  ├─ 📄 animationPacks.ts
│  │  │  ├─ 📄 cosmeticCategories.js
│  │  │  ├─ 📄 cosmeticCategories.ts
│  │  │  ├─ 📄 inventoryTransactions.js
│  │  │  ├─ 📄 inventoryTransactions.ts
│  │  │  ├─ 📄 orderItems.js
│  │  │  ├─ 📄 orderItems.ts
│  │  │  ├─ 📄 orders.js
│  │  │  ├─ 📄 orders.ts
│  │  │  ├─ 📄 productCategories.js
│  │  │  ├─ 📄 productCategories.ts
│  │  │  ├─ 📄 productMedia.js
│  │  │  ├─ 📄 productMedia.ts
│  │  │  ├─ 📄 products.js
│  │  │  ├─ 📄 products.ts
│  │  │  ├─ 📄 rarities.js
│  │  │  ├─ 📄 rarities.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 signatureItems.js
│  │  │  ├─ 📄 signatureItems.ts
│  │  │  ├─ 📄 userInventory.js
│  │  │  ├─ 📄 userInventory.ts
│  │  │  ├─ 📄 userSignatureItems.js
│  │  │  └─ 📄 userSignatureItems.ts
│  │  ├─ 📁 social/
│  │  │  ├─ 📄 follow-requests.js
│  │  │  ├─ 📄 follow-requests.ts
│  │  │  ├─ 📄 friends.js
│  │  │  ├─ 📄 friends.ts
│  │  │  ├─ 📄 mentions.js
│  │  │  ├─ 📄 mentions.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 user-follow-preferences.js
│  │  │  ├─ 📄 user-follow-preferences.ts
│  │  │  ├─ 📄 user-follows.js
│  │  │  └─ 📄 user-follows.ts
│  │  ├─ 📁 system/
│  │  │  ├─ 📄 activityFeed.js
│  │  │  ├─ 📄 activityFeed.ts
│  │  │  ├─ 📄 airdrop-records.js
│  │  │  ├─ 📄 airdrop-records.ts
│  │  │  ├─ 📄 analyticsEvents.js
│  │  │  ├─ 📄 analyticsEvents.ts
│  │  │  ├─ 📄 cooldownState.js
│  │  │  ├─ 📄 cooldownState.ts
│  │  │  ├─ 📄 economyConfigOverrides.js
│  │  │  ├─ 📄 economyConfigOverrides.ts
│  │  │  ├─ 📄 event_logs.js
│  │  │  ├─ 📄 event_logs.ts
│  │  │  ├─ 📄 mentionsIndex.js
│  │  │  ├─ 📄 mentionsIndex.ts
│  │  │  ├─ 📄 notifications.js
│  │  │  ├─ 📄 notifications.ts
│  │  │  ├─ 📄 profileAnalytics.js
│  │  │  ├─ 📄 profileAnalytics.ts
│  │  │  ├─ 📄 rateLimits.js
│  │  │  ├─ 📄 rateLimits.ts
│  │  │  ├─ 📄 referralSources.js
│  │  │  ├─ 📄 referralSources.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 userAbuseFlags.js
│  │  │  ├─ 📄 userAbuseFlags.ts
│  │  │  ├─ 📄 userReferrals.js
│  │  │  └─ 📄 userReferrals.ts
│  │  ├─ 📁 user/
│  │  │  ├─ 📄 avatarFrames.js
│  │  │  ├─ 📄 avatarFrames.ts
│  │  │  ├─ 📄 bans.js
│  │  │  ├─ 📄 bans.ts
│  │  │  ├─ 📄 featurePermissions.js
│  │  │  ├─ 📄 featurePermissions.ts
│  │  │  ├─ 📄 passwordResetTokens.js
│  │  │  ├─ 📄 passwordResetTokens.ts
│  │  │  ├─ 📄 permissions.js
│  │  │  ├─ 📄 permissions.ts
│  │  │  ├─ 📄 preferences.js
│  │  │  ├─ 📄 preferences.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 relationships.js
│  │  │  ├─ 📄 relationships.ts
│  │  │  ├─ 📄 rolePermissions.js
│  │  │  ├─ 📄 rolePermissions.ts
│  │  │  ├─ 📄 roles.js
│  │  │  ├─ 📄 roles.ts
│  │  │  ├─ 📄 sessions.js
│  │  │  ├─ 📄 sessions.ts
│  │  │  ├─ 📄 settingsHistory.js
│  │  │  ├─ 📄 settingsHistory.ts
│  │  │  ├─ 📄 subscriptions.js
│  │  │  ├─ 📄 subscriptions.ts
│  │  │  ├─ 📄 user-social-preferences.js
│  │  │  ├─ 📄 user-social-preferences.ts
│  │  │  ├─ 📄 userGroups.js
│  │  │  ├─ 📄 userGroups.ts
│  │  │  ├─ 📄 userOwnedFrames.js
│  │  │  ├─ 📄 userOwnedFrames.ts
│  │  │  ├─ 📄 userRoles.js
│  │  │  ├─ 📄 userRoles.ts
│  │  │  ├─ 📄 users.js
│  │  │  ├─ 📄 users.ts
│  │  │  ├─ 📄 verificationTokens.js
│  │  │  ├─ 📄 verificationTokens.ts
│  │  │  └─ 📄 xShares.ts
│  │  ├─ 📁 wallet/
│  │  │  ├─ 📄 ccpayment-users.js
│  │  │  ├─ 📄 ccpayment-users.ts
│  │  │  ├─ 📄 crypto-wallets.js
│  │  │  ├─ 📄 crypto-wallets.ts
│  │  │  ├─ 📄 deposit-records.js
│  │  │  ├─ 📄 deposit-records.ts
│  │  │  ├─ 📄 internal-transfers.js
│  │  │  ├─ 📄 internal-transfers.ts
│  │  │  ├─ 📄 relations.ts
│  │  │  ├─ 📄 supported-tokens.js
│  │  │  ├─ 📄 supported-tokens.ts
│  │  │  ├─ 📄 swap-records.js
│  │  │  ├─ 📄 swap-records.ts
│  │  │  ├─ 📄 webhook-events.js
│  │  │  ├─ 📄 webhook-events.ts
│  │  │  ├─ 📄 withdrawal-records.js
│  │  │  └─ 📄 withdrawal-records.ts
│  │  ├─ 📄 index.js
│  │  └─ 📄 index.ts
│  ├─ 📁 types/
│  │  ├─ 📄 announcement.types.ts
│  │  ├─ 📄 emoji.types.ts
│  │  ├─ 📄 forum.types.ts
│  │  ├─ 📄 index.ts
│  │  ├─ 📄 system.types.ts
│  │  ├─ 📄 user-plugin-data.types.ts
│  │  └─ 📄 utils.ts
│  ├─ 📄 .gitignore
│  ├─ 📄 drizzle.config.ts
│  ├─ 📄 index.js
│  ├─ 📄 index.ts
│  ├─ 📄 package.json
│  ├─ 📄 README.md
│  ├─ 📄 tsconfig.build.json
│  └─ 📄 tsconfig.json
├─ 📚 docs/ - Project documentation
│  ├─ 📁 admin/
│  │  └─ 📄 README.md
│  ├─ 📁 api/
│  │  ├─ 📁 admin/
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 auth/
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 chat/
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 forum/
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 monitoring/
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 security/
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 wallet/
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 webhooks/
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 xp/
│  │  │  └─ 📄 README.md
│  │  ├─ 📄 admin-api.md
│  │  ├─ 📄 auth-api.md
│  │  ├─ 📄 backend-improvements.md
│  │  ├─ 📄 forum-api-complete.md
│  │  ├─ 📄 forum-api.md
│  │  ├─ 📄 gamification-api.md
│  │  ├─ 📄 README_API.md
│  │  ├─ 📄 README.md
│  │  ├─ 📄 wallet-api.md
│  │  └─ 📄 xp-api.md
│  ├─ 📁 architecture/
│  │  ├─ 📄 CCPAYMENT_INTEGRATION.md
│  │  ├─ 📄 FORUM_MANAGEMENT.md
│  │  ├─ 📄 MISSION_SYSTEM_ARCHITECTURE.md
│  │  ├─ 📄 server-architecture.md
│  │  └─ 📄 WALLET_CONSOLIDATION_PLAN.md
│  ├─ 📁 archive/
│  │  ├─ 📄 _CODEBASE_TRANSFORMATION_PLAN.md
│  │  ├─ 📄 _QUICK_REFERENCE.md
│  │  ├─ 📄 AGENT_3_IMPLICIT_ANY_ANALYSIS.md
│  │  ├─ 📄 CLEANUP-SUMMARY.md
│  │  ├─ 📄 directory-tree.md
│  │  ├─ 📄 DOMAIN_CONSOLIDATION_STRATEGY.md
│  │  ├─ 📄 IMPLEMENTATION_SAFETY_CHECKLIST.md
│  │  ├─ 📄 PHASE_5_AUDIT_FIXES.md
│  │  ├─ 📄 PHASE_5_FINAL_VERIFICATION.md
│  │  ├─ 📄 PHASE_5_MAX_DEBT_ERADICATION.md
│  │  ├─ 📄 placeholder.txt
│  │  ├─ 📄 server-migration-analysis.md
│  │  ├─ 📄 TESTING_VALIDATION_STRATEGY.md
│  │  └─ 📄 UUID_MIGRATION_COMPREHENSIVE_REVIEW.md
│  ├─ 📁 assets/
│  │  ├─ 📄 generated-icon.png
│  │  ├─ 📄 IMG_5701.png
│  │  ├─ 📄 IMG_5702.png
│  │  ├─ 📄 IMG_5703.jpeg
│  │  ├─ 📄 IMG_5704.png
│  │  ├─ 📄 IMG_5706.png
│  │  ├─ 📄 IMG_5707.png
│  │  ├─ 📄 IMG_5709.png
│  │  └─ 📄 IMG_5710.png
│  ├─ 📁 audits/
│  │  ├─ 📁 archive/
│  │  │  ├─ 📄 backend-domain-audit.md
│  │  │  ├─ 📄 configuration-infrastructure-audit.md
│  │  │  ├─ 📄 database-schema-audit.md
│  │  │  ├─ 📄 feature-completeness-audit.md
│  │  │  ├─ 📄 frontend-architecture-audit.md
│  │  │  ├─ 📄 MASTER-AUDIT.md
│  │  │  ├─ 📄 platform-audit-1.md
│  │  │  ├─ 📄 platform-audit-2.md
│  │  │  ├─ 📄 platform-audit-3.md
│  │  │  ├─ 📄 platform-audit-4.md
│  │  │  └─ 📄 platform-audit-5.md
│  │  ├─ 📄 AUTH_SECURITY_AUDIT.md
│  │  ├─ 📄 CODEBASE_AUDIT_REPORT.md
│  │  ├─ 📄 CODEBASE_AUDIT.md
│  │  ├─ 📄 CODEBASE-DUPLICATION-ANALYSIS.md
│  │  ├─ 📄 CSS_AUDIT_REPORT.md
│  │  ├─ 📄 degentalk_audit_enhanced.md
│  │  ├─ 📄 FEATURE_AUDIT_AND_EXPANSION_PLAN.md
│  │  ├─ 📄 SCHEMA_AUDIT_REPORT.md
│  │  ├─ 📄 VALIDATION_AUDIT_PLAN.md
│  │  └─ 📄 VALIDATION_REPORT.md
│  ├─ 📁 auth/
│  │  ├─ 📄 CURRENT_AUTH_AUDIT.md
│  │  ├─ 📄 LUCIA_IMPLEMENTATION_PLAN.md
│  │  └─ 📄 LUCIA_TESTING_STRATEGY.md
│  ├─ 📁 forum/
│  │  ├─ 📄 ADMIN_FORUM_REFACTOR_PLAN.md
│  │  ├─ 📄 COMPLETE_FORUM_SYSTEM.md
│  │  ├─ 📄 CRITICAL_FIXES.md
│  │  ├─ 📄 IMPLEMENTATION_PROGRESS.md
│  │  ├─ 📄 MYBB_CLASSIC_IMPLEMENTATION.md
│  │  ├─ 📄 README.md
│  │  ├─ 📄 TERMINOLOGY_MIGRATION.md
│  │  └─ 📄 UX_AUDIT_FINDINGS.md
│  ├─ 📁 guides/
│  │  ├─ 📄 AGENT_WORKSTREAMS.md
│  │  ├─ 📄 CRITICAL_FIXES_NEEDED.md
│  │  ├─ 📄 DEMO_INSTRUCTIONS.md
│  │  ├─ 📄 FORUM_DEV_GUIDE.md
│  │  └─ 📄 SERVER_TODOS.md
│  ├─ 📁 implementation/
│  │  └─ 📄 MISSION_SYSTEM_IMPLEMENTATION.md
│  ├─ 📁 migration/
│  │  ├─ 📄 batch-template.md
│  │  └─ 📄 dependency-migration-plan.json
│  ├─ 📁 planning/
│  │  ├─ 📄 COLOR_SYSTEM_MIGRATION_PLAN.md
│  │  ├─ 📄 DOMAIN-ARCHITECTURE-REFACTOR-PLAN.md
│  │  ├─ 📄 EXECUTION_PLAN.md
│  │  ├─ 📄 FEATURE_PRIORITIZATION_MATRIX.md
│  │  ├─ 📄 FRONTEND_POLISH_CONFIGURABLE_COMPONENTS_PLAN.md
│  │  ├─ 📄 FRONTEND_POLISH_PLAN.md
│  │  └─ 📄 IMPLEMENTATION_ROADMAP.md
│  ├─ 📁 quality-reports/
│  │  ├─ 📁 phase5/
│  │  │  ├─ 📁 2025-07-07/
│  │  │  │  ├─ 📄 bridge-removal.json
│  │  │  │  ├─ 📄 eslint-after-final.json
│  │  │  │  ├─ 📄 eslint-after-fixes.json
│  │  │  │  ├─ 📄 eslint-after-include.json
│  │  │  │  ├─ 📄 eslint-before.json
│  │  │  │  ├─ 📄 eslint-errors-only.txt
│  │  │  │  ├─ 📄 eslint-fix-attempt.txt
│  │  │  │  ├─ 📄 eslint-start.txt
│  │  │  │  ├─ 📄 eslint-summary.txt
│  │  │  │  ├─ 📄 eslint-tail.txt
│  │  │  │  ├─ 📄 eslint-zero.json
│  │  │  │  ├─ 📄 fix-completion-status.md
│  │  │  │  ├─ 📄 id-dry.json
│  │  │  │  ├─ 📄 id-live.json
│  │  │  │  ├─ 📄 manual-fix-summary.md
│  │  │  │  ├─ 📄 metrics.md
│  │  │  │  ├─ 📄 numeric-id-migration-dry-run.txt
│  │  │  │  ├─ 📄 numeric-id-scan.txt
│  │  │  │  ├─ 📄 numeric-id-violations-sample.txt
│  │  │  │  ├─ 📄 production-console-statements.txt
│  │  │  │  ├─ 📄 transformer-dry.json
│  │  │  │  ├─ 📄 transformer-fix-1.txt
│  │  │  │  ├─ 📄 transformer-live.json
│  │  │  │  ├─ 📄 transformer-report.txt
│  │  │  │  ├─ 📄 transformer-violations.txt
│  │  │  │  ├─ 📄 ts-after-imports.txt
│  │  │  │  ├─ 📄 ts-errors-before-codemods.txt
│  │  │  │  ├─ 📄 ts-errors-start.txt
│  │  │  │  └─ 📄 VERIFICATION_REPORT.md
│  │  │  ├─ 📁 2025-07-09/
│  │  │  │  └─ 📄 phase1-completion-summary.md
│  │  │  └─ 📁 manual-fix/
│  │  │     ├─ 📄 eslint-after.txt
│  │  │     └─ 📄 ts-after.txt
│  │  ├─ 📄 quality-report-2025-06-22.json
│  │  ├─ 📄 transformer-violations-sample.txt
│  │  └─ 📄 ts2375_errors.txt
│  ├─ 📁 refactor/
│  │  ├─ 📁 component-consolidation/
│  │  │  ├─ 📄 dupes.csv
│  │  │  └─ 📄 import-stats.json
│  │  └─ 📄 phase1-summary.md
│  ├─ 📁 reports/
│  │  └─ 📄 FORUM_API_INCONSISTENCIES_REPORT.md
│  ├─ 📝 schema/ - Database table schemas
│  │  └─ 📄 UUID_SCHEMA_CONSOLIDATION.md
│  ├─ 📁 shop/
│  │  └─ 📄 README.md
│  ├─ 📁 status/
│  │  ├─ 📄 ADMIN_CLEANUP_FINAL.md
│  │  ├─ 📄 ADMIN_CLEANUP_STATUS.md
│  │  ├─ 📄 ADMIN_CLEANUP_SUMMARY.md
│  │  ├─ 📄 CACHING_IMPLEMENTATION.md
│  │  ├─ 📄 COLOR_MIGRATION_SCRIPTS.md
│  │  ├─ 📄 DESIGN_SYSTEM_RESTORED.md
│  │  ├─ 📄 IMPORT_REFACTORING_SUMMARY.md
│  │  ├─ 📄 IMPORT-REFACTOR-VALIDATION.md
│  │  ├─ 📄 SCHEMA_CLEANUP.md
│  │  ├─ 📄 SHOP_VALIDATION_COMPLETE.md
│  │  ├─ 📄 VALIDATION_IMPLEMENTATION_SUMMARY.md
│  │  └─ 📄 VALIDATION_PATCH_SUMMARY.md
│  ├─ 📁 system/
│  │  └─ 📄 UUID_FIRST_CHEAT_SHEET.md
│  ├─ 📁 testing/
│  │  └─ 📄 E2E_DEPOSIT_FLOW.md
│  ├─ 📄 API-INTEGRATION-GUIDE.md
│  ├─ 📄 ARCHITECTURE_GUIDELINES.md
│  ├─ 📄 CLAUDE_CODE_TYPESCRIPT_HOOKS_SETUP.md
│  ├─ 📄 database-cleanup-analysis.md
│  ├─ 📄 dev-auth-workflow.md
│  ├─ 📄 featured-forum-card-layout.md
│  ├─ 📄 FORUM-MUSTDO.md
│  ├─ 📄 IMG_7385.jpg
│  ├─ 📄 INFRASTRUCTURE_FIXES.md
│  ├─ 📄 LEADERBOARD_MIGRATION.md
│  ├─ 📄 LINT_STAGED_GUIDE.md
│  ├─ 📄 LOGGING_API.md
│  ├─ 📄 LOGGING_EXAMPLES.md
│  ├─ 📄 LOGGING_QUICK_REFERENCE.md
│  ├─ 📄 LOGGING_TROUBLESHOOTING.md
│  ├─ 📄 LOGGING.md
│  ├─ 📄 SECURITY_CHECKLIST.md
│  ├─ 📄 TERMINOLOGY_MIGRATION.md
│  ├─ 📄 TITLE_MIGRATION_SUMMARY.md
│  ├─ 📄 TYPE_UTILITIES.md
│  └─ 📄 USER_MODEL_CONTRACT.md
├─ 🛠️ scripts/ - Development & utility scripts
│  ├─ 📁 admin/
│  │  ├─ 📄 admin-performance-indices.sql
│  │  ├─ 📄 query-performance-audit.ts
│  │  └─ 📄 validate-admin-controllers.ts
│  ├─ 📁 codemods/
│  │  ├─ 📁 .codemod-backups/
│  │  │  └─ 📁 import-migration/
│  │  ├─ 📁 phase5/
│  │  │  ├─ 📄 console-to-logger-README.md
│  │  │  ├─ 📄 console-to-logger.ts
│  │  │  ├─ 📄 consolidate-error-boundaries.ts
│  │  │  ├─ 📄 eliminate-res-json.ts
│  │  │  ├─ 📄 enforce-transformers.ts
│  │  │  ├─ 📄 fix-imports.ts
│  │  │  ├─ 📄 numeric-id-migration.ts
│  │  │  ├─ 📄 README.md
│  │  │  ├─ 📄 req-user-removal.ts
│  │  │  ├─ 📄 run-all.ts
│  │  │  └─ 📄 test-compilation.ts
│  │  ├─ 📄 brand-ids-safe.ts
│  │  ├─ 📄 brand-ids.ts
│  │  ├─ 📄 consolidate-auth-guards.ts
│  │  ├─ 📄 fix-id-validation-patterns.ts
│  │  ├─ 📄 fix-import-aliases.ts
│  │  ├─ 📄 fix-react-router-links.ts
│  │  ├─ 📄 fix-server-userid-types.ts
│  │  ├─ 📄 id-to-entityid.ts
│  │  ├─ 📄 logger-migration.ts
│  │  ├─ 📄 migrate-import-aliases.ts
│  │  ├─ 📄 move-db-id-imports.ts
│  │  └─ 📄 replace-as-id-functions.ts
│  ├─ 🗄️ db/ - Database schema, migrations, and seeds
│  │  ├─ 📁 utils/
│  │  │  ├─ 📄 schema.ts
│  │  │  └─ 📄 seedUtils.ts
│  │  ├─ 📄 add_categoryid_to_thread_prefixes.ts
│  │  ├─ 📄 add-color-theme-field.ts
│  │  ├─ 📄 add-critical-indices.ts
│  │  ├─ 📄 apply-migration.ts
│  │  ├─ 📄 backfill-configZoneType.ts
│  │  ├─ 📄 check-indices.ts
│  │  ├─ 📄 check-levels-table.ts
│  │  ├─ 📄 check-reward-tables.ts
│  │  ├─ 📄 create-missing-tables.ts
│  │  ├─ 📄 db-helper.js
│  │  ├─ 📄 db-schema-summary.js
│  │  ├─ 📄 diagnose-mentions-table.ts
│  │  ├─ 📄 diff-schema-snapshot.ts
│  │  ├─ 📄 fix-forum-relationships.ts
│  │  ├─ 📄 generate-performance-migration.ts
│  │  ├─ 📄 generate-relations.ts
│  │  ├─ 📄 initialize-giphy-settings.ts
│  │  ├─ 📄 initialize-xp-system.ts
│  │  ├─ 📄 read-thread.ts
│  │  ├─ 📄 remove-zone-type.ts
│  │  ├─ 📄 reset-and-seed.ts
│  │  ├─ 📄 run-migrations.ts
│  │  ├─ 📄 seed-achievements.ts
│  │  ├─ 📄 seed-badges.ts
│  │  ├─ 📄 seed-chat.ts
│  │  ├─ 📄 seed-default-levels.ts
│  │  ├─ 📄 seed-dev-subscriptions.ts
│  │  ├─ 📄 seed-dev-wallet.ts
│  │  ├─ 📄 seed-economy-settings.ts
│  │  ├─ 📄 seed-promotion-pricing.ts
│  │  ├─ 📄 seed-shop.ts
│  │  ├─ 📄 seed-threads.ts
│  │  ├─ 📄 seed-treasury.ts
│  │  ├─ 📄 seed-ui-config-quotes.ts
│  │  ├─ 📄 seed-user-role-migration.ts
│  │  ├─ 📄 seed-users-with-tokens.ts
│  │  ├─ 📄 seed-users.ts
│  │  ├─ 📄 seed-vaults.ts
│  │  ├─ 📄 seed-xp-actions.ts
│  │  ├─ 📄 sync-forum-config.ts
│  │  ├─ 📄 test-forum-query.ts
│  │  ├─ 📄 test-forum-runtime.ts
│  │  ├─ 📄 update-forum-slugs.ts
│  │  ├─ 📄 update-users-table.ts
│  │  ├─ 📄 verify-neon-connection.ts
│  │  ├─ 📄 zones-to-featured-forums-migration.ts
│  │  └─ 📄 zones-to-forums-migration.ts
│  ├─ 📁 dev/
│  │  ├─ 📄 cursor-sync.sh
│  │  ├─ 📄 generate-scope-guide.ts
│  │  ├─ 📄 setup-fresh-wallet-test.ts
│  │  ├─ 📄 syncForumsToDB.ts
│  │  └─ 📄 test-deposit-flow.ts
│  ├─ 📚 docs/ - Project documentation
│  │  └─ 📄 add-frontmatter.cjs
│  ├─ 📁 migration/
│  │  ├─ 📁 domain-migrations/
│  │  │  ├─ 📄 client-api-migration.ts
│  │  │  ├─ 📄 client-components-migration.ts
│  │  │  ├─ 📄 client-hooks-migration.ts
│  │  │  ├─ 📄 client-types-migration.ts
│  │  │  ├─ 📄 forum-core-migration.ts
│  │  │  └─ 📄 server-storage-migration.ts
│  │  ├─ 📁 notes/
│  │  │  ├─ 📄 current-structure-analysis.md
│  │  │  ├─ 📄 manual-preparation-tasks.md
│  │  │  ├─ 📄 revised-task-assignments.md
│  │  │  ├─ 📄 server-storage-mapping.md
│  │  │  ├─ 📄 subagent-analysis-tasks.md
│  │  │  └─ 📄 subagent-task-assignments.md
│  │  ├─ 📁 output/
│  │  │  ├─ 📄 codebase-analysis.json
│  │  │  ├─ 📄 components-manual-review.md
│  │  │  ├─ 📄 domain-activity.json
│  │  │  ├─ 📄 domain-admin.json
│  │  │  ├─ 📄 domain-advertising.json
│  │  │  ├─ 📄 domain-auth.json
│  │  │  ├─ 📄 domain-ccpayment-webhook.json
│  │  │  ├─ 📄 domain-collectibles.json
│  │  │  ├─ 📄 domain-cosmetics.json
│  │  │  ├─ 📄 domain-dictionary.json
│  │  │  ├─ 📄 domain-economy.json
│  │  │  ├─ 📄 domain-editor.json
│  │  │  ├─ 📄 domain-engagement.json
│  │  │  ├─ 📄 domain-feature-gates.json
│  │  │  ├─ 📄 domain-forum-core.json
│  │  │  ├─ 📄 domain-forum.json
│  │  │  ├─ 📄 domain-gamification.json
│  │  │  ├─ 📄 domain-infrastructure.json
│  │  │  ├─ 📄 domain-messaging.json
│  │  │  ├─ 📄 domain-missions.json
│  │  │  ├─ 📄 domain-moderation.json
│  │  │  ├─ 📄 domain-notifications.json
│  │  │  ├─ 📄 domain-other.json
│  │  │  ├─ 📄 domain-paths.json
│  │  │  ├─ 📄 domain-preferences.json
│  │  │  ├─ 📄 domain-profile.json
│  │  │  ├─ 📄 domain-share.json
│  │  │  ├─ 📄 domain-shop.json
│  │  │  ├─ 📄 domain-shoutbox.json
│  │  │  ├─ 📄 domain-social.json
│  │  │  ├─ 📄 domain-subscriptions.json
│  │  │  ├─ 📄 domain-treasury.json
│  │  │  ├─ 📄 domain-uploads.json
│  │  │  ├─ 📄 domain-user-management.json
│  │  │  ├─ 📄 domain-user.json
│  │  │  ├─ 📄 domain-wallet.json
│  │  │  ├─ 📄 domain-xp.json
│  │  │  ├─ 📄 forum-core-migration-dryrun-1751412697617.json
│  │  │  ├─ 📄 forum-core-migration-dryrun-1751413669981.json
│  │  │  ├─ 📄 forum-core-migration-dryrun-1751413778821.json
│  │  │  ├─ 📄 forum-core-migration-dryrun-1751413797844.json
│  │  │  ├─ 📄 forum-core-migration-live-1751413899507.json
│  │  │  ├─ 📄 hooks-manual-review.md
│  │  │  ├─ 📄 manual-preparation-analysis.md
│  │  │  ├─ 📄 migration-roadmap.md
│  │  │  ├─ 📄 migration-summary.md
│  │  │  ├─ 📄 numeric-id-report.json
│  │  │  └─ 📄 server-storage-dryrun.json
│  │  ├─ 🛠️ scripts/ - Development & utility scripts
│  │  │  └─ 📁 migration/
│  │  │     └─ 📁 output/
│  │  │        ... (max depth reached)
│  │  ├─ 📄 add-missing-references.ts
│  │  ├─ 📄 add-performance-indices.ts
│  │  ├─ 📄 ANALYSIS_VS_DETECTION.md
│  │  ├─ 📄 analyze-codebase.ts
│  │  ├─ 📄 check-ids-ci.ts
│  │  ├─ 📄 check-naming-conventions.ts
│  │  ├─ 📄 cleanup-old-category-schema.ts
│  │  ├─ 📄 fix-admin-api-calls.ts
│  │  ├─ 📄 fix-admin-implicit-any.ts
│  │  ├─ 📄 fix-admin-missing-modules.ts
│  │  ├─ 📄 fix-admin-optional-types.ts
│  │  ├─ 📄 fix-broken-validators.ts
│  │  ├─ 📄 fix-frontend-id-types.ts
│  │  ├─ 📄 fix-integer-id-patterns.ts
│  │  ├─ 📄 fix-seed-files.ts
│  │  ├─ 📄 fix-shoutbox-message-id.ts
│  │  ├─ 📄 fix-test-files.ts
│  │  ├─ 📄 generate-id-aliases.ts
│  │  ├─ 📄 generate-uuid-audit.ts
│  │  ├─ 📄 generated-id-aliases.ts
│  │  ├─ 📄 id-migration-summary.md
│  │  ├─ 📄 identify-numeric-ids.ts
│  │  ├─ 📄 migrate-forum-structure.ts
│  │  ├─ 📄 REVIEWER_GUIDE.md
│  │  ├─ 📄 scan-non-uuid-columns.ts
│  │  ├─ 📄 sync-branded-types.ts
│  │  ├─ 📄 test-update-baseline.ts
│  │  ├─ 📄 update-baseline.ts
│  │  ├─ 📄 uuid-audit.json
│  │  └─ 📄 uuid-first-completion-summary.md
│  ├─ 📜 migrations/ - Database migration files
│  │  ├─ 📄 consolidate-titles.ts
│  │  └─ 📄 mark-featured-forums.ts
│  ├─ 📁 ops/
│  │  ├─ 📄 check-forum-config-sync.ts
│  │  ├─ 📄 neon-sync-agent.ts
│  │  ├─ 📄 setup-xp-system.sh
│  │  └─ 📄 validate-safe-migrations.ts
│  ├─ 📁 phase5/
│  │  ├─ 📄 rollback.ts
│  │  └─ 📄 validate-phase5.ts
│  ├─ 📁 quality/
│  │  └─ 📄 quality-metrics.ts
│  ├─ 📁 seed/
│  │  ├─ 🔧 config/ - Shared build/tooling configuration
│  │  │  ├─ 📄 personas.config.ts
│  │  │  └─ 📄 seed.config.ts
│  │  ├─ 📁 engines/
│  │  │  ├─ 📄 abuse-simulator.ts
│  │  │  ├─ 📄 admin-flow-simulator.ts
│  │  │  ├─ 📄 cosmetics-effects.ts
│  │  │  ├─ 📄 gamification-simulator.ts
│  │  │  └─ 📄 temporal-simulator.ts
│  │  ├─ 📁 generators/
│  │  │  ├─ 📄 content-generator.ts
│  │  │  ├─ 📄 economy-simulator.ts
│  │  │  └─ 📄 user-generator.ts
│  │  ├─ 📁 orchestrator/
│  │  │  └─ 📄 seed-orchestrator.ts
│  │  ├─ 📁 shop/
│  │  │  ├─ 📄 seed-default-cosmetics.ts
│  │  │  └─ 📄 username-colors.ts
│  │  ├─ 📁 sync/
│  │  │  ├─ 📄 config-enforcer.ts
│  │  │  └─ 📄 config-stubs.ts
│  │  ├─ 📄 README.md
│  │  ├─ 📄 run-enhanced-seed.ts
│  │  └─ 📄 seedForumsFromConfig.ts
│  ├─ 📁 templates/
│  │  ├─ 📄 transaction-domain-template.ts
│  │  └─ 📄 vault-domain-template.ts
│  ├─ 📁 test-fixtures/
│  │  ├─ 📁 core/
│  │  │  └─ 📄 factory.ts
│  │  ├─ 📁 factories/
│  │  │  ├─ 📄 forum.factory.ts
│  │  │  └─ 📄 user.factory.ts
│  │  ├─ 📁 utilities/
│  │  │  ├─ 📄 scenario-generator.ts
│  │  │  └─ 📄 test-helpers.ts
│  │  └─ 📄 index.ts
│  ├─ 📁 testing/
│  │  ├─ 📄 admin-api-tests.sh
│  │  ├─ 📄 test-forum-endpoints.js
│  │  ├─ 📄 test-mock-requests.js
│  │  ├─ 📄 test-repository-implementation.ts
│  │  ├─ 📄 test-xp-actions.js
│  │  ├─ 📄 test-xp-endpoints.js
│  │  ├─ 📄 test-xp-system.ts
│  │  ├─ 📄 validate-domain-migration.sh
│  │  ├─ 📄 validate-forum-fks.ts
│  │  └─ 📄 validate-routes.js
│  ├─ 📁 tools/
│  │  ├─ 📄 generate-tree.js
│  │  └─ 📄 icon-scan.ts
│  ├─ 📁 validation/
│  │  ├─ 📄 no-orphan-bak.ts
│  │  ├─ 📄 unique-pg-tables.ts
│  │  ├─ 📄 validate-boundaries.cjs
│  │  ├─ 📄 validate-dependencies.cjs
│  │  ├─ 📄 validate-forum-structure-migration.ts
│  │  └─ 📄 validate-uuid-migration.ts
│  ├─ 📄 analyze-foreign-keys.js
│  ├─ 📄 analyze-ts-errors.ts
│  ├─ 📄 apply-mission-migrations.cjs
│  ├─ 📄 apply-performance-indexes.ts
│  ├─ 📄 audit-imports.ts
│  ├─ 📄 backfill-thread-parentForumSlug.ts
│  ├─ 📄 build-forum-sdk.ts
│  ├─ 📄 cache-decorator-test.ts
│  ├─ 📄 cache-smoke.ts
│  ├─ 📄 check-debug-artifacts.sh
│  ├─ 📄 check-forum-distribution.cjs
│  ├─ 📄 check-forum-parents.ts
│  ├─ 📄 check-forum-structure.ts
│  ├─ 📄 check-forums.ts
│  ├─ 📄 check-imports.ts
│  ├─ 📄 check-orphaned-threads.cjs
│  ├─ 📄 check-test-users.ts
│  ├─ 📄 check-thread-slugs.cjs
│  ├─ 📄 check-threads.ts
│  ├─ 📄 check-types.ts
│  ├─ 📄 cleanup-admin-subdomain.sh
│  ├─ 📄 cleanup-legacy-files.sh
│  ├─ 📄 cleanup-quotfix-files.sh
│  ├─ 📄 cleanup-remaining-imports.cjs
│  ├─ 📄 cleanup-types.sh
│  ├─ 📄 clear-rate-limits.ts
│  ├─ 📄 commit-no-hooks.sh
│  ├─ 📄 complete-theme-migration.sh
│  ├─ 📄 convert-admin-pages.cjs
│  ├─ 📄 convert-client.cjs
│  ├─ 📄 convert-server.cjs
│  ├─ 📄 create-foreign-key-indexes-verified.sql
│  ├─ 📄 create-foreign-key-indexes.sql
│  ├─ 📄 create-test-data.sql
│  ├─ 📄 create-test-user.cjs
│  ├─ 📄 create-test-users-simple.ts
│  ├─ 📄 debug-context.cjs
│  ├─ 📄 debug-threads.ts
│  ├─ 📄 dev-login.ts
│  ├─ 📄 dev-logout.ts
│  ├─ 📄 docker-dev.sh
│  ├─ 📄 ensure-default-frames.ts
│  ├─ 📄 find-foreign-keys.js
│  ├─ 📄 fix-all-imports.cjs
│  ├─ 📄 fix-all-schema-issues.ts
│  ├─ 📄 fix-all-user-refs.ts
│  ├─ 📄 fix-common-type-errors.ts
│  ├─ 📄 fix-conversion-errors.cjs
│  ├─ 📄 fix-db-imports.cjs
│  ├─ 📄 fix-db-imports.sh
│  ├─ 📄 fix-db-imports.ts
│  ├─ 📄 fix-db-migrations.ts
│  ├─ 📄 fix-forums.cjs
│  ├─ 📄 fix-frontend-imports.sh
│  ├─ 📄 fix-integer-imports.ts
│  ├─ 📄 fix-missing-uuid-imports.ts
│  ├─ 📄 fix-mission-imports.cjs
│  ├─ 📄 fix-orphaned-threads.cjs
│  ├─ 📄 fix-package-manager.sh
│  ├─ 📄 fix-script-imports.sh
│  ├─ 📄 fix-shared-imports.sh
│  ├─ 📄 fix-table-references.ts
│  ├─ 📄 fix-transformer-violations.ts
│  ├─ 📄 fix-unterminated-strings.cjs
│  ├─ 📄 fix-userid-types.ts
│  ├─ 📄 fix-widget-loading.ts
│  ├─ 📄 manual-fix-critical.cjs
│  ├─ 📄 mark-repository-todos.ts
│  ├─ 📄 migrate-to-pnpm.js
│  ├─ 📄 nuke-user-types.sh
│  ├─ 📄 package.json
│  ├─ 📄 README.md
│  ├─ 📄 refactor-zone-to-category.sh
│  ├─ 📄 refactor-zones-to-featured-forums.sh
│  ├─ 📄 remove-bloat-conservative.sh
│  ├─ 📄 remove-bloat.sh
│  ├─ 📄 remove-mission-system.sh
│  ├─ 📄 remove-old-aliases.cjs
│  ├─ 📄 remove-zones-completely.sh
│  ├─ 📄 rename-reputation-to-reputation.sh
│  ├─ 📄 reset-database.sql
│  ├─ 📄 reset-db-clean.ts
│  ├─ 📄 run-migrations.ts
│  ├─ 📄 run-schema-migration.ts
│  ├─ 📄 run-with-paths.sh
│  ├─ 📄 SCRIPT_GUIDE.md
│  ├─ 📄 seed-admin-user.ts
│  ├─ 📄 seed-demo-forum.ts
│  ├─ 📄 seed-missions-direct.cjs
│  ├─ 📄 seed.ts
│  ├─ 📄 sync-forum-hierarchy.ts
│  ├─ 📄 sync-forums-now.ts
│  ├─ 📄 test-admin-xp.js
│  ├─ 📄 test-api-response.cjs
│  ├─ 📄 test-ci-readiness.ts
│  ├─ 📄 test-forum-api.ts
│  ├─ 📄 test-forum-hierarchy.cjs
│  ├─ 📄 test-frontend-data.cjs
│  ├─ 📄 test-hybrid-auth.ts
│  ├─ 📄 test-jwt-auth.ts
│  ├─ 📄 test-mission-system.sh
│  ├─ 📄 test-performance.cjs
│  ├─ 📄 test-thread-api.ts
│  ├─ 📄 test-user-flow.ts
│  ├─ 📄 tsconfig.json
│  ├─ 📄 unfuck-baseline.sh
│  ├─ 📄 unfuck-build.sh
│  ├─ 📄 unfuck-lint-autofix.sh
│  ├─ 📄 unfuck-status.sh
│  ├─ 📄 validate-everything.ts
│  ├─ 📄 validate-imports.ts
│  ├─ 📄 validate-no-zones.sh
│  ├─ 📄 validate-theme-migration.ts
│  └─ 📄 verify-imports.cjs
├─ ⚙️  server/ - Backend Server (Express)
│  ├─ 📁 .cache/
│  │  └─ 📄 dev-tokens.json
│  ├─ 📁 .claude/
│  │  ├─ 📁 agents/
│  │  └─ 📁 audit/
│  │     ├─ 📄 2025-07-13.jsonl
│  │     ├─ 📄 2025-07-14.jsonl
│  │     ├─ 📄 2025-07-15.jsonl
│  │     ├─ 📄 2025-07-16.jsonl
│  │     ├─ 📄 2025-07-22.jsonl
│  │     ├─ 📄 2025-07-23.jsonl
│  │     ├─ 📄 2025-07-24.jsonl
│  │     ├─ 📄 2025-07-25.jsonl
│  │     ├─ 📄 2025-07-26.jsonl
│  │     ├─ 📄 2025-07-27.jsonl
│  │     ├─ 📄 2025-07-28.jsonl
│  │     └─ 📄 2025-07-29.jsonl
│  ├─ 🔧 config/ - Shared build/tooling configuration
│  │  └─ 📄 loadEnv.ts
│  ├─ 📁 lib/
│  │  ├─ 📄 report-error.js
│  │  └─ 📄 report-error.ts
│  ├─ 📁 logs/
│  ├─ 📜 migrations/ - Database migration files
│  │  ├─ 📁 archive/
│  │  │  ├─ 📄 README.md
│  │  │  └─ 📄 run-tip-rain.ts
│  │  ├─ 📄 20250510_create_xp_adjustment_logs.ts
│  │  ├─ 📄 20250512_create_xp_action_logs.ts
│  │  ├─ 📄 20250513_create_xp_action_settings.ts
│  │  ├─ 📄 20250618_add_reputation_achievements.ts
│  │  ├─ 📄 20250618_add_rollout_percentage_to_feature_flags.ts
│  │  ├─ 📄 20250618_add_updated_by_to_site_settings.ts
│  │  ├─ 📄 20250618_create_xp_reputation_settings.ts
│  │  ├─ 📄 20250624_add_visual_fields_to_levels.ts
│  │  ├─ 📄 20250626_extend_ui_themes.ts
│  │  ├─ 📄 add-daily-xp-tracking.ts
│  │  ├─ 📄 add-dgt-packages-table.ts
│  │  ├─ 📄 add-dgt-purchase-orders-table.ts
│  │  └─ 📄 xp-reputation-levels-migration.ts
│  ├─ 📁 quality-reports/
│  │  └─ 📁 phase5/
│  │     └─ 📁 manual-fix/
│  │        ├─ 📄 eslint-after.txt
│  │        └─ 📄 ts-after.txt
│  ├─ 📁 services/
│  │  └─ 📄 xp-level-service.ts
│  ├─ 🏗️ src/ - Core application source code
│  │  ├─ 📁 __tests__/
│  │  │  └─ 📁 database/
│  │  │     └─ 📄 foreign-key-indexes.test.ts
│  │  ├─ 📁 core/
│  │  │  ├─ 📁 audit/
│  │  │  │  └─ 📄 audit-logger.ts
│  │  │  ├─ 📁 cache/
│  │  │  │  ├─ 📄 cache.config.js
│  │  │  │  ├─ 📄 cache.config.ts
│  │  │  │  ├─ 📄 decorators.js
│  │  │  │  ├─ 📄 decorators.ts
│  │  │  │  ├─ 📄 getCacheKey.js
│  │  │  │  ├─ 📄 getCacheKey.ts
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 invalidateCache.ts
│  │  │  │  ├─ 📄 MIGRATION_GUIDE.md
│  │  │  │  ├─ 📄 redis.service.js
│  │  │  │  ├─ 📄 redis.service.ts
│  │  │  │  ├─ 📄 stampede-protection.js
│  │  │  │  ├─ 📄 stampede-protection.ts
│  │  │  │  ├─ 📄 unified-cache.service.js
│  │  │  │  └─ 📄 unified-cache.service.ts
│  │  │  ├─ 🔧 config/ - Shared build/tooling configuration
│  │  │  │  └─ 📄 environment.ts
│  │  │  ├─ 📁 events/
│  │  │  │  ├─ 📄 achievement-events.service.js
│  │  │  │  ├─ 📄 achievement-events.service.ts
│  │  │  │  ├─ 📄 event-bus.ts
│  │  │  │  └─ 📄 types.ts
│  │  │  ├─ 📁 helpers/
│  │  │  │  ├─ 📄 safe-id-converter.ts
│  │  │  │  └─ 📄 validate-controller-ids.ts
│  │  │  ├─ 📁 middleware/
│  │  │  │  └─ 📄 security.middleware.ts
│  │  │  ├─ 📁 monitoring/
│  │  │  │  ├─ 📄 health-check.ts
│  │  │  │  └─ 📄 query-performance.ts
│  │  │  ├─ 📁 repository/
│  │  │  │  ├─ 📁 __tests__/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 repositories/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 ANALYSIS.md
│  │  │  │  ├─ 📄 base-repository.js
│  │  │  │  ├─ 📄 base-repository.ts
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 interfaces.js
│  │  │  │  ├─ 📄 interfaces.ts
│  │  │  │  ├─ 📄 repository-factory.js
│  │  │  │  └─ 📄 repository-factory.ts
│  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  └─ 📁 api/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 security/
│  │  │  │  └─ 📄 security-monitor.service.ts
│  │  │  ├─ 📁 services/
│  │  │  │  ├─ 📄 rate-limit.service.ts
│  │  │  │  ├─ 📄 settings.service.ts
│  │  │  │  └─ 📄 user.service.ts
│  │  │  ├─ 📁 utils/
│  │  │  │  ├─ 📄 auth.helpers.js
│  │  │  │  ├─ 📄 auth.helpers.ts
│  │  │  │  ├─ 📄 error-response.util.ts
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  └─ 📄 transformer.helpers.ts
│  │  │  ├─ 📁 validation/
│  │  │  │  ├─ 📄 id-validators.ts
│  │  │  │  └─ 📄 permission-validators.ts
│  │  │  ├─ 📁 websocket/
│  │  │  │  └─ 📄 websocket.service.ts
│  │  │  ├─ 📄 background-processor.js
│  │  │  ├─ 📄 background-processor.ts
│  │  │  ├─ 📄 base-controller.ts
│  │  │  ├─ 📄 cache.service.ts
│  │  │  ├─ 📄 config.service.ts
│  │  │  ├─ 📄 database.ts
│  │  │  ├─ 📄 db.js
│  │  │  ├─ 📄 db.ts
│  │  │  ├─ 📄 errors.js
│  │  │  ├─ 📄 errors.ts
│  │  │  ├─ 📄 logger-config.js
│  │  │  ├─ 📄 logger-config.ts
│  │  │  ├─ 📄 logger-control.js
│  │  │  ├─ 📄 logger-control.ts
│  │  │  ├─ 📄 logger-types.js
│  │  │  ├─ 📄 logger-types.ts
│  │  │  ├─ 📄 logger.js
│  │  │  ├─ 📄 logger.ts
│  │  │  ├─ 📄 middleware.ts
│  │  │  ├─ 📄 rate-limiter.ts
│  │  │  ├─ 📄 storage.service.ts
│  │  │  └─ 📄 wallet-validators.ts
│  │  ├─ 📁 cron/
│  │  │  └─ 📄 subscription-management.ts
│  │  ├─ 🏗️ domains/ - Domain-driven business logic
│  │  │  ├─ 📁 activity/
│  │  │  │  ├─ 📁 controllers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 services/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 admin/
│  │  │  │  ├─ 📁 announcements/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 content/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 controllers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 economy/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 gamification/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🤝 shared/ - Isomorphic code (client & server)
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 sub-domains/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 system/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 users/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 admin.controller.ts
│  │  │  │  ├─ 📄 admin.errors.ts
│  │  │  │  ├─ 📄 admin.middleware.ts
│  │  │  │  ├─ 📄 admin.routes.ts
│  │  │  │  ├─ 📄 admin.service.ts
│  │  │  │  ├─ 📄 admin.validation.ts
│  │  │  │  ├─ 📄 adminRegistry.ts
│  │  │  │  └─ 📄 index.ts
│  │  │  ├─ 📁 advertising/
│  │  │  │  ├─ 📄 ad-admin.controller.ts
│  │  │  │  ├─ 📄 ad-configuration.service.ts
│  │  │  │  ├─ 📄 ad-serving.service.ts
│  │  │  │  ├─ 📄 ad.controller.ts
│  │  │  │  ├─ 📄 ad.routes.ts
│  │  │  │  ├─ 📄 campaign-management.service.ts
│  │  │  │  ├─ 📄 user-promotion.routes.ts
│  │  │  │  └─ 📄 user-promotion.service.ts
│  │  │  ├─ 📁 analytics/
│  │  │  │  ├─ 📁 admin/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 controllers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 middleware/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 repositories/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📄 index.ts
│  │  │  ├─ 📁 auth/
│  │  │  │  ├─ 📁 controllers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 middleware/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 utils/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 validation/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 auth.routes.ts
│  │  │  │  └─ 📄 index.ts
│  │  │  ├─ 📁 collectibles/
│  │  │  │  ├─ 📁 stickers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 transformers/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 core/
│  │  │  │  └─ 📄 logger.ts
│  │  │  ├─ 📁 cosmetics/
│  │  │  │  ├─ 📄 avatarFrameStore.service.ts
│  │  │  │  └─ 📄 frameEquip.service.ts
│  │  │  ├─ 📁 dictionary/
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 dictionary.routes.ts
│  │  │  │  └─ 📄 dictionary.service.ts
│  │  │  ├─ 📁 economy/
│  │  │  │  └─ 📁 admin/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 editor/
│  │  │  │  └─ 📄 editor.routes.ts
│  │  │  ├─ 📁 engagement/
│  │  │  │  ├─ 📁 airdrop/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 rain/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 tip/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 vault/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📄 engagement.service.ts
│  │  │  ├─ 📁 example/
│  │  │  │  └─ 📄 example-updated.controller.ts
│  │  │  ├─ 📁 feature-gates/
│  │  │  │  ├─ 📄 feature-gates.controller.ts
│  │  │  │  ├─ 📄 feature-gates.routes.ts
│  │  │  │  └─ 📄 feature-gates.service.ts
│  │  │  ├─ 📁 forum/
│  │  │  │  ├─ 📁 admin/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 controllers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 features/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 rules/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 sub-domains/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 types/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 validation/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 forum.controller.ts
│  │  │  │  ├─ 📄 forum.routes.ts
│  │  │  │  ├─ 📄 forum.service.js
│  │  │  │  ├─ 📄 forum.service.test.ts
│  │  │  │  └─ 📄 forum.service.ts
│  │  │  ├─ 📁 gamification/
│  │  │  │  ├─ 📁 achievements/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 admin/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 achievement.controller.ts
│  │  │  │  ├─ 📄 achievement.routes.ts
│  │  │  │  ├─ 📄 admin.controller.ts
│  │  │  │  ├─ 📄 admin.routes.ts
│  │  │  │  ├─ 📄 analytics.controller.ts
│  │  │  │  ├─ 📄 analytics.routes.ts
│  │  │  │  ├─ 📄 gamification.routes.ts
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 leveling.controller.ts
│  │  │  │  ├─ 📄 leveling.routes.ts
│  │  │  │  ├─ 📄 reputation.controller.ts
│  │  │  │  ├─ 📄 reputation.routes.ts
│  │  │  │  ├─ 📄 titles.controller.ts
│  │  │  │  ├─ 📄 titles.routes.ts
│  │  │  │  ├─ 📄 titles.service.js
│  │  │  │  ├─ 📄 titles.service.ts
│  │  │  │  ├─ 📄 titles.validators.js
│  │  │  │  └─ 📄 titles.validators.ts
│  │  │  ├─ 📁 messaging/
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 types/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 message.routes.ts
│  │  │  │  ├─ 📄 message.service.ts
│  │  │  │  └─ 📄 README.md
│  │  │  ├─ 📁 notifications/
│  │  │  │  ├─ 📁 admin/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 event-notification-listener.js
│  │  │  │  ├─ 📄 event-notification-listener.ts
│  │  │  │  ├─ 📄 notification-generator.service.js
│  │  │  │  ├─ 📄 notification-generator.service.ts
│  │  │  │  ├─ 📄 notification.routes.ts
│  │  │  │  ├─ 📄 notification.service.js
│  │  │  │  └─ 📄 notification.service.ts
│  │  │  ├─ 📁 preferences/
│  │  │  │  ├─ 📁 validation/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 preferences.routes.ts
│  │  │  │  ├─ 📄 preferences.service.ts
│  │  │  │  └─ 📄 preferences.validators.ts
│  │  │  ├─ 📁 profile/
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 profile-stats.controller.ts
│  │  │  │  ├─ 📄 profile-stats.routes.ts
│  │  │  │  ├─ 📄 profile-stats.service.ts
│  │  │  │  ├─ 📄 profile.routes.ts
│  │  │  │  ├─ 📄 profile.service.ts
│  │  │  │  ├─ 📄 referrals.service.ts
│  │  │  │  ├─ 📄 schema-updates.sql
│  │  │  │  ├─ 📄 signature.routes.ts
│  │  │  │  ├─ 📄 signature.service.ts
│  │  │  │  ├─ 📄 social-actions.controller.ts
│  │  │  │  ├─ 📄 social-actions.routes.ts
│  │  │  │  └─ 📄 social-actions.service.ts
│  │  │  ├─ 📁 share/
│  │  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 transformers/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 shop/
│  │  │  │  ├─ 📁 admin/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 cosmetics/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 types/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 validation/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 cosmetics.routes.ts
│  │  │  │  └─ 📄 shop.routes.ts
│  │  │  ├─ 📁 shoutbox/
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📄 shoutbox.routes.ts
│  │  │  ├─ 📁 social/
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 follows.routes.ts
│  │  │  │  ├─ 📄 follows.service.ts
│  │  │  │  ├─ 📄 follows.types.ts
│  │  │  │  ├─ 📄 friends.routes.ts
│  │  │  │  ├─ 📄 friends.service.ts
│  │  │  │  ├─ 📄 mentions.routes.ts
│  │  │  │  ├─ 📄 mentions.service.js
│  │  │  │  ├─ 📄 mentions.service.ts
│  │  │  │  ├─ 📄 mentions.types.js
│  │  │  │  ├─ 📄 mentions.types.ts
│  │  │  │  ├─ 📄 relationships.routes.ts
│  │  │  │  ├─ 📄 social.routes.ts
│  │  │  │  ├─ 📄 whale-watch.routes.ts
│  │  │  │  └─ 📄 whale-watch.service.ts
│  │  │  ├─ 📁 subscriptions/
│  │  │  │  ├─ 📁 admin/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 subscription-permissions.service.ts
│  │  │  │  ├─ 📄 subscription.controller.ts
│  │  │  │  ├─ 📄 subscription.routes.ts
│  │  │  │  ├─ 📄 subscription.service.ts
│  │  │  │  └─ 📄 subscription.validators.ts
│  │  │  ├─ 📁 themes/
│  │  │  │  ├─ 📜 migrations/ - Database migration files
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 services/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 treasury/
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📄 treasury.routes.ts
│  │  │  ├─ 📁 uploads/
│  │  │  │  ├─ 📄 upload.controller.ts
│  │  │  │  ├─ 📄 upload.routes.ts
│  │  │  │  ├─ 📄 upload.service.ts
│  │  │  │  └─ 📄 upload.transformer.ts
│  │  │  ├─ 📁 user/
│  │  │  │  ├─ 📄 user-preferences.routes.ts
│  │  │  │  └─ 📄 user-preferences.service.ts
│  │  │  ├─ 📁 users/
│  │  │  │  ├─ 📁 middleware/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 social/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  └─ 📁 types/
│  │  │  │     ... (max depth reached)
│  │  │  ├─ 📁 wallet/
│  │  │  │  ├─ 📁 __tests__/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 adapters/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 admin/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 builders/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 controllers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 middleware/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 providers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 transformers/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 types/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 utils/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 validation/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 webhooks/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  └─ 📄 README.md
│  │  │  ├─ 📁 xp/
│  │  │  │  ├─ 📁 events/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 🚏 routes/ - API route definitions
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 services/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 xp-actions-schema.js
│  │  │  │  ├─ 📄 xp-actions-schema.ts
│  │  │  │  ├─ 📄 xp-actions.controller.ts
│  │  │  │  ├─ 📄 xp-actions.js
│  │  │  │  ├─ 📄 xp-actions.ts
│  │  │  │  ├─ 📄 xp.admin.routes.ts
│  │  │  │  ├─ 📄 xp.controller.ts
│  │  │  │  ├─ 📄 xp.events.js
│  │  │  │  ├─ 📄 xp.events.ts
│  │  │  │  ├─ 📄 xp.routes.ts
│  │  │  │  ├─ 📄 xp.service.js
│  │  │  │  └─ 📄 xp.service.ts
│  │  │  └─ 📄 README.md
│  │  ├─ 📁 lib/
│  │  │  ├─ 📁 auth/
│  │  │  │  └─ 📄 canUser.ts
│  │  │  ├─ 📁 emoji/
│  │  │  │  └─ 📄 unlockEmojiPack.ts
│  │  │  ├─ 📁 forum/
│  │  │  │  ├─ 📄 canUserPost.ts
│  │  │  │  ├─ 📄 getAvailablePrefixes.ts
│  │  │  │  ├─ 📄 getForumRules.ts
│  │  │  │  ├─ 📄 prefixEngine.ts
│  │  │  │  └─ 📄 shouldAwardXP.ts
│  │  │  ├─ 📁 mentions/
│  │  │  │  ├─ 📄 createMentionsIndex.ts
│  │  │  │  └─ 📄 utils.ts
│  │  │  ├─ 📁 moderation/
│  │  │  │  └─ 📄 applyModerationAction.ts
│  │  │  ├─ 📁 wallet/
│  │  │  │  └─ 📄 testUtils.ts
│  │  │  ├─ 📄 auth.ts
│  │  │  ├─ 📄 lucia.ts
│  │  │  └─ 📄 session.ts
│  │  ├─ 📁 middleware/
│  │  │  ├─ 📁 __tests__/
│  │  │  │  └─ 📄 authenticate-jwt.test.ts
│  │  │  ├─ 📄 AUTH_MIGRATION.md
│  │  │  ├─ 📄 auth-hybrid.ts
│  │  │  ├─ 📄 auth.ts
│  │  │  ├─ 📄 auth.unified.ts
│  │  │  ├─ 📄 authenticate-jwt.ts
│  │  │  ├─ 📄 authenticate.ts
│  │  │  ├─ 📄 centralized-error-handler.middleware.ts
│  │  │  ├─ 📄 dev-security.middleware.ts
│  │  │  ├─ 📄 error-handler.middleware.ts
│  │  │  ├─ 📄 lucia-auth.ts
│  │  │  ├─ 📄 subscription-permissions.ts
│  │  │  ├─ 📄 trace.middleware.ts
│  │  │  ├─ 📄 transform-response.ts
│  │  │  ├─ 📄 validate-request.ts
│  │  │  ├─ 📄 validate-route-ids.middleware.ts
│  │  │  └─ 📄 validate.ts
│  │  ├─ 🚏 routes/ - API route definitions
│  │  │  ├─ 📁 api/
│  │  │  │  ├─ 📁 store/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📁 user/
│  │  │  │  │  ... (max depth reached)
│  │  │  │  ├─ 📄 index.ts
│  │  │  │  ├─ 📄 notifications.ts
│  │  │  │  ├─ 📄 preferences.routes.ts
│  │  │  │  ├─ 📄 themes.routes.ts
│  │  │  │  └─ 📄 ui.routes.ts
│  │  │  ├─ 📄 dev.routes.ts
│  │  │  └─ 📄 test-error.route.ts
│  │  ├─ 📁 test/
│  │  │  └─ 📁 performance/
│  │  │     └─ 📄 cache-test.ts
│  │  ├─ 📁 types/
│  │  │  └─ 📄 auth.types.ts
│  │  ├─ 📁 utils/
│  │  │  ├─ 📄 api-responses.ts
│  │  │  ├─ 📄 db-utils.ts
│  │  │  ├─ 📄 dev-auth-startup.ts
│  │  │  ├─ 📄 dgt-treasury-init.ts
│  │  │  ├─ 📄 dgt-wallet-integration.ts
│  │  │  ├─ 📄 economy-loader.ts
│  │  │  ├─ 📄 environment.ts
│  │  │  ├─ 📄 response.ts
│  │  │  ├─ 📄 seed-dev-user.ts
│  │  │  ├─ 📄 shop-utils.ts
│  │  │  ├─ 📄 slugify.ts
│  │  │  ├─ 📄 task-scheduler.ts
│  │  │  └─ 📄 wallet-utils.ts
│  │  └─ 📄 app.ts
│  ├─ 📁 test/
│  │  ├─ 📁 ccpayment-webhook/
│  │  │  └─ 📄 webhook.test.ts
│  │  ├─ 📁 engagement/
│  │  │  └─ 📄 tip.test.ts
│  │  └─ 📁 wallet/
│  │     └─ 📄 dgt-service.test.ts
│  ├─ 📄 .eslintignore
│  ├─ 📄 cookies.txt
│  ├─ 📄 IMPORT_GUIDE.md
│  ├─ 📄 index.ts
│  ├─ 📄 package.json
│  ├─ 📄 README.md
│  ├─ 📄 register-path-aliases.ts
│  ├─ 📄 routes.ts
│  ├─ 📄 tsconfig.build.json
│  ├─ 📄 tsconfig.build.tsbuildinfo
│  ├─ 📄 tsconfig.eslint.json
│  ├─ 📄 tsconfig.json
│  └─ 📄 tsconfig.typecheck.json
├─ 🤝 shared/ - Isomorphic code (client & server)
│  ├─ 📁 admin-core/
│  │  └─ 📄 README.md
│  ├─ 🔧 config/ - Shared build/tooling configuration
│  │  ├─ 📄 auth.config.ts
│  │  ├─ 📄 default-frames.config.js
│  │  ├─ 📄 default-frames.config.ts
│  │  ├─ 📄 economy.config.js
│  │  ├─ 📄 economy.config.ts
│  │  ├─ 📄 forum-economy.config.ts
│  │  ├─ 📄 forum-map.config.js
│  │  ├─ 📄 forum-map.config.ts
│  │  ├─ 📄 forumThemes.config.ts
│  │  ├─ 📄 index.ts
│  │  ├─ 📄 paths.config.ts
│  │  ├─ 📄 social.config.ts
│  │  └─ 📄 wallet.config.ts
│  ├─ 📁 dto/
│  │  ├─ 📁 user/
│  │  │  ├─ 📄 index.ts
│  │  │  └─ 📄 UserDTO.ts
│  │  └─ 📄 index.ts
│  ├─ 📁 economy/
│  │  ├─ 📄 index.ts
│  │  ├─ 📄 rain-tip-config.ts
│  │  ├─ 📄 README.md
│  │  ├─ 📄 reputation-calculator.ts
│  │  ├─ 📄 reward-calculator.ts
│  │  └─ 📄 shop-items.ts
│  ├─ 📁 lib/
│  │  ├─ 📁 emoji/
│  │  │  ├─ 📄 types.ts
│  │  │  └─ 📄 unlockEmojiPack.ts
│  │  ├─ 📁 forum/
│  │  │  ├─ 📄 getAvailablePrefixes.ts
│  │  │  ├─ 📄 prefixEngine.ts
│  │  │  └─ 📄 shouldAwardXP.ts
│  │  ├─ 📁 mentions/
│  │  │  ├─ 📄 createMentionsIndex.ts
│  │  │  ├─ 📄 types.ts
│  │  │  └─ 📄 utils.ts
│  │  ├─ 📁 moderation/
│  │  │  ├─ 📄 applyModerationAction.ts
│  │  │  └─ 📄 types.ts
│  │  ├─ 📁 wallet/
│  │  │  └─ 📄 testUtils.ts
│  │  └─ 📄 index.ts
│  ├─ 📁 shims/
│  ├─ 📁 signature/
│  │  └─ 📄 SignatureTierConfig.ts
│  ├─ 🏗️ src/ - Core application source code
│  │  ├─ 📁 types/
│  │  │  └─ 📄 brand.types.ts
│  │  └─ 📁 utils/
│  │     ├─ 📄 ids.ts
│  │     └─ 📄 index.ts
│  ├─ 📁 test-utils/
│  │  └─ 📄 mock-uuid.ts
│  ├─ 📁 types/
│  │  ├─ 📁 entities/
│  │  │  ├─ 📄 index.ts
│  │  │  ├─ 📄 reputation.types.ts
│  │  │  ├─ 📄 title.types.js
│  │  │  └─ 📄 title.types.ts
│  │  ├─ 📄 api.types.ts
│  │  ├─ 📄 config.types.ts
│  │  ├─ 📄 core-entities.schema.ts
│  │  ├─ 📄 cosmetics.types.ts
│  │  ├─ 📄 economy-core.types.ts
│  │  ├─ 📄 economy.schema.ts
│  │  ├─ 📄 economy.ts
│  │  ├─ 📄 features.schema.ts
│  │  ├─ 📄 forum-core.types.ts
│  │  ├─ 📄 ids.js
│  │  ├─ 📄 ids.ts
│  │  ├─ 📄 index.ts
│  │  ├─ 📄 payment.types.ts
│  │  ├─ 📄 post.types.ts
│  │  ├─ 📄 role.types.ts
│  │  ├─ 📄 role.utils.ts
│  │  ├─ 📄 theme.types.ts
│  │  ├─ 📄 thread.types.ts
│  │  ├─ 📄 transaction.types.ts
│  │  ├─ 📄 user.types.ts
│  │  ├─ 📄 validation.ts
│  │  ├─ 📄 wallet.transformer.js
│  │  ├─ 📄 wallet.transformer.ts
│  │  ├─ 📄 wallet.types.js
│  │  ├─ 📄 wallet.types.ts
│  │  └─ 📄 xp.schema.ts
│  ├─ 📁 utils/
│  │  ├─ 📄 cast.ts
│  │  ├─ 📄 id-conversions.ts
│  │  ├─ 📄 id-validation.ts
│  │  ├─ 📄 id.js
│  │  ├─ 📄 id.ts
│  │  ├─ 📄 title-utils.js
│  │  └─ 📄 title-utils.ts
│  ├─ 📁 validation/
│  │  ├─ 📁 user/
│  │  │  ├─ 📄 index.ts
│  │  │  └─ 📄 user.schema.ts
│  │  ├─ 📄 common.schemas.ts
│  │  └─ 📄 index.ts
│  ├─ 📁 validators/
│  │  ├─ 📄 admin.ts
│  │  └─ 📄 index.ts
│  ├─ 📄 constants.ts
│  ├─ 📄 index.ts
│  ├─ 📄 package.json
│  ├─ 📄 README.md
│  ├─ 📄 tsconfig.json
│  ├─ 📄 types.js
│  └─ 📄 types.ts
├─ 🧪 tests/ - End-to-end tests (Playwright)
│  ├─ 📁 e2e/
│  │  ├─ 📁 behavioral-flows/
│  │  │  └─ 📄 user-journey-analytics.spec.ts
│  │  ├─ 📁 cross-domain/
│  │  │  └─ 📄 data-consistency.spec.ts
│  │  ├─ 📁 fixtures/
│  │  │  ├─ 📄 test-setup.ts
│  │  │  └─ 📄 user-journeys.ts
│  │  ├─ 📁 helpers/
│  │  │  └─ 📄 behavior-analyzer.ts
│  │  ├─ 📁 reports/
│  │  │  └─ 📄 test-analytics-reporter.ts
│  │  ├─ 📄 admin-settings.spec.ts
│  │  ├─ 📄 forum-home.spec.ts
│  │  ├─ 📄 test-forum-e2e.spec.ts
│  │  └─ 📄 wallet-deposit-flow.spec.ts
│  ├─ 📁 integration/
│  │  └─ 📄 webhook.test.ts
│  └─ 📄 test-threads-simple.ts
├─ 📁 tools/
│  ├─ 📁 claude-hooks/
│  │  └─ 📄 run-checks.cjs
│  ├─ 📁 eslint/
│  │  ├─ 📁 plugins/
│  │  │  └─ 📁 degen/
│  │  │     ├─ 📁 rules/
│  │  │     │  ... (max depth reached)
│  │  │     ├─ 📄 index.cjs
│  │  │     ├─ 📄 index.js
│  │  │     └─ 📄 package.json
│  │  └─ 📁 rules/
│  │     ├─ 📄 enforce-path-aliases.js
│  │     └─ 📄 no-number-id.js
│  └─ 📄 runClaude.ts
├─ 📄 .claude.json
├─ 📄 .cursorrules
├─ 📄 .env.example
├─ 📄 .eslintignore
├─ 📄 .eslintrc.cjs
├─ 📄 .gitignore
├─ 📄 .jscodeshift.json
├─ 📄 .lintstagedrc.js
├─ 📄 .markdown-link-check.json
├─ 📄 .markdownlint.json
├─ 📄 .markdownlintignore
├─ 📄 .mcp.json
├─ 📄 .mcp.json.example
├─ 📄 .npmrc
├─ 📄 .prettierignore
├─ 📄 .prettierrc
├─ 📄 AGENT_BRIEFING.md
├─ 📄 CHANGELOG.md
├─ 📄 CLAUDE.md
├─ 📄 CODEBASE_RULES.md
├─ 📄 codegen.yml
├─ 📄 DATABASE_MIGRATION_PLAN.md
├─ 📄 GEMINI.md
├─ 📄 IMMEDIATE_ACTIONS.md
├─ 📄 package.json
├─ 📄 pnpm-workspace.yaml
├─ 📄 PROJECT_TREE.md
├─ 📄 README.md
├─ 📄 test-profile.html
├─ 📄 TITLE_DISPLAY_FIX.md
├─ 📄 tsconfig.base.json
├─ 📄 tsconfig.eslint.json
├─ 📄 tsconfig.json
├─ 📄 TYPE_CLEANUP_ACTION_PLAN.md
├─ 📄 TYPE_MIGRATION_PLAN.md
├─ 📄 TYPE_MIGRATION_SUMMARY.md
├─ 📄 unfuck-progress.md
└─ 📄 UNFUCKING-OPERATIONS.md
```

---

### 📊 Project Stats

- **Total Lines of Code:** ~697,070

Directory tree written to: directory-tree.md
