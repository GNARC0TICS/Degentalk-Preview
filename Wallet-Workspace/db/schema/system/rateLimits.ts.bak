...
export const rateLimits = pgTable('rate_limits', {
  ...
  createdAt: timestamp('created_at').notNull().default(sql`now()`), // Changed from .notNull() to .default(sql`now()`) for auto-population
  updatedAt: timestamp('updated_at').notNull().default(sql`now()`), // Changed from .notNull() to .default(sql`now()`) for auto-population
});
