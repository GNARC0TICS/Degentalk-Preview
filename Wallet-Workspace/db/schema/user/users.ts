import { pgTable, serial, text, varchar, boolean, timestamp, integer, bigint, uuid, jsonb, unique, AnyPgColumn } from "drizzle-orm/pg-core";
...
export const users = pgTable('users', {
  ...
  avatarFrameId: integer('avatar_frame_id').references(() => avatarFrames.id, { onDelete: 'set null' }),
  ...
  referrerId: integer('referrer_id').references((): AnyPgColumn => users.id, { onDelete: 'set null' }),
  ...
  // usernameUnique: unique('users_username_unique').on(table.username), // Temporarily commented out for debugging
...
/* // Temporarily comment out Zod schema generation for debugging Drizzle Kit
