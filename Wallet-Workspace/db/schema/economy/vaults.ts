import { pgTable, serial, integer, varchar, doublePrecision, timestamp, jsonb, text, index } from "drizzle-orm/pg-core";
...
export const vaults = pgTable('vaults', {
  ...
  lockTransactionId: integer('lock_transaction_id').references(() => transactions.id, { onDelete: 'set null' }),
  unlockTransactionId: integer('unlock_transaction_id').references(() => transactions.id, { onDelete: 'set null' }),
...
const walletAddressSchema = z.union([
  ...
  z.string().refine(
    (addr) => isDevelopment() && ['TREASURY_SYSTEM_TRON_ADDRESS', 'TRzJRNqjgmzCR4zwm6wLMnECDj35zZZnVt'].includes(addr),
