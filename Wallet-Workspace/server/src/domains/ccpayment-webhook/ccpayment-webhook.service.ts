...
export class CCPaymentWebhookService {
   ...
   * @returns Processing result
   */
  private async handleDepositCompleted(event: CCPaymentWebhookEvent): Promise<{ success: boolean; message: string }> {
    try {
      ...
      await dgtService.fulfillDgtPurchase(
        purchaseOrder.id,
        'confirmed',
        {
   ...
   * @returns Processing result
   */
  private async handleDepositFailed(event: CCPaymentWebhookEvent): Promise<{ success: boolean; message: string }> {
    try {
      // Find the DGT purchase order associated with this deposit
      const [purchaseOrder] = await db
        .select()
   ...
   * @returns Processing result
   */
  private async handleWithdrawalCompleted(event: CCPaymentWebhookEvent): Promise<{ success: boolean; message: string }> {
    try {
   ...
   * @returns Processing result
   */
  private async handleWithdrawalFailed(event: CCPaymentWebhookEvent): Promise<{ success: boolean; message: string }> {
    try {
      // Find the transaction associated with this withdrawal
      const [transaction] = await db
        .select()
