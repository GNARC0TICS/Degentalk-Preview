...
export class AdminTreasuryController {
  ...
  async sendFromTreasury(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid input', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      const adminId = getUserId(req);
      const result = await adminTreasuryService.sendFromTreasury(validation.data, adminId);
      
      await adminController.logAction(req, 'TREASURY_SEND', 'treasury', validation.data.userId.toString(), {
  ...
  async recoverToTreasury(req: Request, res: Response) {
    try {
      ...
      await adminController.logAction(req, 'TREASURY_RECOVER', 'treasury', validation.data.userId.toString(), {
        amount: validation.data.amount,
  ...
  async massAirdrop(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid input for mass airdrop', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      const adminId = getUserId(req);
  ...
  async updateTreasurySettings(req: Request, res: Response) {
    try {
      ...
      await adminController.logAction(req, 'TREASURY_SETTINGS_UPDATE', 'treasury_settings', 'general', {
