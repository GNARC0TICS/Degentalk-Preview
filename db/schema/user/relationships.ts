...
export const userRelationships = pgTable(
	...
	(table) => ({
		userTargetTypeUnique: unique('user_relationships_user_target_type_unique').on(
			table.userId,
			table.targetUserId,
			table.type
		),
		// Legacy unique constraint
		followerFollowingUnique: unique('user_relationships_follower_following_type_unique').on(
			table.followerId,
			table.followingId,
