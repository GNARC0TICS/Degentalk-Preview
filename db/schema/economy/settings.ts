import { pgTable, serial, varchar, bigint, integer, doublePrecision, boolean, timestamp, text } from "drizzle-orm/pg-core";
...
export const treasurySettings = pgTable('treasury_settings', {
  ...
  minWithdrawalAmount: bigint('min_withdrawal_amount', { mode: 'number' }).notNull().default(5000000),
...
export const tipSettings = pgTable('tip_settings', {
  ...
  minAmountDGT: doublePrecision('min_amount_dgt').notNull().default(10.00),
  minAmountUSDT: doublePrecision('min_amount_usdt').notNull().default(0.10),
  maxAmountDGT: doublePrecision('max_amount_dgt').notNull().default(1000.00),
  maxAmountUSDT: doublePrecision('max_amount_usdt').notNull().default(100.00),
  burnPercentage: doublePrecision('burn_percentage').notNull().default(5.00),
  processingFeePercentage: doublePrecision('processing_fee_percentage').notNull().default(2.50),
...
export const rainSettings = pgTable('rain_settings', {
  ...
  enabled: boolean('enabled').notNull().default(true),
  minAmountDGT: doublePrecision('min_amount_dgt').notNull().default(10.00),
  minAmountUSDT: doublePrecision('min_amount_usdt').notNull().default(1.00),
