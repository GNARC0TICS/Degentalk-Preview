import { pgTable, uuid, varchar, decimal, timestamp, jsonb, integer, bigint, pgEnum, index } from 'drizzle-orm/pg-core';
...
export const adImpressions = pgTable('ad_impressions', {
    ...
    timestampCampaignIdx: index('idx_impressions_timestamp_campaign').on(table.timestamp, table.campaignId),
    campaignPlacementIdx: index('idx_impressions_campaign_placement').on(table.campaignId, table.placementId),
    userHashTimestampIdx: index('idx_impressions_user_timestamp').on(table.userHash, table.timestamp),
    // Performance indexes
    revenueTimestampIdx: index('idx_impressions_revenue_timestamp').on(table.revenue, table.timestamp)
}));
