...
import { pgTable, serial, text, integer, timestamp, uniqueIndex, primaryKey, boolean, jsonb } from "drizzle-orm/pg-core";
...
export const inventoryTransactionLinks = pgTable("inventory_transaction_links", {
  id: serial("id").primaryKey(),
  inventoryId: integer("inventory_id").notNull().references(() => userInventory.id, { onDelete: "cascade" }),
