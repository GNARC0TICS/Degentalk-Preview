...
export const conversationParticipants = pgTable('conversation_participants', {
  ...
  joinedAt: timestamp('joined_at').notNull().default(sql`now()`), // Changed defaultNow() to sql`now()`
  lastReadAt: timestamp('last_read_at'),
  isActive: boolean('is_active').notNull().default(true),
  isMuted: boolean('is_muted').notNull().default(false),
  isAdmin: boolean('is_admin').notNull().default(false),
}, (table) => ({
  conversationUserUnique: unique('conversation_user_unique').on(table.conversationId, table.userId),
  conversationIdx: index('idx_conversation_participants_conversation_id').on(table.conversationId),
