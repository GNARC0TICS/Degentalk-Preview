import { pgTable, serial, integer, timestamp, varchar, text, jsonb, index, unique } from "drizzle-orm/pg-core";
...
export const onlineUsers = pgTable('online_users', {
  ...
  lastActive: timestamp('last_active').notNull().default(sql`now()`), // Changed defaultNow() to sql`now()`
