...
router.post('/', async (req, res) => {
	try {
		...
		const transaction = await db.query.transactions.findFirst({
			where: (transactions, { eq }) => eq(transactions.metadata['orderId'], orderId)
		});
		...
		if (transactionType === 'deposit' && newStatus === 'completed') {
			...
			if (transaction.userId) {
				...
				logger.info(
					`User ${transaction.userId} credited with ${numericAmount} DGT for deposit ${orderId}`
				);
			}
			...
			if (newStatus === 'completed' && transaction.userId) {
				...
				logger.info(
					`User ${transaction.userId} debited ${numericAmount} DGT for withdrawal ${orderId}`
				);
			}
		}
