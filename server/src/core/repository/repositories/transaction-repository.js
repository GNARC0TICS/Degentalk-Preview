...
export class TransactionRepository extends BaseRepository {
    ...
    async findByUserId(userId, options = {}) {
        try {
            const queryOptions = {
                ...
                sort: options.sort || [{ column: 'createdAt', direction: 'desc' }]
            };
            ...
            throw new RepositoryError('Failed to find transactions by user ID', 'FIND_BY_USER_ID_ERROR', 500, { userId, options, originalError: error });
        }
    }
    /**
    ...
    async findByStatus(status, options = {}) {
        try {
            ...
            throw new RepositoryError('Failed to find transactions by status', 'FIND_BY_STATUS_ERROR', 500, { status, options, originalError: error });
        }
    }
    /**
    ...
    async getTotalByUser(userId, type) {
        try {
            ...
            throw new RepositoryError('Failed to get total transactions by user', 'GET_TOTAL_BY_USER_ERROR', 500, { userId, type, originalError: error });
        }
    }
    /**
    ...
    async getBalanceByUser(userId) {
        try {
            ...
            const [creditsResult] = await db
                .select({
                credits: sql `COALESCE(SUM(${transactions.amount}), 0)`
            })
                .from(transactions)
            ...
            const [debitsResult] = await db
                .select({
                debits: sql `COALESCE(SUM(ABS(${transactions.amount})), 0)`
            })
                .from(transactions)
                .where(and(eq(transactions.userId, userId), sql `${transactions.amount} < 0`));
    ...
    async findByDateRange(startDate, endDate, options = {}) {
        try {
            ...
            throw new RepositoryError('Failed to find transactions by date range', 'FIND_BY_DATE_RANGE_ERROR', 500, { startDate, endDate, options, originalError: error });
        }
    }
    /**
    ...
    async getUserStats(userId) {
        try {
            const [statsResult] = await db
                ...
                totalCredits: sql `COALESCE(SUM(CASE WHEN ${transactions.amount} > 0 THEN ${transactions.amount} ELSE 0 END), 0)`,
                totalDebits: sql `COALESCE(SUM(CASE WHEN ${transactions.amount} < 0 THEN ABS(${transactions.amount}) ELSE 0 END), 0)`,
                lastTransactionAt: sql `MAX(${transactions.createdAt})`
            })
                .from(transactions)
            ...
            throw new RepositoryError('Failed to get user transaction statistics', 'GET_USER_STATS_ERROR', 500, { userId, originalError: error });
        }
    }
    /**
