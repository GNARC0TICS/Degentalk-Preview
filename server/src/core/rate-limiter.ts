 ...
 */
export function rateLimiter(options: { endpoint: string; limit?: number; windowMs?: number }) {
	const {
		endpoint,
	...
	return async function (req: Request, res: Response, next: NextFunction) {
		try {
			...
			const existingLimits = await db
				.select()
				.from(rateLimits)
				.where(and(eq(rateLimits.key, key), eq(rateLimits.endpoint, endpoint)));
					...
					if (limitRecord.count >= limit) {
						...
						return res.status(429).json({
							...
							retryAfter: Math.ceil(
								(new Date(limitRecord.resetAt).getTime() - now.getTime()) / 1000
							)
						});
					} else {
						// Increment counter
