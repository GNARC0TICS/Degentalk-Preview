 ...
 */
export function getTTL(policy, key) {
    const policyGroup = CachePolicies[policy];
    const ttl = policyGroup[key];
...
export const DevCachePolicies = {
    realtime: Object.fromEntries(Object.entries(CachePolicies.realtime).map(([k, v]) => [k, Math.max(5, v / 2)])),
    standard: Object.fromEntries(Object.entries(CachePolicies.standard).map(([k, v]) => [k, Math.max(10, v / 3)])),
    extended: Object.fromEntries(Object.entries(CachePolicies.extended).map(([k, v]) => [k, Math.max(30, v / 4)]))
};
/**
 * Get environment-appropriate cache policies
 */
export function getCachePolicies() {
