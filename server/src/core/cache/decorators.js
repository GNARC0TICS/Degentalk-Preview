...
function logCacheOperation(operation) {
    ...
    // Development logging
    debugCacheKey(operation.key, operation.operation);
}
/**
 * Generate cache key from class, method, and arguments
 */
function generateCacheKey(className, methodName, args) {
 ...
 */
function createCacheDecorator(policy, policyKey) {
    return function (target, propertyName, descriptor) {
        ...
        descriptor.value = async function (...args) {
            ...
            const ttl = policies[policy][policyKey];
 ...
 */
export function CacheMinute(minutes = 5) {
    return function (target, propertyName, descriptor) {
        const originalMethod = descriptor.value;
        const className = target.constructor.name;
        descriptor.value = async function (...args) {
 ...
 */
export function CustomCache(ttlSeconds, keyPrefix) {
    return function (target, propertyName, descriptor) {
        const originalMethod = descriptor.value;
        const className = target.constructor.name;
        descriptor.value = async function (...args) {
 ...
 */
export function getRecentCacheOperations(limit = 20) {
