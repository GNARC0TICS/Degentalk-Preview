...
const CATEGORY_TTLS: Record<CacheCategory, number> = {
...
class UnifiedCacheService {
  ...
  private readonly maxMemoryMB = 100; // Max memory usage for in-memory cache
  ...
  private generateKey(key: string, category?: CacheCategory, prefix?: string): string {
  ...
  private updateCategoryMetrics(category: CacheCategory, hit: boolean): void {
  ...
  async get<T>(key: string, options: CacheOptions = {}): Promise<T | null> {
    ...
    try {
      ...
      if (options.fallbackToMemory !== false) {
        const memoryCached = this.memoryCache.get<T>(fullKey);
  ...
  async deletePattern(pattern: string, category?: CacheCategory): Promise<number> {
    let deletedCount = 0;
    ...
    try {
      ...
      const memoryKeys = this.memoryCache.keys().filter(key => 
        key.includes(pattern) && (!category || key.includes(category))
      );
...
export function CacheMinute(category?: CacheCategory) {
  return Cache({ ttl: 60, category });
}

export function CacheHour(category?: CacheCategory) {
  return Cache({ ttl: 3600, category });
}

export function CacheDay(category?: CacheCategory) {
