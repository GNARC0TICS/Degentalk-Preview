...
class SettingsService extends EventEmitter {
  ...
  getWalletSettingsSync(): WalletSettings {
    return this.walletSettingsCache ? { ...this.walletSettingsCache } : { ...this.defaultWalletSettings };
  }
  ...
  private validateWalletSettings(updates: Partial<WalletSettings>): void {
    ...
    if (updates.autoConvertDeposits !== undefined && typeof updates.autoConvertDeposits !== 'boolean') {
    ...
    if (updates.withdrawalsEnabled !== undefined && typeof updates.withdrawalsEnabled !== 'boolean') {
  ...
  private async saveWalletSettingsToDb(settings: WalletSettings): Promise<void> {
    try {
      ...
      logger.error('SettingsService', 'Failed to save wallet settings to database', { error, settings });
      throw error;
    }
  }
}
