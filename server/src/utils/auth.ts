...
export function getUserIdFromRequest(req: Request): number | undefined {
	if (req.user) {
		...
		if (typeof user.user_id === 'number') {
			logger.warn('AuthUtil', 'User ID accessed via fallback req.user.user_id', {
				userIdSource: 'user_id_fallback'
			});
			return user.user_id;
		}
	}
	logger.error('AuthUtil', 'User ID not found or not a number in req.user', {
		userObject: req.user
	});
