...
export class ForumRepository {
	 ...
	 */
	async countPostsByThread(threadId: ThreadId): Promise<number> {
	 ...
	 * Find post by ID
	 */
	async findPostById(postId: PostId) {
		const [post] = await db
			.select()
			.from(posts)
	 ...
	 * Update post content
	 */
	async updatePost(postId: PostId, data: PostUpdateData) {
	 ...
	 */
	async incrementLikeCount(postId: PostId) {
		await db
			.update(posts)
			.set({
	 ...
	 */
	async decrementLikeCount(postId: PostId) {
		await db
			.update(posts)
			.set({
				likeCount: sql`${posts.likeCount} - 1`,
	 ...
	 */
	async findPostReaction(postId: PostId, userId: UserId) {
	 ...
	 */
	async findPostReplies(postId: PostId, limit: number = 50) {
		return await db
			.select()
			.from(posts)
	 ...
	 */
	async getUserPostStats(userId: UserId) {
	 ...
	 */
	async incrementThreadViewCount(threadId: ThreadId) {
		await db
			.update(threads)
			.set({
	 ...
	 */
	async getEarliestPostsForThreads(threadIds: ThreadId[]) {
		return await db
			.select()
			.from(posts)
			.where(
				and(
					sql`${posts.threadId} = ANY(${threadIds})`,
	 ...
	 */
	async getDescendantForumThreads(forumId: string, limit: number = 50) {
