...
describe('ForumService - getForumStructure', () => {
  ...
  test('should correctly identify primary zones via pluginData.configZoneType', async () => {
    mockGetCategoriesWithStats.mockResolvedValue([
      { id: 1, name: 'Primary Zone 1', slug: 'pz1', type: 'zone', pluginData: { configZoneType: 'primary', features: ['feat1'] }, parentId: null, threadCount: 0, postCount: 0, isZone: true, canonical: true },
      { id: 2, name: 'General Zone 1', slug: 'gz1', type: 'zone', pluginData: { configZoneType: 'general' }, parentId: null, threadCount: 0, postCount: 0, isZone: true, canonical: true },
      { id: 3, name: 'Forum In Primary', slug: 'fip1', type: 'forum', pluginData: {}, parentId: 1, threadCount: 5, postCount: 10, isZone: false, canonical: false },
    ]);
  ...
  test('should default to isPrimary: false and log warning if pluginData is missing configZoneType', async () => {
    mockGetCategoriesWithStats.mockResolvedValue([
      { id: 1, name: 'Zone Missing Type', slug: 'zmt', type: 'zone', pluginData: { someOtherKey: 'value' }, parentId: null, threadCount: 0, postCount: 0, isZone: true, canonical: true },
    ]);

    const structure = await forumService.getForumStructure();
    expect(structure.zones[0]?.isPrimary).toBe(false);
    expect(mockLoggerWarn).toHaveBeenCalledWith('ForumService', 'Missing configZoneType in pluginData', expect.anything());
  });

  test('should default to isPrimary: false and log warning if pluginData is null', async () => {
    mockGetCategoriesWithStats.mockResolvedValue([
      { id: 1, name: 'Zone Null PluginData', slug: 'znpd', type: 'zone', pluginData: null, parentId: null, threadCount: 0, postCount: 0, isZone: true, canonical: true },
    ]);

    const structure = await forumService.getForumStructure();
    expect(structure.zones[0]?.isPrimary).toBe(false);
    expect(mockLoggerWarn).toHaveBeenCalledWith('ForumService', 'Missing or null pluginData for zone', expect.anything());
  });

  test('should default to isPrimary: false and log warning if pluginData.configZoneType is malformed', async () => {
    mockGetCategoriesWithStats.mockResolvedValue([
      { id: 1, name: 'Zone Malformed Type', slug: 'zmtp', type: 'zone', pluginData: { configZoneType: 'super_primary_ultra' }, parentId: null, threadCount: 0, postCount: 0, isZone: true, canonical: true },
    ]);

    const structure = await forumService.getForumStructure();
    expect(structure.zones[0]?.isPrimary).toBe(false);
    expect(mockLoggerWarn).toHaveBeenCalledWith('ForumService', 'Malformed configZoneType in pluginData', expect.anything());
  });

  test('should correctly parse features, customComponents, and staffOnly from pluginData', async () => {
    mockGetCategoriesWithStats.mockResolvedValue([
      { 
        id: 1, name: 'Full Zone', slug: 'fz1', type: 'zone', 
        pluginData: { 
        ...
        parentId: null, threadCount: 0, postCount: 0, isZone: true, canonical: true 
      },
      { 
        id: 2, name: 'Minimal Zone', slug: 'mz1', type: 'zone', 
        pluginData: { 
  ...
  test('should handle empty pluginData object for a zone', async () => {
    mockGetCategoriesWithStats.mockResolvedValue([
      { 
        id: 1, name: 'Empty Plugin Zone', slug: 'epz', type: 'zone', 
        pluginData: {}, 
        parentId: null, threadCount: 0, postCount: 0, isZone: true, canonical: true 
      },
    ]);
