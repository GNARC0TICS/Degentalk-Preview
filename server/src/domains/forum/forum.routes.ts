...
async function userCanManageThread(currentUserId: number, threadId: number, dbInstance: typeof db): Promise<boolean> {
  const threadDetails = await dbInstance.select({ ownerId: threads.userId }).from(threads).where(eq(threads.id, threadId)).limit(1);
...
const updateDraftSchema = insertThreadDraftSchema.extend({
  editorState: z.any().optional(),
}).partial().required({
  categoryId: true
});
...
router.get('/threads', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const threadListRaw = await db.select({
      ...
      if (threadIds.length > 0) {
        ...
        tagsByThreadId = tagResults.reduce((acc: Record<number, { id: number; name: string; slug: string }[]>, row: any) => {
          if (!acc[row.threadId]) {
            acc[row.threadId] = [];
          }
...
router.get('/threads/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (userId) {
       const bookmarkCheck = await db.select({ threadId: userThreadBookmarks.threadId })
           .from(userThreadBookmarks)
           .where(and(
               eq(userThreadBookmarks.userId, userId),
               eq(userThreadBookmarks.threadId, threadId)
           ))
           .limit(1);
    ...
    if (userId && postIds.length > 0) {
       const likesResult = await db.select({ postId: postReactions.postId })
         .from(postReactions)
         .where(and(
...
router.post('/threads', requireAuth, async (req: Request, res: Response) => { // Changed isAuthenticated to requireAuth
  try {
      const userId = getUserId(req);

      const combinedSchema = insertThreadSchema.extend({
          content: z.string().min(1), 
          editorState: z.any().optional(),
      ...
      if (!parseResult.success) {
          return res.status(400).json({ message: "Invalid thread data", errors: parseResult.error.flatten() });
      }
      ...
      const category = await db.select().from(forumCategories).where(eq(forumCategories.id, Number(validatedThreadData.categoryId))).limit(1);
      if (!category[0]) {
      ...
      const { thread: newThreadData, firstPost } = await db.transaction(async (tx: typeof db) => {
          ...
          const [createdThread] = await tx.insert(threads)
              .values({
          ...
          const [createdPost] = await tx.insert(posts)
              .values({
          ...
          if (validatedThreadData.tagNames && validatedThreadData.tagNames.length > 0) {
            for (const tagName of validatedThreadData.tagNames) {
              let tag;
              const existingTags = await tx.select({ id: tags.id }).from(tags).where(eq(tags.name, tagName)).limit(1);
              tag = existingTags[0];

              if (!tag) {
                const newTagSlug = await slugify(tagName);
                const newTagsResults = await tx.insert(tags).values({ name: tagName, slug: newTagSlug }).returning({ id: tags.id, name: tags.name, slug: tags.slug });
                tag = newTagsResults[0];
              }
              if (tag) { // Ensure tag is defined before inserting into threadTags
                const existingThreadTag = await tx.select({ threadId: threadTags.threadId }) // select a field to check length
                  .from(threadTags)
          ...
          const finalThreadDetailsArray = await tx.select({
            id: threads.id, title: threads.title, slug: threads.slug, userId: threads.userId,
            prefixId: threads.prefixId, isSticky: threads.isSticky, isLocked: threads.isLocked,
            isHidden: threads.isHidden, viewCount: threads.viewCount, postCount: threads.postCount,
            firstPostLikeCount: threads.firstPostLikeCount, dgtStaked: threads.dgtStaked,
            createdAt: threads.createdAt, updatedAt: threads.updatedAt,
            isSolved: threads.isSolved, solvingPostId: threads.solvingPostId,
            user: { id: users.id, username: users.username, activeAvatarUrl: users.activeAvatarUrl, role: users.role, level: users.level, clout: users.clout, createdAt: users.createdAt },
          ...
          const threadTagsData = await tx.select({ id: tags.id, name: tags.name, slug: tags.slug })
            .from(threadTags)
            .innerJoin(tags, eq(threadTags.tagId, tags.id))
          ...
          return { thread: { ...finalThreadDetails, user: userPayload, category: categoryPayload, tags: threadTagsData }, firstPost: createdPost };
      });
...
router.put('/threads/:threadId/solve', requireAuth, async (req, res, next) => {
    try {
        ...
        if (!canManage) {
            return res.status(403).json({ message: 'Unauthorized to manage solved status for this thread.' });
        }
...
router.post('/threads/:threadId/tags', requireAuth, async (req: Request, res: Response) => { // Changed isAuthenticated to requireAuth
  try {
    const threadId = parseInt(req.params.threadId);
    const userId = getUserId(req);
    ...
    if (!canManage) {
      return res.status(403).json({ message: "You do not have permission to manage this thread's tags." });
    }

    // Insert or retrieve tag and create thread-tag association in a transaction
    const result = await db.transaction(async (tx: typeof db) => {
      // Check if tag already exists
      let [tag] = await tx.select({ id: tags.id, name: tags.name, slug: tags.slug })
        .from(tags)
      ...
      const existingThreadTag = await tx.select({ threadId: threadTags.threadId })
        .from(threadTags)
        .where(and(
          eq(threadTags.threadId, threadId), 
          eq(threadTags.tagId, tag.id)
        ))
        .limit(1);

      // If thread doesn't have this tag, add it
      if (existingThreadTag.length === 0) {
        await tx.insert(threadTags)
          .values({ threadId: threadId, tagId: tag.id });
...
router.delete('/threads/:threadId/tags/:tagId', requireAuth, async (req: Request, res: Response) => { // Changed isAuthenticated to requireAuth
  try {
    const threadId = parseInt(req.params.threadId);
    ...
    const deleteResult = await db.delete(threadTags)
...
router.post('/posts/:postId/react', requireAuth, async (req, res) => {
  try {
    ...
    const postResult = await db.select().from(posts).where(eq(posts.id, parseInt(postId)));
...
router.delete('/bookmarks/:threadId', requireAuth, async (req, res) => {
  try {
    ...
    await db.delete(userThreadBookmarks)
      .where(
        and(
          eq(userThreadBookmarks.threadId, threadId),
          eq(userThreadBookmarks.userId, userId)
...
router.post('/threads/:threadId/solve', requireAuth, async (req, res) => {
  try {
    ...
    const canManage = await userCanManageThread(userId, threadId, db);
    if (!canManage) {
      return res.status(403).json({ error: 'You do not have permission to mark this thread as solved' });
    }

    // If postId is provided, verify it exists and belongs to this thread
    if (postId) {
      const postResult = await db.select({ id: posts.id })
        .from(posts)
        .where(and(
          eq(posts.id, postId),
          eq(posts.threadId, threadId)
...
router.post('/threads/:threadId/unsolve', requireAuth, async (req, res) => {
  try {
    ...
    const canManage = await userCanManageThread(userId, threadId, db);
    if (!canManage) {
      return res.status(403).json({ error: 'You do not have permission to unmark this thread as solved' });
    }
...
router.get('/hot-threads', async (req: Request, res: Response) => {
  try {
    ...
    const hotThreads = await db.select({
      id: threads.id,
      title: threads.title,
      slug: threads.slug,
      postCount: threads.postCount,
    ...
    .leftJoin(posts, and(
      eq(threads.id, posts.threadId),
      eq(posts.isFirstPost, true)
    ))
    .where(
      and(
        eq(threads.isDeleted, false),
...
router.get('/debug/forums-by-parent', async (req: Request, res: Response) => {
  try {
    ...
    if (childForums.length === 0) {
      ...
      console.log('[DEBUG] Child forum names:', childForums.map(f => f.name));
    }
  ...
  try {
    ...
    const stats = {
      ...
      totalChildForumCount: relationships.categories.reduce(
        (sum, cat) => sum + cat.childForums.length, 0
      ),
      categoriesWithNoChildren: relationships.categories.filter(
        cat => cat.childForums.length === 0
...
router.get('/structure', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[API /forum/structure] Returning ${primaryZones.length} primary zones and ${categories.length} categories`);
...
router.get('/threads/:categoryId', async (req: Request, res: Response) => {
  try {
    ...
    const categoryThreads = await db.select({
      id: threads.id,
      title: threads.title,
      slug: threads.slug,
      postCount: threads.postCount,
    ...
    // Return the threads
    console.log(`[Forum Routes] Returning ${categoryThreads.length} threads for category ID ${categoryId}`);
