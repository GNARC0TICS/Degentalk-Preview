...
import { getForumBySlug as getForumBySlugFromConfig } from '@config/forumMap';
export class ConfigService {
    /**
     * Parse zone type from pluginData with proper validation
     */
    parseZoneType(pluginData) {
        if (pluginData &&
            typeof pluginData === 'object' &&
            pluginData !== null &&
            'configZoneType' in pluginData) {
            const t = pluginData.configZoneType;
            if (t === 'primary') {
                return 'primary';
            }
            else if (t === 'general') {
     ...
     */
    mapConfigForumToCategory(forum, zone, parentId = null) {
     ...
     */
    processZoneFeatures(pluginData) {
