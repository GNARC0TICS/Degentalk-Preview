...
export class PostService {
    ...
    async unlikePost(postId, userId) {
        try {
            ...
            const deletedRows = await db
                ...
                .where(and(eq(postReactions.postId, postId), eq(postReactions.userId, userId), eq(postReactions.reactionType, 'like')));
            if (deletedRows === 0) {
     ...
     */
    async tipPost(postId, tipperId, amount) {
        try {
     ...
     */
    async getPostReplies(parentPostId) {
        try {
