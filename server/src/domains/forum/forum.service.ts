...
import {
  ...
  inArray,
  SQL,
} from "drizzle-orm"; // Added inArray and SQL
...
export const forumService = {
    ...
    try {
      ...
      const categoriesWithCounts = await db
        .select({
          ...
          threadCount: sql<number>`count(DISTINCT ${threads.id})`.as(
            "threadCount"
          ),
      ...
      console.log(
        `[forumService] getCategoriesWithStats - Raw query result count: ${categoriesWithCounts.length}`
      );
      // console.log(
      //   "[forumService] getCategoriesWithStats - Raw query result (first 5):",
      //   categoriesWithCounts.slice(0, 5)
      // );
      // console.log(
      //   "[forumService] getCategoriesWithStats - Raw query result (full):",
      //   categoriesWithCounts
      // ); // Added full raw result log
      ...
      console.log(
        `[forumService] getCategoriesWithStats - Mapped categories count: ${categories.length}`
      );
      ...
      categories.slice(0, 5).forEach((cat) => {
        console.log(
          `[forumService] Category ${cat.name} (ID: ${cat.id}): Threads=${cat.threadCount}, Posts=${cat.postCount}`
        );
      });
  ...
  async getCategoriesTree(options: CategoriesTreeOptions = {}): Promise<any[]> {
    ...
    if (!allCategories || !Array.isArray(allCategories)) {
      console.error(
        "[SERVICE-FORUM] getCategoriesWithStats did not return a valid array for tree construction."
      );
      return [];
    }
    ...
    const categoryMap = new Map<
      number,
      ForumCategoryWithStats & { children: any[] }
    >();
    const tree: any[] = [];
  ...
  async getCategoryBySlug(
    slug: string
  ): Promise<ForumCategoryWithStats | null> {
  ...
  async getForumBySlugWithTopics(slug: string): Promise<{
    ...
    console.log(
      `[Forum Service] getForumBySlugWithTopics called with slug: ${slug}`
    );
    const categories = await this.getCategoriesWithStats();
    const forum = categories.find((cat) => cat.slug === slug) || null;
    if (!forum) {
      console.log(
        `[Forum Service] getForumBySlugWithTopics - Forum not found for slug: ${slug}`
      );
    ...
    console.log(
      `[Forum Service] getForumBySlugWithTopics - Found forum "${forum.name}" and ${topics.length} topics for slug: ${slug}`
    );
    return { forum, topics };
  },
  ...
  async getForumsByParentId(
    ...
    try {
      ...
      console.log(
        `[forumService] Found ${
          childForums.length
        } child forums for parentId ${parentId} (${
          parentForum?.name || "unknown parent"
        })`
      );

      if (childForums.length > 0) {
        console.log(
          `[forumService] Child forum names:`,
          childForums.map((f) => `${f.name} (ID: ${f.id})`)
        );
      } else {
        console.log(
          `[forumService] No child forums found for parentId ${parentId}`
        );
      }
      ...
      const categories = allCategories
        ...
        .map((category) => {
          ...
          const childForums = allCategories
            .filter(
              (forum) =>
                forum.parentId === category.id && forum.type === "forum"
            )
            .map((forum) => ({
              id: forum.id,
              name: forum.name,
  ...
  async searchThreads(params: ThreadSearchParams): Promise<{
    ...
    if (prefix) {
      const prefixResult = await db
        ...
        .from(threadPrefixes)
        .where(eq(threadPrefixes.name, prefix))
        .limit(1);
      if (prefixResult.length > 0)
        conditions.push(eq(threads.prefixId, prefixResult[0].id));
    }
    ...
    let orderByClause: (SQL<unknown> | SQL.Aliased<unknown>)[] = [
    ...
    const selectFields = {
      ...
      isFeatured: threads.isFeatured,
      featuredAt: threads.featuredAt,
      featuredBy: threads.featuredBy,
      featuredExpiresAt: threads.featuredExpiresAt,
      isDeleted: threads.isDeleted,
      deletedAt: threads.deletedAt,
      deletedBy: threads.deletedBy,
      ...
      isBoosted: threads.isBoosted,
      boostAmountDgt: threads.boostAmountDgt,
      boostExpiresAt: threads.boostExpiresAt,
    ...
    if (tagId) {
      threadResults = await db
        .select(selectFields)
        .from(threads)
        .innerJoin(users, eq(threads.userId, users.id))
        .innerJoin(
          threadTags,
          and(eq(threadTags.threadId, threads.id), eq(threadTags.tagId, tagId))
        )
    ...
    if (prefixIds.length > 0) {
      const prefixResults = await db
        .select()
        .from(threadPrefixes)
        .where(inArray(threadPrefixes.id, prefixIds));
      prefixResults.forEach((prefix: ThreadPrefixType) => {
        prefixesMap[prefix.id] = prefix;
      });
    }
  ...
  async updateThreadSolvedStatus({
    ...
    try {
      if (solvingPostId !== null && solvingPostId !== undefined) {
        ...
        if (!postCheck)
          throw new Error(
            "Invalid solving post ID or post does not belong to the thread."
          );
      }
