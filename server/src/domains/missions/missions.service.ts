...
export class MissionsService {
  ...
  async getAllMissions(): Promise<Mission[]> {
    try {
      ...
      logger.error('Error getting all missions:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  async getActiveMissions(userLevel?: number): Promise<Mission[]> {
    try {
      ...
      logger.error('Error getting active missions:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  async getMission(id: number): Promise<Mission | null> {
    try {
      ...
      logger.error('Error getting mission:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  async createMission(data: InsertMission): Promise<Mission> {
    try {
      ...
      const result = await db.insert(missions)
        .values({
          ...data,
          expiresAt
        })
      ...
      logger.error('Error creating mission:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  async updateMission(id: number, data: Partial<Mission>): Promise<Mission | null> {
    try {
      ...
      logger.error('Error updating mission:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
   ...
   */
  async getUserMissionProgress(userId: number): Promise<(UserMissionProgress & { mission: Mission })[]> {
    try {
      ...
      logger.error('Error getting user mission progress:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
   ...
   */
  private async initializeMissionsForUser(userId: number, missionsToInit: Mission[]): Promise<void> {
    try {
      ...
      logger.error('Error initializing missions for user:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  async resetDailyMissions(): Promise<void> {
    try {
      ...
      const expiredMissions = await db.select()
        .from(missions)
        .where(and(
      ...
      await db.update(userMissionProgress)
        ...
        .where(
          inArray(missions.id, expiredMissions.map((m: Mission) => m.id))
        );
    } catch (error: any) {
      logger.error('Error resetting daily missions:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
  ...
  async resetWeeklyMissions(): Promise<void> {
    try {
      ...
      // Update expired missions with new expiration date
      const updateWeeklyMissionsQuery = db.update(missions)
        .set({ expiresAt: nextWeek })
        .where(
          eq(missions.isWeekly, true)
        );
      logger.info('MISSION_RESET', `Update Weekly Missions Query: ${updateWeeklyMissionsQuery.getQuery()}`);
      ...
      logger.error('Error resetting weekly missions:', error instanceof Error ? error.message : String(error));
      throw error;
    }
  }
   ...
   */
  async updateMissionProgress({ userId, actionType, metadata = {} }: MissionProgressUpdate): Promise<UserMissionProgress[]> {
    try {
      ...
      for (const mission of activeMissions) {
        ...
        if (!progress) {
          const freshProgress = await db.select()
            .from(userMissionProgress)
            .where(and(
              eq(userMissionProgress.userId, userId),
        ...
        const updated = await db.update(userMissionProgress)
          ...
          .where(and(
            eq(userMissionProgress.id, progress.id),
            eq(userMissionProgress.isCompleted, false)
          ))
          .returning();
   ...
   */
  async claimMissionReward(userId: number, missionId: number): Promise<{
    success: boolean;
    rewards?: {
      xp?: number;
      dgt?: number;
    ...
    try {
      ...
      const missionResult = await db.select({
        mission: missions,
        progress: userMissionProgress
      })
        .from(missions)
  ...
  async createDefaultMissions(): Promise<void> {
    ...
    for (const missionData of defaultMissions) {
      ...
      const existing = await db.select().from(missions)
        .where(and(
          eq(missions.title, missionData.title),
          eq(missions.type, missionData.type)
        ))
        .limit(1);
