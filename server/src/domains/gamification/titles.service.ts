...
import { validateTitleUnlock, getTitlePriority } from '@shared/utils/title-utils';
...
export class TitlesService {
	...
	async getTitleById(id: TitleId): Promise<Title | null> {
		...
		return title as Title || null;
	}

	async getUserTitles(userId: UserId): Promise<UserTitle[]> {
	...
	async grantTitle(userId: UserId, titleId: TitleId, reason?: string, grantedBy?: UserId): Promise<void> {
	...
	async revokeTitle(userId: UserId, titleId: TitleId, reason?: string, revokedBy?: UserId): Promise<void> {
		...
		try {
			...
			// Remove the title
			await db.delete(userTitles).where(and(
				eq(userTitles.userId, userId),
				eq(userTitles.titleId, titleId)
			));
			...
			await db
				...
				.where(and(
					eq(users.id, userId),
					eq(users.activeTitleId, titleId)
				));
	...
	async equipTitle(userId: UserId, titleId: TitleId): Promise<void> {
	...
	async unequipTitle(userId: UserId): Promise<void> {
	...
	async checkAndGrantLevelTitles(userId: UserId, newLevel: number): Promise<TitleId[]> {
		logger.info('TITLES', `Checking level titles for user ${userId} at level ${newLevel}`);

		const grantedTitles: TitleId[] = [];
	...
	async checkAndGrantRoleTitles(userId: UserId, roleId: string): Promise<TitleId[]> {
		...
		try {
			...
			for (const title of roleTitles) {
				await this.grantTitle(userId, title.id as TitleId, `Role assignment: ${roleId}`);
				grantedTitles.push(title.id as TitleId);
			}
	...
	async update(id: TitleId, data: UpdateTitleInput) {
		// Validate role exists if roleId is provided
		if (data.roleId) {
			const [role] = await db
				.select({ id: rolesTable.id })
