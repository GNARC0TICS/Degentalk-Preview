 ...
 */
export const checkFeatureAccessForUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const access = await featureGatesService.checkFeatureAccess(Number(userId), featureId);
    res.status(200).json(access);
  } catch (error) {
 ...
 */
export const getAllFeatureAccessForUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const access = await featureGatesService.checkAllFeatureAccess(Number(userId));
    res.status(200).json(access);
  } catch (error) {
