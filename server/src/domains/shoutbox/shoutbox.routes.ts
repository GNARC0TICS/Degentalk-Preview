  ...
  try {
    ...
    if (room.minXpRequired && room.minXpRequired > 0) {
      ...
      if (
        userInfo.length === 0 ||
        !userInfo[0].xp ||
        userInfo[0].xp < room.minXpRequired
      ) {
        return false;
      }
    }
  ...
  try {
    ...
    if (!isAdminOrMod && userId) {
      ...
      query = db
        ...
        .from(chatRooms)
        .where(
          and(eq(chatRooms.isDeleted, false), eq(chatRooms.isPrivate, false))
        )
        .orderBy(asc(chatRooms.order));
    }
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `[MODERATION] User ${moderatorId} deleted shoutbox message ${messageId}`
      );
...
router.get("/messages", async (req: Request, res: Response) => {
  try {
    ...
    if (!targetRoomId) {
      try {
        const defaultRoom = await db
          ...
          .where(
            and(
              eq(chatRooms.name, "degen-lounge"),
              eq(chatRooms.isDeleted, false)
            )
          )
          .limit(1);
    ...
    if (!targetRoomId) {
      ...
      return res
        .status(400)
        .json({ error: "Could not determine the target chat room." });
    }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (typeof isPinned !== "boolean") {
        return res
          .status(400)
          .json({ error: "isPinned must be a boolean value" });
      }
