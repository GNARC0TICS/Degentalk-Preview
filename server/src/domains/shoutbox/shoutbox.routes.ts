...
async function userHasRoomAccess(userId: number, roomId: number): Promise<boolean> {
  try {
    ...
    if (room.minXpRequired && room.minXpRequired > 0) {
      const userInfo = await db.select({ xp: users.xp })
        .from(users)
    ...
    if (room.minGroupIdRequired) {
      const userInfo = await db.select({ groupId: users.groupId })
        .from(users)
        .where(eq(users.id, userId))
        .limit(1);
...
router.get('/rooms', async (req: Request, res: Response) => {
  try {
    ...
    const roomsWithAccess = rooms.map(room => {
      const accessible = !room.isPrivate || 
                        isAdminOrMod || 
...
router.delete('/messages/:id', isAdminOrModerator, async (req: Request, res: Response) => {
  try {
    ...
    const result = await db.update(shoutboxMessages)
      .set({ 
        isDeleted: true,
        editedAt: new Date()
      })
...
router.get('/messages', async (req: Request, res: Response) => {
  try {
    ...
    if (roomId && currentUserId) {
      ...
      const roomInfo = await db.select({ isPrivate: chatRooms.isPrivate })
        .from(chatRooms)
        .where(eq(chatRooms.id, roomId))
        .limit(1);
    ...
    if (!targetRoomId) {
      try {
        const defaultRoom = await db.select({ id: chatRooms.id })
          .from(chatRooms)
...
router.patch('/messages/:id', isAdminOrModerator, async (req: Request, res: Response) => {
  try {
    ...
    // Log the moderation action
    const moderatorId = getUserId(req);
    console.log(`[MODERATION] User ${moderatorId} ${isPinned ? 'pinned' : 'unpinned'} shoutbox message ${messageId}`);
