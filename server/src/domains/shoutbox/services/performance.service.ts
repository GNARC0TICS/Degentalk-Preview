...
export class PerformanceService {
   ...
   */
  static async batchInsertMessages(messages: Array<{
    userId: number;
    roomId: number;
    content: string;
    type?: string;
    ...
    try {
      ...
      logger.error('PerformanceService', 'Batch insert failed', { error, messageCount: messages.length });
   ...
   */
  static async getActiveUsersInRoom(roomId: number): Promise<Array<{
    id: number;
    username: string;
    level: number;
    roles: string[];
    ...
    const activeUsers = await db
      ...
      .where(and(
        ...
        gte(shoutboxMessages.createdAt, recentActiveTime)
      ))
