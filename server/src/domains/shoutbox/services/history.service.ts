...
export class MessageHistoryService {
  ...
  static async getMessageHistory(options: MessageHistoryOptions): Promise<{
    ...
    try {
      ...
      const previousCursor = cursor && resultMessages.length > 0
        ? resultMessages[0].id
        : undefined;
    ...
    try {
      ...
      for (const config of configs) {
        ...
        if (messagesToDelete.length > 0) {
          ...
          await db.update(shoutboxMessages)
            .set({ 
              isDeleted: true,
              editedAt: new Date()
            })
            .where(and(
              inArray(shoutboxMessages.id, messagesToDelete.map(m => m.id))
            ));
  ...
  private static async exportAsText(
    ...
    // Write header
    const header = 'DEGENTALK SHOUTBOX EXPORT\n' +
                  '========================\n' +
