 ...
 */
import { users, levels, xpAdjustmentLogs, userRoles, roles as rolesTable, forumStructure } from '@schema';
import { eq, and, desc, gte, lt, asc, gt, count } from 'drizzle-orm';
import { db } from '@degentalk/db';
import { logger } from '@core/logger';
import { getXpAction } from './xp-actions';
import { xpActionLogs } from './xp-actions-schema';
// Import the centralized event handlers
import { handleXpAward, handleXpLoss } from './events/xp.events';
...
export class XpService {
     ...
     */
    async updateUserXp(userId, amount, adjustmentType = 'add', options = {}) {
        logger.info('Updating user XP', { userId, amount, adjustmentType, options });
        const { reason = 'System adjustment', adminId, logAdjustment = true, skipLevelCheck = false, skipTriggers = false } = options;
        ...
        try {
            ...
            if (logAdjustment && adminId) {
                await this.logXpAdjustment(userId, adminId, adjustmentType, amount, reason, result.oldXp, result.newXp);
            }
            return {
                userId,
                oldXp: result.oldXp,
     ...
     */
    async logXpAdjustment(userId, adminId, adjustmentType, amount, reason, oldXp, newXp) {
        try {
     ...
     * @returns Result of the XP update operation
     */
    async awardXpWithContext(userId, action, metadata, forumId) {
        try {
     ...
     */
    async checkActionLimits(userId, action) {
        try {
            const actionConfig = await getXpAction(action);
            if (!actionConfig || !actionConfig.enabled) {
            ...
            if (actionConfig.maxPerDay !== undefined) {
                ...
                const [dailyCountResult] = await db
                    .select({ count: count() })
                    .from(xpActionLogs)
                    .where(and(eq(xpActionLogs.userId, userId), eq(xpActionLogs.action, action), gte(xpActionLogs.createdAt, startOfDay), lt(xpActionLogs.createdAt, endOfDay)));
            ...
            if (actionConfig.cooldownSeconds !== undefined && actionConfig.cooldownSeconds > 0) {
                const [lastAction] = await db
                    ...
                    .limit(1);
                if (lastAction) {
                    const timeSinceLastAward = (now.getTime() - new Date(lastAction.createdAt).getTime()) / 1000;
     ...
     */
    async updateActionLimits(userId, action) {
     ...
     */
    async logXpAction(userId, action, amount, metadata) {
        try {
     ...
     */
    async getActionLimitsForUser(userId, action) {
        try {
            const actionConfig = await getXpAction(action);
            if (!actionConfig || !actionConfig.enabled) {
     ...
     */
    async getUserRoleMultiplier(userId) {
        try {
     ...
     */
    async getForumMultiplier(forumId) {
        try {
