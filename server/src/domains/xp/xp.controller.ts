...
// import { xpAdjustmentLog } from '@shared/schema'; // Removed as unused
...
export const awardXpForAction = async (req: Request, res: Response, next: NextFunction) => {
  ...
  } catch (error: any) { // Keep error as any for now, specific handling below
    logger.error('Error awarding XP for action:', error instanceof Error ? error.message : String(error));
    if (error.message && error.message.includes('not found')) { // This check might need adjustment if error is not Error instance
...
export const getUserXpInfo = async (req: Request, res: Response, next: NextFunction) => {
  ...
  } catch (error: any) { // Keep error as any for now
    logger.error('Error getting user XP info:', error instanceof Error ? error.message : String(error));
    if (error.message && error.message.includes('not found')) { // This check might need adjustment
      return res.status(404).json({ message: error.message });
    }
    next(error);
  }
...
export const getUserXpLogs = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (period) {
      ...
      if (periodStartDate !== null) { // Explicitly check for non-null
    ...
    logger.error('Error getting user XP logs:', error instanceof Error ? error.message : String(error));
    next(error);
  }
};
...
export const awardActionXp = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId || !action ) { // entityId can be optional for some actions
    ...
    if (!setting || !setting.enabled) {
      ...
      return res.status(200).json({ xpAwarded: 0, message: 'XP action not configured or disabled.' });
    }

    const xpToAward = setting.baseValue;
    if (xpToAward <= 0) {
        return res.status(200).json({ xpAwarded: 0, message: 'XP amount for action is zero or negative.' });
    }
