 ...
 */
export const DEFAULT_XP_ACTIONS = {
 ...
 */
export async function loadXpActionsFromDb(forceRefresh = false) {
    const now = Date.now();
    ...
    try {
        ...
        logger.error('XP_ACTIONS', 'Error loading XP actions from database:', error instanceof Error ? error.message : String(error));
        return DEFAULT_XP_ACTIONS;
    }
}
/**
 ...
 */
export async function getXpActions() {
    return loadXpActionsFromDb();
}
/**
 * Get an XP action configuration
 */
export async function getXpAction(key) {
 ...
 */
export async function getXpValueForAction(key) {
...
export function getXpActionSync(key) {
...
export function getXpValueForActionSync(key) {
