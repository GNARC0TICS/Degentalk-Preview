...
import { db } from '../../../../db';
import { users, transactions, tips } from '@shared/schema';
...
import { pgTable, serial, integer, text, timestamp, jsonb, varchar, decimal } from 'drizzle-orm/pg-core';
...
export class TipService {
  ...
  async sendTip(request: TipRequest): Promise<TipResponse> {
    ...
    try {
      ...
      const [tipRecord] = await db.insert(tipRecords)
        .values({
          fromUserId,
          toUserId,
          amount,
  ...
  private async validateTipSettings(
    ...
    if (settings.tipCooldownSeconds > 0) {
      ...
      const [recentTip] = await db.select()
        .from(tipRecords)
        .where(sql`
          from_user_id = ${fromUserId} 
