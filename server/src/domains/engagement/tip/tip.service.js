...
import { pgTable, uuid, text, timestamp, jsonb, varchar, decimal, serial, integer } from 'drizzle-orm/pg-core';
...
export class TipService {
    ...
    async sendTip(request) {
        ...
        try {
            ...
            if (currency === 'DGT') {
                ...
                throw new WalletError('Cryptocurrency tipping not supported yet', ErrorCodes.OPERATION_NOT_ALLOWED);
            }
            ...
            throw new WalletError(`Failed to send tip: ${errorMessage}`, ErrorCodes.WALLET_TRANSACTION_FAILED);
        }
    }
    /**
     * Validate tip settings, including limits and cooldowns
     */
    async validateTipSettings(fromUserId, toUserId, amount, currency, source) {
        ...
        if (currency === 'DGT' &&
            ...
            throw new WalletError(`Minimum DGT tip amount is ${settings.minTipAmountDGT}`, ErrorCodes.BAD_REQUEST);
        }
        // Check maximum tip amount
        if (currency === 'DGT' &&
            settings.maxTipAmountDGT &&
            amount > Number(settings.maxTipAmountDGT)) {
            throw new WalletError(`Maximum DGT tip amount is ${settings.maxTipAmountDGT}`, ErrorCodes.BAD_REQUEST);
        }
        // Check daily limit
      ...
      `);
            ...
            if (totalSentToday + amount > Number(settings.dailyTipLimitDGT)) {
                throw new WalletError(`Daily DGT tip limit of ${settings.dailyTipLimitDGT} would be exceeded`, ErrorCodes.RATE_LIMITED);
            }
        }
        ...
        if (settings.tipCooldownSeconds > 0) {
            ...
            const [recentTip] = await db
                .select()
                .from(tipRecords)
                .where(and(eq(tipRecords.fromUserId, fromUserId), eq(tipRecords.toUserId, toUserId), sql `${tipRecords.createdAt} >= ${cooldownTime.toISOString()}`))
                .limit(1);
            if (recentTip) {
                ...
                throw new WalletError(`Tip cooldown in effect. You can tip this user again at ${nextAllowedTime.toISOString()}`, ErrorCodes.RATE_LIMITED, { nextAllowedTime });
            }
        }
    }
    /**
     ...
     */
    async getTipHistory(userId, type = 'both', limit = 20, offset = 0) {
        try {
            let whereClause;
            if (type === 'sent') {
