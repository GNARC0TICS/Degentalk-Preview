...
export class RainService {
  ...
  async processRain(
    ...
    try {
      ...
      const [rainTransaction] = await db.insert(transactions).values({
        userId: senderUserId,
      ...
      if (currency === 'DGT') {
        await this.processDGTRain(senderUserId, amount, perUserAmount, recipients, rainTransaction.id);
      } else {
      ...
      logger.info('RainService', `Rain completed: User ${senderUserId} -> ${recipientCount} users`, {
        amount,
        currency,
        transactionId: rainTransaction.id
      });
   ...
   */
  private async getRandomActiveUsers(
    ...
    const onlineUserResults = await db.select({
  ...
  private async checkCooldowns(userId: number, commandType: 'tip' | 'rain'): Promise<void> {
    try {
      ...
      if ((isAdmin && cooldownSetting.adminBypassCooldown) || 
      ...
      const cooldownSeconds = commandType === 'tip' 
  ...
  async updateRainSettings(userId: number, settings: any) {
    try {
      ...
      if (existingSettings.length > 0) {
        ...
        const [updated] = await db.update(rainSettings)
          .set({
            ...settings,
            updatedAt: new Date()
          })
        ...
        const [created] = await db.insert(rainSettings)
          .values({
