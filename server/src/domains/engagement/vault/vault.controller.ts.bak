...
export class VaultController {
  ...
  lockTokens = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (error instanceof WalletError) {
        return res.status(error.httpStatus).json({ // Changed statusCode to httpStatus
          error: error.message,
          code: error.code,
      ...
      logger.error('VaultController', `Error locking tokens: ${error instanceof Error ? error.message : String(error)}`); // Handle unknown error
  ...
  unlockTokens = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error('VaultController', `Error unlocking tokens: ${error instanceof Error ? error.message : String(error)}`); // Handle unknown error
      return res.status(500).json({
  ...
  getUserVaultLocks = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error('VaultController', `Error getting user vault locks: ${error instanceof Error ? error.message : String(error)}`); // Handle unknown error
      return res.status(500).json({
  ...
  processAutomaticUnlocks = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error('VaultController', `Error processing automatic unlocks: ${error instanceof Error ? error.message : String(error)}`); // Handle unknown error
      return res.status(500).json({
