...
export class CloutService {
  ...
  async checkAchievements(
    ...
    const available = await this.drizzle
      ...
      .where(and(eq(cloutAchievements.enabled, true), eq(cloutAchievements.criteriaType, triggerType)));
    ...
    if (newlyAwarded.length > 0) {
      logger.info('CloutService', `Awarded achievements ${newlyAwarded.join(', ')} to user ${userId}`);
    }
