...
export class MessageTransformer {
	...
	static toAdminMessage(dbMessage: any, analytics?: any): AdminMessage {
		...
		const authenticatedData = this.toAuthenticatedMessage(dbMessage, {
			role: 'admin',
			id: 'admin'
		});
	 ...
	 */
	static toAuthenticatedConversation(
		dbConversation: any,
		requestingUser: any,
		participants?: any[]
	): AuthenticatedConversation {
	...
	static toAdminConversation(dbConversation: any, analytics?: any): AdminConversation {
		...
		const authenticatedData = this.toAuthenticatedConversation(dbConversation, {
			role: 'admin',
			id: 'admin'
		});
	...
	static toMessageSearchResult(searchResult: any, requestingUser: any): MessageSearchResult {
		return {
			message: this.toAuthenticatedMessage(
	...
	private static calculateMessageSize(dbMessage: any): number {
		...
		const attachmentSize =
			dbMessage.attachments?.reduce((sum: number, att: any) => sum + (att.size || 0), 0) || 0;
