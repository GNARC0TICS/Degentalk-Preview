...
import { dictionaryEntries, DictionaryEntry, dictionaryUpvotes, insertDictionaryEntrySchema } from '@schema';
...
export class DictionaryService {
    ...
    static async create(data: unknown & { authorId: string }) {
        ...
        // Award XP to author
        await xpLevelService.awardXp(data.authorId as unknown as number, XP_ACTIONS.DICTIONARY_ENTRY_SUBMITTED);
    ...
    static async moderate(entryId: number, status: 'approved' | 'rejected', approverId: string) {
        ...
        if (status === 'approved') {
            ...
            if (updated) {
                await Promise.all([
                    xpLevelService.awardXp(updated.authorId as unknown as number, XP_ACTIONS.DICTIONARY_ENTRY_APPROVED),
                    xpLevelService.awardXp(approverId as unknown as number, XP_ACTIONS.DICTIONARY_ENTRY_APPROVAL)
                ]);
            }
        }
        return updated;
    ...
    static async toggleUpvote(entryId: number, userId: string) {
        ...
        await db.transaction(async (tx) => {
            if (existing.length) {
                ...
                await tx
                    ...
                    .where(eq(dictionaryUpvotes.id, existing[0].id));
                await tx
                    .update(dictionaryEntries)
        ...
        if (upvoted) {
            const [entry] = await db
                ...
                .limit(1);
            if (entry) {
                await xpLevelService.awardXp(entry.authorId as unknown as number, XP_ACTIONS.DICTIONARY_ENTRY_UPVOTED);
            }
        }
