 ...
 */
router.post('/shop/purchase', validateRequest(purchaseItemSchema), async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
 ...
 */
router.post('/shop/activate', validateRequest(activateItemSchema), async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
