...
export const EmojiBaseSchema = z.object({
    ...
    unlockType: z.enum(['free', 'purchase', 'path_unlock', 'admin_grant'])
        .optional()
        .default('free'),
    unlock_type: z.enum(['free', 'purchase', 'path_unlock', 'admin_grant'])
        .optional(), // snake_case support
    ...
    requiredPath: z.string()
        .max(50, 'Required path cannot exceed 50 characters')
        .optional(),

    requiredPathXP: z.number()
        .min(0, 'Required path XP must be non-negative')
        .optional(),
    required_path_xp: z.number()
        .min(0, 'Required path XP must be non-negative')
        .optional(), // snake_case support
    ...
    cloutValue: z.number()
        .min(0, 'Clout value must be non-negative')
        .optional()
        .default(0)
});
...
export const CreateEmojiSchema = EmojiBaseSchema.omit({}).refine(
    ...
    {
        message: 'Purchase unlock type requires priceDgt, path unlock type requires requiredPath and requiredPathXP'
    }
);
...
export const BulkDeleteEmojisSchema = z.object({
    ids: z.array(z.number().positive()).min(1, 'At least one emoji ID is required').max(50, 'Cannot delete more than 50 emojis at once')
});

export const BulkUpdateEmojisSchema = z.object({
    ids: z.array(z.number().positive()).min(1, 'At least one emoji ID is required').max(50, 'Cannot update more than 50 emojis at once'),
