...
export const emojiService = {
    ...
    async create(rawEmojiData: any) {
        try {
            ...
            logger.info('EMOJI_SERVICE', 'Creating emoji:', { name: emojiData.name, type: emojiData.type });
            ...
            if (existing.length > 0) {
                ...
                throw new Error(`Emoji with this ${conflictType} already exists: ${conflict[conflictType]}`);
            }
            ...
            logger.info('EMOJI_SERVICE', `Successfully created emoji: ${newEmoji.name} (ID: ${newEmoji.id})`);
        ...
        try {
            ...
            if (emojiData.name || emojiData.code) {
                ...
                if (conflicting.length > 0) {
                    ...
                    const conflictType = conflict.name === emojiData.name ? 'name' : 'code';
                    throw new Error(`Another emoji already uses this ${conflictType}: ${conflict[conflictType]}`);
                }
            }
