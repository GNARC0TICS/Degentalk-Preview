...
export class AdminAnalyticsController {
  async getOverviewStats(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid query parameters for overview stats', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
  ...
  async getUserGrowthChart(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid query parameters for user growth chart', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
  ...
  async getMostActiveThreads(req: Request, res: Response) {
    try {
      const validation = AnalyticsQuerySchema.safeParse(req.query);
      if (!validation.success) {
        throw new AdminError('Invalid query parameters for most active threads', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
