...
import { db } from '../../../../../../db';
import { rainEvents, users, transactions } from '@shared/schema';
...
class RainAnalyticsService {
  ...
  async getRainAnalytics(days = 30, topRainersLimit = 10): Promise<RainEventAnalytics> {
    ...
    // Get top rainers
    const topRainers = await db.select({
    ...
    const averageRainAmount = summaryResult[0].totalRainVolume && summaryResult[0].totalRainEvents 
   ...
   */
  private fillMissingDates(data: { date: string; amount: any; eventCount: any }[], days: number): { date: string; amount: number; eventCount: number }[] {
