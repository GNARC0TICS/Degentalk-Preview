...
export class AdminAnalyticsService {
  async getOverviewStats(params: AnalyticsPeriodInput) {
    ...
    try {
      ...
      const [newUsers] = await db.select({ value: count(users.id) })
        .from(users)
        .where(between(users.createdAt, startDateSQL, endDateSQL));
      
      const [activeUsers] = await db.select({ value: count(users.id) })
        .from(users)
      ...
      const [newThreads] = await db.select({ value: count(threads.id) })
        .from(threads)
        .where(between(threads.createdAt, startDateSQL, endDateSQL));

      const [totalPosts] = await db.select({ value: count(posts.id) }).from(posts);
      const [newPosts] = await db.select({ value: count(posts.id) })
        .from(posts)
        .where(between(posts.createdAt, startDateSQL, endDateSQL));
      
      const [totalReactions] = await db.select({ value: count(postReactions.userId) }).from(postReactions);
      
      const [totalTipAmount] = await db.select({ value: sum(transactions.amount) })
        .from(transactions)
        .where(eq(transactions.type, 'TIP'));
        
      const [shoutboxMessageCount] = await db.select({ value: count(shoutboxMessages.id) }).from(shoutboxMessages);
      ...
      return {
        ...
        period: { startDate: startDateSQL, endDate: endDateSQL, periodUsed: params.period || 'custom' }
      };
    } catch (error: any) {
      console.error("Error fetching overview stats:", error);
      throw new AdminError('Failed to fetch overview statistics', 500, AdminErrorCodes.DB_ERROR, { originalError: error.message });
    }
  }
  ...
  async getUserGrowthChart(params: AnalyticsQueryInput) {
    ...
    switch (granularity) {
      ...
      case 'weekly': dateFormat = '%Y-%W'; break; // Year-WeekNumber
    ...
    try {
      ...
      throw new AdminError('Failed to fetch user growth data', 500, AdminErrorCodes.DB_ERROR, { originalError: error.message });
    }
  }
  ...
  async getMostActiveThreads(params: AnalyticsQueryInput) {
    ...
    try {
        ...
        throw new AdminError('Failed to fetch most active threads', 500, AdminErrorCodes.DB_ERROR, { originalError: error.message });
    }
  }
}
