...
export class UiConfigService implements IUiConfigService {
    ...
    async getQuotes(
        ...
        if (filters?.searchTerm) {
            ...
            conditions.push(
                or(
                    like(uiQuotes.headline, searchTerm),
                    like(uiQuotes.subheader, searchTerm)
                )
            );
        }
        ...
        switch (sortBy) {
            case 'displayOrder':
                orderByClause = sortOrder === 'desc' ? desc(uiQuotes.displayOrder) : asc(uiQuotes.displayOrder);
                break;
            case 'createdAt':
    ...
    async updateCollection(data: UpdateCollectionRequest, userId: string): Promise<UiCollection> {
        ...
        if (quoteIds) {
            await db.transaction(async (tx: any) => {
                ...
                await tx
                    .delete(uiCollectionQuotes)
                    .where(eq(uiCollectionQuotes.collectionId, id));
    ...
    async getQuoteAnalytics(request: QuoteAnalyticsRequest): Promise<QuoteAnalyticsResponse> {
        ...
        return {
            ...
            topPositions: topPositions.map((p: any) => ({ position: p.position || 'unknown', count: p.count })),
            dateRange: {
                ...
                try {
                    ...
                    if (!data.overwriteExisting) {
                        const [existing] = await tx
                            ...
                            .where(
                                and(
                                    eq(uiQuotes.headline, quoteData.headline),
                                    eq(uiQuotes.type, quoteData.type)
                                )
                            )
                            .limit(1);
    ...
    async exportQuotes(request: ExportQuotesRequest): Promise<Buffer | string> {
        ...
        if (format === 'csv') {
            ...
            const headers = [
                'id', 'type', 'headline', 'subheader', 'tags', 'intensity',
                'theme', 'isActive', 'impressions', 'clicks', 'createdAt'
            ];
