...
export class AdminForumController {
  ...
  async getCategoryById(req: Request, res: Response) {
    try {
      ...
      if (isNaN(categoryId)) throw new AdminError('Invalid category ID', 400, AdminErrorCodes.INVALID_REQUEST);
  ...
  async createCategory(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid category data', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      
      const category = await adminForumService.createCategory(validation.data);
      await adminController.logAction(req, 'CREATE_CATEGORY', 'category', category.id.toString(), validation.data);
  ...
  async updateCategory(req: Request, res: Response) {
    try {
      ...
      await adminController.logAction(req, 'UPDATE_CATEGORY', 'category', categoryId.toString(), validation.data);
      res.json(category);
    } catch (error) {
      if (error instanceof AdminError) return res.status(error.httpStatus).json({ error: error.message, code: error.code, details: error.details });
  ...
  async deleteCategory(req: Request, res: Response) {
    try {
      ...
      await adminController.logAction(req, 'DELETE_CATEGORY', 'category', categoryId.toString(), {});
      res.json(result);
    } catch (error) {
      if (error instanceof AdminError) return res.status(error.httpStatus).json({ error: error.message, code: error.code, details: error.details });
  ...
  async createPrefix(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid prefix data', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      
      const prefix = await adminForumService.createPrefix(validation.data);
      await adminController.logAction(req, 'CREATE_THREAD_PREFIX', 'thread_prefix', prefix.id.toString(), validation.data);
  ...
  async moderateThread(req: Request, res: Response) {
    try {
      ...
      if (isNaN(threadId)) throw new AdminError('Invalid thread ID', 400, AdminErrorCodes.INVALID_REQUEST);
      
      const validation = ModerateThreadSchema.safeParse(req.body);
      if (!validation.success) {
        throw new AdminError('Invalid moderation data', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      ...
      await adminController.logAction(req, actionType, 'thread', threadId.toString(), validation.data);
      res.json(thread);
    } catch (error) {
      if (error instanceof AdminError) return res.status(error.httpStatus).json({ error: error.message, code: error.code, details: error.details });
