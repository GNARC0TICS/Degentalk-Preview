...
class AvatarFrameService {
  ...
  async createFrame(data: CreateFrameData) {
    try {
      const [frame] = await db.insert(avatarFrames).values({
        name: data.name,
  ...
  async updateFrame(id: number, data: UpdateFrameData) {
    try {
      const [frame] = await db.update(avatarFrames)
        .set(data)
  ...
  async deleteFrame(id: number) {
    try {
      ...
      const usersWithFrame = await db.select({ id: users.id })
        .from(users)
      ...
      const [deletedFrame] = await db.delete(avatarFrames)
        .where(eq(avatarFrames.id, id))
        .returning();
  ...
  async getFrameUsageCount(id: number) {
    try {
      const [result] = await db.select({ count: eq(users.activeFrameId, id) })
        .from(users)
        .where(eq(users.activeFrameId, id));
