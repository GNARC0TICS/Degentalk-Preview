...
export class AdminUserGroupsService {
	async getAllGroupsWithCounts() {
		const allGroups = await db
			...
			.from(userGroups)
			.orderBy(
				desc(userGroups.isAdmin),
				desc(userGroups.isModerator),
				desc(userGroups.isStaff),
				userGroups.id
			);
		...
		userCountsResult.forEach((row) => {
			if (row.groupId !== null) {
				// Ensure groupId is not null before setting
	...
	async updateGroup(groupId: GroupId, data: UserGroupInput) {
		...
		if (data.name && data.name !== existingGroup.name) {
			const [nameConflict] = await db
				.select()
				.from(userGroups)
				.where(and(eq(userGroups.name, data.name), ne(userGroups.id, groupId)));
			if (nameConflict) {
				throw new AdminError(
	...
	async deleteGroup(groupId: GroupId) {
		...
		if (existingGroup.isDefault) {
			throw new AdminError(
				'Cannot delete the default user group. Set another group as default first.',
				400,
		...
		if (usersInGroupCount > 0) {
			...
			if (!defaultGroup) {
				throw new AdminError(
					'Cannot delete group with users if no other default group exists to reassign them.',
					400,
					AdminErrorCodes.OPERATION_FAILED
				);
			}
		...
		return {
			...
			reassignedUsers: usersInGroupCount > 0
		};
	}
	...
	async getUsersInGroup(groupId: GroupId, params: ListGroupUsersQueryInput) {
		...
		const usersInGroupList = await db
			.select({
				id: users.id,
