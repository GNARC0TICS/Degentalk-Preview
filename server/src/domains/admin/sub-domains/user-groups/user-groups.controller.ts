...
export class AdminUserGroupsController {
  ...
  async getGroupById(req: Request, res: Response) {
    try {
      ...
      if (isNaN(groupId)) throw new AdminError('Invalid group ID', 400, AdminErrorCodes.INVALID_REQUEST);
  ...
  async createGroup(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid user group data', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      const newGroup = await adminUserGroupsService.createGroup(validation.data);
      await adminController.logAction(req, 'CREATE_USER_GROUP', 'user_group', newGroup.id.toString(), validation.data);
  ...
  async updateGroup(req: Request, res: Response) {
    try {
      ...
      const validation = UserGroupSchema.safeParse(req.body);
      if (!validation.success) {
        throw new AdminError('Invalid user group data for update', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      const updatedGroup = await adminUserGroupsService.updateGroup(groupId, validation.data);
      await adminController.logAction(req, 'UPDATE_USER_GROUP', 'user_group', groupId.toString(), validation.data);
  ...
  async deleteGroup(req: Request, res: Response) {
    try {
      ...
      await adminController.logAction(req, 'DELETE_USER_GROUP', 'user_group', groupId.toString(), { reassignedUsers: result.reassignedUsers });
      res.json(result);
    } catch (error) {
      if (error instanceof AdminError) return res.status(error.httpStatus).json({ error: error.message, code: error.code, details: error.details });
  ...
  async getUsersInGroup(req: Request, res: Response) {
    try {
      ...
      if (!queryValidation.success) {
        throw new AdminError('Invalid pagination parameters', 400, AdminErrorCodes.VALIDATION_ERROR, queryValidation.error.format());
      }
