...
export const executeAirdrop = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!validationResult.success) {
      logger.warn('AIRDROP_CONTROLLER', 'Invalid airdrop request payload', { errors: validationResult.error.format() });
      return res.status(400).json({ message: 'Invalid request payload.', errors: validationResult.error.format() });
    }
    ...
    if (result.success) {
      ...
      res.status(result.message.includes('Unsupported') || result.message.includes('positive') ? 400 : 500 ).json(result);
    }
  } catch (error) {
