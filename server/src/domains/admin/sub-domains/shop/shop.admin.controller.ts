...
export const shopAdminController = {
  ...
  async createProduct(req, res) {
    const { name, description, priceDGT, priceUSDT, category, rarity, imageUrl, pluginReward, stockLimit, pointsPrice, effectType } = req.body;
    ...
    try {
      const newProduct = await db.insert(products).values({
        ...
        pluginReward: pluginReward || { type: effectType, rarity: rarity, imageUrl: imageUrl, priceUSDT: priceUSDT }, // Consolidate extra fields here
  ...
  async deleteProduct(req, res) {
    ...
    try {
      ...
      res.status(200).json({ message: 'Product archived successfully', product: deletedProduct[0] });
    } catch (error) {
