...
export class AdminSettingsController {
  ...
  async updateSetting(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid setting data', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      
      const setting = await adminSettingsService.updateSetting(validation.data);
      await adminController.logAction(req, 'UPDATE_SETTING', 'setting', validation.data.key, validation.data);
      res.json(setting);
    } catch (error) {
      if (error instanceof AdminError) return res.status(error.httpStatus).json({ error: error.message, code: error.code, details: error.details });
  ...
  async updateSettings(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid settings data', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
  ...
  async createSettingGroup(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid setting group data', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      
      const group = await adminSettingsService.createSettingGroup(validation.data);
      await adminController.logAction(req, 'CREATE_SETTING_GROUP', 'setting_group', group.key, validation.data);
  ...
  async createSetting(req: Request, res: Response) {
    try {
      ...
      await adminController.logAction(req, 'CREATE_SETTING', 'setting', setting.key, validation.data);
  ...
  async updateSettingMetadata(req: Request, res: Response) {
    try {
      ...
      if (!validation.success) {
        throw new AdminError('Invalid setting metadata', 400, AdminErrorCodes.VALIDATION_ERROR, validation.error.format());
      }
      
      const setting = await adminSettingsService.updateSettingMetadata(key, validation.data);
      await adminController.logAction(req, 'UPDATE_SETTING_METADATA', 'setting', key, validation.data);
      res.json(setting);
    } catch (error) {
      if (error instanceof AdminError) return res.status(error.httpStatus).json({ error: error.message, code: error.code, details: error.details });
