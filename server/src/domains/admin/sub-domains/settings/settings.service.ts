...
export class AdminSettingsService {
  ...
  async updateSettingGroup(key: string, data: SettingGroupInput) {
    try {
      ...
      const [settingCount] = await db.select({
        count: sql`count(*)`
      }).from(siteSettings).where(eq(siteSettings.group, key));
  ...
  async createSetting(data: CreateSettingInput) {
    try {
      ...
      const [existingSetting] = await db.select({key: siteSettings.key}).from(siteSettings).where(eq(siteSettings.key, data.key));
      
      if (existingSetting) {
        throw new AdminError(`Setting with key "${data.key}" already exists`, 400, AdminErrorCodes.DUPLICATE_ENTRY);
      }
      
      // If group is provided, verify it exists
      if (data.group) {
        const [group] = await db.select({key: siteSettings.group}).from(siteSettings).where(eq(siteSettings.group, data.group));
        
        if (!group) {
          throw new AdminError(`Group with key "${data.group}" not found`, 400, AdminErrorCodes.NOT_FOUND);
        }
      }
      ...
      const [newSetting] = await db.insert(siteSettings)
        .values({
          ...
          description: data.description,
          type: data.type,
          defaultValue: typeof data.defaultValue === 'object' ? JSON.stringify(data.defaultValue) : data.defaultValue !== undefined ? String(data.defaultValue) : null,
          value: typeof value === 'object' ? JSON.stringify(value) : value !== undefined ? String(value) : null,
          group: data.group,
    ...
    try {
      ...
      const [updatedSetting] = await db.update(siteSettings)
        .set({
          ...
          validationRules: data.validationRules !== undefined ? JSON.stringify(data.validationRules) : existingSetting.validationRules,
          options: data.options !== undefined ? JSON.stringify(data.options) : existingSetting.options,
    ...
    switch (type) {
      case 'string':
        if (typeof value !== 'string') {
          throw new AdminError(`Value must be a string for setting type "string"`, 400, AdminErrorCodes.VALIDATION_ERROR);
        }
        break;
      case 'number':
        if (typeof value !== 'number' && (typeof value !== 'string' || isNaN(Number(value)))) {
          throw new AdminError(`Value must be a number for setting type "number"`, 400, AdminErrorCodes.VALIDATION_ERROR);
        }
        break;
      case 'boolean':
        if (typeof value !== 'boolean' && value !== 'true' && value !== 'false') {
          throw new AdminError(`Value must be a boolean for setting type "boolean"`, 400, AdminErrorCodes.VALIDATION_ERROR);
        }
        break;
      case 'json':
        try {
          ...
          throw new AdminError(`Value must be valid JSON for setting type "json"`, 400, AdminErrorCodes.VALIDATION_ERROR);
        }
        break;
      case 'array':
        if (!Array.isArray(value) && (typeof value !== 'string' || !value.startsWith('[') || !value.endsWith(']'))) {
          throw new AdminError(`Value must be an array for setting type "array"`, 400, AdminErrorCodes.VALIDATION_ERROR);
        }
        break;
    }
    return true;
