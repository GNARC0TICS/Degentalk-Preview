...
export const userInventoryAdminController = {
  ...
  async grantItem(req, res) {
    ...
    try {
      ...
      const existingItem = await db.select().from(userInventory)
        .where(and(eq(userInventory.userId, parseInt(userId)), eq(userInventory.productId, productId)))
        .limit(1);

      if (existingItem.length > 0) {
        // Optionally, update quantity if item is stackable, or just return info
        return res.status(409).json({ message: 'User already owns this item.', item: existingItem[0] });
      }

      const newItem = await db.insert(userInventory).values({
  ...
  async equipItem(req, res) {
    ...
    try {
      await db.transaction(async (tx) => {
        const itemToEquipResult = await tx.select({
          id: userInventory.id,
        ...
        if (currentItemType) {
          ...
          const allEquippedItems = await tx.select({
          ...
          for (const equippedItem of allEquippedItems) {
            try {
              ...
              console.error("Failed to parse pluginReward for an equipped item:", equippedItem.inventoryId, e);
        ...
        const updateResult = await tx.update(userInventory)
      ...
      const finalEquippedItem = await db.select()
        .from(userInventory)
        .where(eq(userInventory.id, parseInt(inventoryItemId)))
        .limit(1);
  ...
  async unequipItem(req, res) {
    const { userId, inventoryItemId } = req.params;
    try {
      const unequippedItem = await db.update(userInventory)
        .set({ equipped: false, updatedAt: new Date() })
        .where(and(eq(userInventory.id, parseInt(inventoryItemId)), eq(userInventory.userId, parseInt(userId))))
        .returning();
