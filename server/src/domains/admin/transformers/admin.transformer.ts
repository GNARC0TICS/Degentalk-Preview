...
export class AdminTransformer {
  ...
  private static calculateActionSeverity(action: string, entityType: string): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private static sanitizeUserAgent(userAgent: string): string {
    ...
    return userAgent.replace(/\([^)]*\)/g, '').trim().substring(0, 100);
  }
