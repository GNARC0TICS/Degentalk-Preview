...
function transformUserData(data: any, requestingUser?: any, targetUser?: any): any {
  ...
  if (typeof data === 'object' && data !== null) {
    ...
    if (transformed.creator) {
      transformed.creator = transformSingleUserData(transformed.creator, requestingUser, targetUser);
    }
...
function transformSingleUserData(userData: any, requestingUser?: any, targetUser?: any): any {
  ...
  const isAdmin = requestingUser && (requestingUser.role === 'admin' || requestingUser.role === 'owner');
 ...
 */
export const dataAccessAuditMiddleware = (req: PrivacyRequest, res: Response, next: NextFunction) => {
  const originalJson = res.json;
