...
class TypedEventEmitter extends EventEmitter {
  // Type-safe emit with validation, fallback for compatibility
  emit<K extends keyof DomainEventMap>(event: K, payload: DomainEventMap[K]): boolean;
  emit(event: string | symbol, ...args: any[]): boolean;
  emit(event: string | symbol, payload?: any, ...args: any[]): boolean {
    ...
    if (typeof event === 'string' && event in DomainEvents && payload !== undefined) {
      try {
        const schema = DomainEvents[event as keyof typeof DomainEvents];
