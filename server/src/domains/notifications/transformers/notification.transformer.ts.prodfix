...
export class NotificationTransformer {
   ...
   */
  static toAuthenticatedNotification(dbNotification: any, requestingUser: any): AuthenticatedNotification {
  ...
  static toNotificationSummary(
    ...
    return {
      ...
      highPriority: notifications.filter(n => 
        this.calculatePriority(n.eventType) === 'high'
      ).length,
      requiresAction: notifications.filter(n => 
        this.requiresUserAction(n)
      ).length
    };
  }
  ...
  static toAuthenticatedNotificationSummary(
    ...
    return {
      ...
      preferences: {
        ...
        categories: Object.keys(preferences?.categories || {})
          .filter(key => preferences.categories[key])
      }
    };
  }
  ...
  private static categorizeNotification(eventType: string): 'system' | 'social' | 'activity' | 'transaction' {
    if (eventType.includes('transaction') || eventType.includes('tip') || 
        eventType.includes('rain') || eventType.includes('airdrop')) {
      return 'transaction';
    }
    
    if (eventType.includes('reply') || eventType.includes('mention') || 
        eventType.includes('friend') || eventType.includes('follow')) {
      return 'social';
    }
    
    if (eventType.includes('level') || eventType.includes('badge') || 
        eventType.includes('mission') || eventType.includes('achievement')) {
      return 'activity';
    }
  ...
  private static categorizeNotifications(notifications: any[]) {
    return {
      system: notifications.filter(n => this.categorizeNotification(n.eventType) === 'system').length,
      social: notifications.filter(n => this.categorizeNotification(n.eventType) === 'social').length,
      activity: notifications.filter(n => this.categorizeNotification(n.eventType) === 'activity').length,
      transaction: notifications.filter(n => this.categorizeNotification(n.eventType) === 'transaction').length
    };
  }
  ...
  private static extractRelatedEntities(notification: any): Array<{type: string, id: string, name?: string}> {
