...
export class WalletService {
	...
	async getUserWallet(userId: number): Promise<{
		...
		try {
			...
			throw new WalletError('Failed to get user wallet', 500, ErrorCodes.DB_ERROR, {
	...
	async ensureCcPaymentWallet(userId: number): Promise<string> {
		try {
			...
			// Update user record
			await db.update(users).set({ ccpaymentAccountId }).where(eq(users.id, userId));

			return ccpaymentAccountId;
		} catch (error) {
			logger.error(
				'Error ensuring CCPayment wallet:',
				error instanceof Error ? error.message : String(error)
			);
			...
			throw new WalletError('Failed to create CCPayment wallet', 500, ErrorCodes.DB_ERROR, {
				originalError: error instanceof Error ? error.message : String(error)
			});
		}
	}
	...
	async processDgtPurchaseCompletion(
		...
		try {
			return await dgtService.fulfillDgtPurchase(purchaseOrderId, 'confirmed', {
				transactionHash,
				actualAmount,
			...
			logger.error(
				'Error processing DGT purchase completion:',
				error instanceof Error ? error.message : String(error)
			);
			...
			throw WalletError.transactionFailed('Failed to process DGT purchase');
		}
	}
		...
		try {
			...
			if (type) {
				...
				if (validTypes.includes(type as (typeof transactionTypeEnum.enumValues)[number])) {
					conditions.push(
						eq(transactions.type, type as (typeof transactionTypeEnum.enumValues)[number])
					);
				} else {
			...
			throw new WalletError('Failed to get transaction history', 500, ErrorCodes.DB_ERROR, {
				originalError: error instanceof Error ? error.message : String(error)
			});
		}
	}
