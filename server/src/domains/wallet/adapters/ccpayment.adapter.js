 ...
 */
import { fromCCPaymentBalance, fromCCPaymentDepositAddress, fromCCPaymentTokenInfo } from '@shared/types/wallet.transformer';
...
export class CCPaymentAdapter {
    ...
    async getUserBalance(userId) {
        try {
            ...
            const cryptoBalances = ccBalances.map(fromCCPaymentBalance);
            ...
            throw new WalletError('Failed to fetch user balance', ErrorCodes.PAYMENT_PROVIDER_ERROR, 500, { userId, originalError: error });
        }
    }
    /**
    ...
    async createDepositAddress(userId, coinSymbol, chain) {
        try {
            ...
            const ccPaymentUserId = await this.getCCPaymentUserId(userId);
            const ccAddress = await ccpaymentService.getOrCreateUserDepositAddress(ccPaymentUserId, coinSymbol);
            ...
            throw new WalletError('Failed to create deposit address', ErrorCodes.PAYMENT_PROVIDER_ERROR, 500, { userId, coinSymbol, chain, originalError: error });
        }
    }
    /**
    ...
    async requestWithdrawal(userId, request) {
        try {
            ...
            const isValidAddress = await ccpaymentTokenService.checkWithdrawalAddressValidity(request.currency, request.address);
            if (!isValidAddress) {
            ...
            const transactionId = `tx_${orderId}`;
            ...
            throw new WalletError('Failed to process withdrawal', ErrorCodes.PAYMENT_PROVIDER_ERROR, 500, { userId, request, originalError: error });
        }
    }
    /**
    ...
    async getSupportedCoins() {
        try {
            ...
            throw new WalletError('Failed to fetch supported coins', ErrorCodes.PAYMENT_PROVIDER_ERROR, 500, { originalError: error });
        }
    }
    /**
        ...
        try {
            ...
            if (ccPaymentUserId) {
                try {
                    ...
                    logger.error('CCPaymentAdapter', 'Failed to fetch CCPayment transactions, continuing with local.', { userId, error });
                }
            }
            ...
            const localDgtTransactions = localDbTransactions.map((tx) => ({
                id: tx.id,
                userId: tx.userId,
                type: tx.type,
                amount: tx.amount,
            ...
            throw new WalletError('Failed to fetch transaction history', ErrorCodes.PAYMENT_PROVIDER_ERROR, 500, { userId, options, originalError: error });
        }
    }
    /**
    ...
    async processWebhook(payload, signature) {
        try {
            ...
            await ccpaymentService.processWebhookEvent({ ...event, signature }, this.handleDepositWebhook.bind(this), this.handleWithdrawalWebhook.bind(this));
            return {
                success: true,
                transactionId: `tx_${event.orderId}`,
     ...
     */
    async handleDepositWebhook(orderId, amount, currency, txHash) {
     ...
     */
    async handleWithdrawalWebhook(orderId, status, txHash) {
     ...
     */
    async getCCPaymentUserId(userId) {
        ...
        if (!user || !user.ccpaymentAccountId) {
            ...
            throw new WalletError(`CCPayment account ID not found for user: ${userId}`, ErrorCodes.NOT_FOUND, 404);
        }
