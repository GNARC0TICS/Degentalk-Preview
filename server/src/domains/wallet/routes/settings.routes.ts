...
router.get('/', isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    if (!hasPermission(user.role, 'economy', 'read')) {
      throw new WalletError('Insufficient permissions to view wallet settings', ErrorCodes.FORBIDDEN, 403);
    }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!hasPermission(user.role, 'economy', 'write')) {
        throw new WalletError('Insufficient permissions to modify wallet settings', ErrorCodes.FORBIDDEN, 403);
      }
