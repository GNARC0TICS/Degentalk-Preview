...
const CCPAYMENT_NOTIFICATION_URL = process.env.CCPAYMENT_NOTIFICATION_URL || 'https://api.degentalk.com/api/wallet/webhook';
const CCPAYMENT_WEBHOOK_URL = process.env.CCPAYMENT_WEBHOOK_URL || 'https://yourdomain.com/api/ccpayment/webhook';
...
export class CCPaymentService {
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      logger.error('CCPaymentService', `CCPayment API Error: ${error instanceof Error ? error.message : String(error)}`, { endpoint, method });
  ...
  async createCcPaymentWalletForUser(internalUserId: number): Promise<string> {
    try {
      ...
      logger.error('CCPaymentService', 'Error creating CCPayment wallet:', error instanceof Error ? error.message : String(error));
      throw new WalletError(
  ...
  async createDepositAddress(ccPaymentUserId: string, coin: string): Promise<DepositAddress> {
    try {
      ...
      logger.info('CCPaymentService', `[MOCK] Created ${coin} deposit address for CCPayment user ${ccPaymentUserId}`);
      ...
      logger.error('CCPaymentService', 'Error creating deposit address:', error instanceof Error ? error.message : String(error));
      throw new WalletError(
  ...
  async getUserCryptoBalances(ccPaymentUserId: string): Promise<CryptoBalance[]> {
    try {
      ...
      logger.info('CCPaymentService', `[MOCK] Retrieved balances for CCPayment user ${ccPaymentUserId}`);
      ...
      logger.error('CCPaymentService', 'Error getting crypto balances:', error instanceof Error ? error.message : String(error));
      throw new WalletError(
  ...
  async getTransactionHistory(
    ...
    try {
      ...
      logger.info('CCPaymentService', `[MOCK] Retrieved transaction history for CCPayment user ${ccPaymentUserId}`);
      ...
      if (filters.startDate) {
        ...
        filteredTransactions = filteredTransactions.filter(tx => new Date(tx.createdAt).getTime() >= startDate);
      }
      
      if (filters.endDate) {
        const endDate = new Date(filters.endDate).getTime();
        filteredTransactions = filteredTransactions.filter(tx => new Date(tx.createdAt).getTime() <= endDate);
      }
      ...
      logger.error('CCPaymentService', 'Error getting transaction history:', error instanceof Error ? error.message : String(error));
      throw new WalletError(
  ...
  async createDepositLink(request: DepositRequest): Promise<string> {
    try {
      ...
      logger.error('CCPaymentService', 'Error creating deposit link:', error instanceof Error ? error.message : String(error));
      throw new WalletError(
  ...
  async requestWithdrawal(request: WithdrawalRequest): Promise<CCPaymentWithdrawalResponse> {
    try {
      ...
      logger.info('CCPaymentService', `[MOCK] Created withdrawal request for order ${request.orderId}`);
      ...
      logger.error('CCPaymentService', 'Error requesting withdrawal:', error instanceof Error ? error.message : String(error));
      throw new WalletError(
  ...
  async getTransactionStatus(orderId: string): Promise<CCPaymentOrderStatus> {
    try {
      ...
      logger.error('CCPaymentService', 'Error getting transaction status:', error instanceof Error ? error.message : String(error));
      throw new WalletError(
  ...
  verifyWebhookSignature(
    ...
    try {
      ...
      logger.error('CCPaymentService', 'Error verifying webhook signature:', error instanceof Error ? error.message : String(error));
      return false;
    }
  }
  ...
  async processWebhookEvent(
    ...
    try {
      logger.info('CCPaymentService', 'Processing CCPayment webhook event', { eventType: event.eventType, orderId: event.orderId });
      ...
      logger.error('CCPaymentService', 'Error processing webhook event:', error instanceof Error ? error.message : String(error));
