...
export class WalletController {
   ...
   */
    ...
    try {
      ...
      logger.info('WALLET_CONTROLLER', `Successfully retrieved DGT balance: ${dgtBalance} for user ID: ${userId}`);
      ...
      logger.error('WALLET_CONTROLLER', `Error getting wallet balance: ${errorMessage}. Details: ${error instanceof Error ? error.stack : String(error)}`, { stack: error instanceof Error ? error.stack : undefined, errorObj: error });
  ...
  async getTransactionHistory(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (currencyFilter) {
        ...
        logger.warn('WALLET_CONTROLLER', `Currency filter ignored: transactions table has no currency column. Filter was: ${currencyFilter}`); 
      }

      if (typeFilter) {
        const validTransactionTypes = transactionTypeEnum.enumValues;
        if (validTransactionTypes.includes(typeFilter as (typeof transactionTypeEnum.enumValues)[number])) {
          conditions.push(eq(transactions.type, typeFilter as (typeof transactionTypeEnum.enumValues)[number]));
        } else {
          logger.warn('WALLET_CONTROLLER', 'Invalid transaction type filter provided: %s. Ignoring filter.', typeFilter);
        }
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error('WALLET_CONTROLLER', `Error getting transaction history: ${errorMessage}. Details: ${error instanceof Error ? error.stack : String(error)}`, { stack: error instanceof Error ? error.stack : undefined, errorObj: error });
  ...
  async createDepositAddress(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user) {
        logger.warn('WALLET_CONTROLLER', `Attempt to access createDepositAddress without authentication. Request details: path ${req.path}, ip ${req.ip}`);
        return res.status(401).json({ error: 'User not authenticated', code: WalletErrorCodes.UNAUTHORIZED });
      }
      ...
      res.status(501).json({ message: "Crypto deposits are temporarily disabled.", code: "SERVICE_UNAVAILABLE" });
      return;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error('WALLET_CONTROLLER', `Error creating deposit address: ${errorMessage}. Details: ${error instanceof Error ? error.stack : String(error)}`, { stack: error instanceof Error ? error.stack : undefined, errorObj: error });
  ...
  async createDgtPurchase(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user) {
        logger.warn('WALLET_CONTROLLER', `Attempt to access createDgtPurchase without authentication. Request details: path ${req.path}, ip ${req.ip}`);
        return res.status(401).json({ error: 'User not authenticated', code: WalletErrorCodes.UNAUTHORIZED });
      }
      const userId = (req.user as { id: number }).id; // Explicit cast after check
      ...
      res.status(501).json({ message: "DGT purchases via crypto are temporarily disabled.", code: "SERVICE_UNAVAILABLE" });
      return;

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error('WALLET_CONTROLLER', `Error creating DGT purchase order: ${errorMessage}`, { details: { stack: error instanceof Error ? error.stack : undefined, errorObj: error } });
  ...
  async getPurchaseOrderStatus(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user) {
        logger.warn('Attempt to access getPurchaseOrderStatus without authentication. Request details: path %s, ip %s', req.path, req.ip);
        return res.status(401).json({ error: 'User not authenticated', code: WalletErrorCodes.UNAUTHORIZED });
      }
      const userId = (req.user as { id: number }).id; // Explicit cast after check
      ...
      const [purchaseOrder] = await db
        ...
        .where(and(
          eq(dgtPurchaseOrders.id, orderId),
          eq(dgtPurchaseOrders.userId, userId)
        ))
        .limit(1);
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error('Error getting purchase order status: %s. Details: %o', errorMessage, { stack: error instanceof Error ? error.stack : undefined, errorObj: error });
  ...
  async transferDgt(req: Request, res: Response): Promise<void> {
    try {
      if (!req.user) {
        logger.warn('Attempt to access transferDgt without authentication. Request details: path %s, ip %s', req.path, req.ip);
        return res.status(401).json({ error: 'User not authenticated', code: WalletErrorCodes.UNAUTHORIZED });
      }
      const userId = (req.user as { id: number }).id; // Explicit cast after check
      ...
      // Transfer DGT
      const result = await dgtService.transferDgt( // Used instance dgtService
        userId,
        toUserId,
        BigInt(amount),
        'TIP',
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error('Error transferring DGT: %s. Details: %o', errorMessage, { stack: error instanceof Error ? error.stack : undefined, errorObj: error });
  ...
  async getSupportedCurrencies(req: Request, res: Response): Promise<void> {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error('WALLET_CONTROLLER', `Error getting supported currencies: ${errorMessage}`, { details: { stack: error instanceof Error ? error.stack : undefined, errorObj: error } });
  ...
  async createDgtRewardTransaction(req: Request, res: Response) {
    try {
      ...
      const dgtAmountToAward = Number(process.env.DGT_REWARD_CREATE_THREAD) || DEFAULT_DGT_REWARD_CREATE_THREAD;

      if (dgtAmountToAward <= 0) {
          logger.info('DGT_CONTROLLER', `DGT reward for '${context}' is zero or negative. No transaction created for user ${userId}.`)
      ...
      logger.error('DGT_CONTROLLER', `Error in createDgtRewardTransaction for context '${req.body.context}':`, err);
