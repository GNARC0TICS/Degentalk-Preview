...
import { db } from '@db';
import { wallets, transactions, supportedTokens, cryptoWallets, swapRecords } from '@schema';
...
export class WalletService {
    ...
    async createDepositAddress(userId, coinSymbol, chain) {
        try {
            ...
            const depositAddress = await this.primaryAdapter.createDepositAddress(userId, coinSymbol, chain || this.getDefaultChain(coinSymbol));
        ...
        try {
            ...
            return await db.transaction(async (tx) => {
                ...
                try {
                    ...
                    const primaryCoins = supportedCoins.filter((coin) => ['BTC', 'ETH', 'USDT'].includes(coin.symbol.toUpperCase()));
     ...
     */
    async ensureCcPaymentWallet(userId) {
        try {
     ...
     */
    async swapCrypto(userId, params) {
        try {
    ...
    async getTransactionHistory(userId, options = {}) {
        try {
            ...
            const transactions = await this.primaryAdapter.getTransactionHistory(userId, paginationOptions);
    ...
    async processWebhook(provider, payload, signature) {
        try {
            ...
            if (provider !== this.config.primaryProvider) {
                throw new WalletError(`Unsupported webhook provider: ${provider}`, ErrorCodes.VALIDATION_ERROR, 400, { provider });
            }
    ...
    validateWithdrawalRequest(request) {
        ...
        if (request.amount <= 0) {
            throw new WalletError('Withdrawal amount must be positive', ErrorCodes.VALIDATION_ERROR, 400, { amount: request.amount });
        }
     ...
     */
    async getOrCreateDgtWallet(tx, userId) {
     ...
     */
    getTransactionDescription(metadata) {
    ...
    transformDbTransaction(dbTx, balanceAfter) {
        return {
            id: dbTx.id,
            userId: dbTx.userId,
            type: dbTx.type,
