...
export class CCPaymentTokenService {
	...
	private supportedTokensCache: CCPaymentTokenInfo[] | null = null;
	 ...
	 */
	async getSupportedTokens(): Promise<CCPaymentTokenInfo[]> {
		try {
			...
			const response = await ccpaymentApiService.makeRequest<{
				list: CCPaymentTokenInfo[];
			}>('/ccpayment/v2/getSupportCoinList');

			if (!response.list) {
				throw new WalletError(
					'Failed to parse supported tokens from CCPayment response',
					ErrorCodes.PAYMENT_PROVIDER_ERROR,
					500,
					{ response }
				);
