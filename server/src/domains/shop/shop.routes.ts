...
router.post('/purchase', isAuthenticated, validateRequest(shopValidation.purchaseItem), async (req, res) => {
	try {
...
router.get('/inventory', isAuthenticated, validateRequest(shopValidation.getUserInventory), async (req, res) => {
	try {
		const requestingUser = userService.getUserFromRequest(req);
		if (!requestingUser) {
			return sendErrorResponse(res, 'User not authenticated', 401);
