 ...
 */
router.get('/browse', validateRequest(shopValidation.browseCosmetics), async (req, res) => {
	try {
		// req.query is now validated by shopValidation.browseCosmetics schema
...
router.get('/equipped', isAuthenticated, async (req, res) => {
	try {
		...
		const equippedItems = await db
			.select()
			.from(userInventory)
			.where(and(eq(userInventory.userId, requestingUser.id), eq(userInventory.isEquipped, true)));
	...
	async (req, res) => {
		try {
			...
			if (action === 'equip') {
				...
				const validation = CosmeticsTransformer.validateEquipmentChange(
					cosmeticData,
					userInventoryData
				);
...
router.post(
	'/customize',
	isAuthenticated,
	validateRequest(shopValidation.customizeCosmetic),
	async (req, res) => {
		try {
			const requestingUser = userService.getUserFromRequest(req);
			if (!requestingUser?.id) {
			...
			// req.body is now validated by shopValidation.customizeCosmetic schema
			const { itemId, customizations, dgtCost } = req.body;
			const userId = requestingUser.id;
...
router.get(
	'/preview/:itemIds',
	validateRequest(shopValidation.previewCosmetics),
	async (req, res) => {
		try {
			// req.params is now validated by shopValidation.previewCosmetics schema
