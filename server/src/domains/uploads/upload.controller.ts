...
export async function createPresignedUploadUrlController(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized: User ID missing. You gotta be logged in to flex that new PFP, anon.' });
    }

    if (!fileName || !fileType || fileSize === undefined || !uploadType) {
      return res.status(400).json({ error: 'Missing parameters: fileName, fileType, fileSize, and uploadType are required. Spill the tea, what are you uploading?' });
    }

    if (uploadType !== 'avatar' && uploadType !== 'banner') {
        return res.status(400).json({ error: 'Invalid uploadType. Must be "avatar" or "banner". Choose your fighter!' });
    }

    const params: CreatePresignedUrlServiceParams = { // Updated type name
    ...
    return res.status(500).json({ error: 'Internal server error: Our server just rugged itself trying to get that URL. Try again later, maybe?' });
  }
}
...
export async function confirmUploadController(req: ConfirmUploadAuthenticatedRequest, res: Response) {
  try {
    const { relativePath, uploadType } = req.body; // Changed from supabasePath
    const userId = req.user?.id;

    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized: User ID missing. Can\'t confirm without knowing who you are, space cadet.' });
    }

    if (!relativePath || !uploadType) {
      return res.status(400).json({ error: 'Missing parameters: relativePath and uploadType are required. Details, degen, details!' });
    }
     if (uploadType !== 'avatar' && uploadType !== 'banner') {
        return res.status(400).json({ error: 'Invalid uploadType. Must be "avatar" or "banner". Pick one, chief.' });
    }
    ...
    const confirmation: UploadConfirmationServiceResult = await uploadService.confirmUpload(String(userId), confirmServiceParams); 
    ...
    if (!confirmation.success || !confirmation.relativePath || !confirmation.newPublicUrl) {
      ...
      return res.status(400).json({ 
        error: confirmation.message || 'Upload confirmation failed for reasons... unknown and unknowable. Relative path or public URL missing.' 
      });
    }
    ...
    if (!profileUpdateResult.success) {
      ...
      console.error(`Failed to update profile for user ${userId} after upload confirmation: ${profileUpdateResult.message}`);
    ...
    if (error instanceof Error && error.message.includes('Failed to update')) {
        ...
        return res.status(500).json({ error: `Profile update failed after confirming upload: ${error.message}` });
    }
    console.error('Unexpected error in confirmUploadController:', error);
    return res.status(500).json({ error: 'Internal server error: The confirmation ritual failed. Server demons strike again.' });
  }
}
