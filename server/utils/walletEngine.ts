...
export class WalletEngine {
   ...
   */
  static async getUserUnlocks(userId: number): Promise<Array<{ unlock: DgtUnlock; acquired_at: Date }>> {
    try {
      const result = await db.execute(sql`
   ...
   */
  static async purchaseDgtUnlock(
    userId: number,
    unlockId: number
  ...
  static async transferDgt(options: DgtTransferOptions): Promise<DgtPurchaseResult> {
    ...
    try {
      ...
      console.error(`Error transferring ${amount} DGT from user ${fromUserId} to user ${toUserId}:`, error);
