...
console.log(
	`[LOAD_ENV] Script executing. Attempting to load env files from project root: ${projectRoot}`
);
...
if (envLocalResult.error) {
	...
	if (envLocalResult.parsed) {
		console.log(
			`[LOAD_ENV] Successfully parsed ${envLocalPath}. Keys: ${Object.keys(envLocalResult.parsed).join(', ')}`
		);
	} else {
		console.log(
			`[LOAD_ENV] Parsed ${envLocalPath}, but it was empty or contained no new variables.`
		);
	}
}
...
if (dotEnvResult.error) {
	...
	if ((dotEnvResult.error as any).code !== 'ENOENT') {
		...
		console.log(
			`[LOAD_ENV] ${dotEnvPath} not found, which is acceptable if env.local is used primarily.`
		);
	}
} else {
	if (dotEnvResult.parsed) {
		console.log(
			`[LOAD_ENV] Successfully parsed ${dotEnvPath}. Keys: ${Object.keys(dotEnvResult.parsed).join(', ')}`
		);
	} else {
		console.log(
			`[LOAD_ENV] Parsed ${dotEnvPath}, but it was empty or contained no new variables (possibly all overridden by env.local).`
		);
	}
}
