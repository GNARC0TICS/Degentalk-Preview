...
=======
  ...
  async getThreads(categoryId?: number, limit = 20, offset = 0, sortBy = 'latest'): Promise<ThreadWithUser[]> {
    ...
    switch (sortBy) {
      case 'popular':
        query = query.orderBy(desc(threads.isSticky), desc(threads.viewCount), desc(threads.createdAt));
        break;
      case 'replies':
        query = query.orderBy(desc(threads.isSticky), desc(threads.postCount), desc(threads.createdAt));
        break;
      case 'recent': // Recently active by last post time
        query = query.orderBy(desc(threads.isSticky), desc(threads.lastPostAt), desc(threads.createdAt));
        break;
      case 'latest':
      default:
  ...
  async getThreadFeaturePermissions(): Promise<typeof threadFeaturePermissions.$inferSelect[]> {
    return db
      .select()
      .from(threadFeaturePermissions)
      .orderBy(threadFeaturePermissions.featureName);
  }
    ...
    await db.transaction(async (tx: PgTransaction<any, any, any>) => {
      ...
      if (thread && thread.lastPostId === id) {
        ...
        const [previousPost] = await tx
          ...
          .from(posts)
          .where(and(
            eq(posts.threadId, post.threadId),
            eq(posts.isHidden, false),
            not(eq(posts.id, id))
          ))
      ...
      if (deleteResult.rowCount > 0 && post && post.userId !== userId) {
        ...
        if (values) {
          ...
          try {
            ...
            logger.error('XpCloutService', `Error subtracting points from user ${post.userId}:`, updateError);
          }
        }
      }
  ...
  async getAvailableEmojisForUser(userId: number): Promise<EmojiWithAvailability[]> {
    ...
    // Get all emojis
    const allEmojis = await db.select()
      .from(customEmojis)
      .where(eq(customEmojis.isDeleted, false));
  ...
  async createForumRule(rule: InsertForumRule & { createdBy?: number }): Promise<ForumRule> {
    ...
    const [newRule] = await db.insert(forumRules).values({
      ...rule,
  ...
  async updateForumRule(id: number, rule: Partial<ForumRule> & { updatedBy?: number }): Promise<ForumRule> {
    ...
    const [updatedRule] = await db.update(forumRules).set({
      ...rule,
      ...(versionHash ? { lastAgreedVersionHash: versionHash } : {}),
      updatedAt: new Date()
    }).where(eq(forumRules.id, id)).returning();
  ...
  async getUserRuleAgreements(userId: number): Promise<UserRulesAgreement[]> {
    const agreements = await db.select().from(userRulesAgreements).where(eq(userRulesAgreements.userId, userId));
    return agreements;
  }
  ...
  async agreeToRule(userId: number, ruleId: number, versionHash: string): Promise<void> {
    ...
    const [existingAgreement] = await db.select()
      ...
      .where(and(
        ...
        eq(userRulesAgreements.ruleId, ruleId)
      ));

    if (existingAgreement) {
      // Update the existing agreement with the new version hash
      await db.update(userRulesAgreements)
        .set({
          versionHash,
          agreedAt: new Date()
        })
      ...
      await db.insert(userRulesAgreements)
        .values({
          userId,
          ruleId,
          versionHash,
...
=======
  ...
  async purchaseProduct(userId: number, productId: number, quantity: number = 1): Promise<Order> {
    ...
    return db.transaction(async (tx: PgTransaction<any, any, any>) => {
      ...
      // Record inventory transaction
      await tx.insert(inventoryTransactions)
        .values({
          productId,
          type: 'sale',
          quantity: -quantity,
      ...
      if (product.pluginReward) {
        ...
        // Process sticker unlocks
        if (product.pluginReward.stickerUnlocks) {
          const stickerIds = product.pluginReward.stickerUnlocks;
          const currentUnlocks = user.unlockedStickers || [];

          await tx.update(users)
            .set({
              unlockedStickers: [...new Set([...currentUnlocks, ...stickerIds])]
            })
            .where(eq(users.id, userId));
        }
  ...
  async getConversations(userId: number): Promise<(Conversation & { participants: ConversationParticipant[] })[]> {
    ...
    // Group by conversation
    const conversationsMap = new Map<number, Conversation & { participants: ConversationParticipant[] }>();
  ...
  async markMessagesAsRead(conversationId: number, userId: number): Promise<void> {
    await db.transaction(async (tx: PgTransaction<any, any, any>) => {
      // Get unread messages
      const unreadMessages = await tx.select()
        .from(messages)
        .leftJoin(
          messageReads,
          and(
  ...
  async addUserXp(userId: number, amount: number, path?: string): Promise<void> {
    ...
    await db.transaction(async (tx: PgTransaction<any, any, any>) => {
      ...
      if (path) {
        ...
        // For Category-based XP gain, check for and unlock any eligible emojis
        const emojiUnlocks = await tx.select()
          .from(customEmojis)
          .where(and(
            eq(customEmojis.isLocked, true),
            eq(customEmojis.unlockType, 'path_xp'),
  ...
  async checkUserOwnsProduct(userId: number, productId: number): Promise<boolean> {
    const [item] = await db.select({
      ...
      .where(and(
        ...
        eq(userInventory.productId, productId)
      ));
  ...
  async addProductToUserInventory(item: InsertUserInventoryItem): Promise<UserInventoryItem> {
    ...
    if (existingItem) {
      // Update existing inventory item
      const [updatedItem] = await db.update(userInventory)
        .set({
          quantity: sql`${userInventory.quantity} + ${item.quantity || 1}`,
          lastUsedAt: new Date()
        })
        .where(and(
          eq(userInventory.userId, item.userId),
          eq(userInventory.productId, item.productId)
        ))
        .returning();
      ...
      // Insert new inventory item
      const [inventoryItem] = await db.insert(userInventory)
        .values({
          ...item,
          transactionId,
          acquiredAt: new Date()
  ...
  async getInventoryTransactions(userId: number): Promise<typeof inventoryTransactions.$inferSelect[]> {
    return db.select()
      .from(inventoryTransactions)
      .where(eq(inventoryTransactions.userId, userId))
      .orderBy(desc(inventoryTransactions.createdAt));
