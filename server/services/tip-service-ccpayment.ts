...
export class TipService {
	...
	async processTip(
		...
		try {
			logger.info(
				'TipService',
				`Processing tip from user ${senderUserId} to user ${recipientUserId}`,
				{
					amount
				}
			);
			...
			if (!recipient) {
				throw new WalletError('Recipient not found', 404, WalletErrorCodes.USER_NOT_FOUND);
			}
        ...
        `);
				...
				logger.info(
					'TipService',
					`Tip completed: User ${senderUserId} -> User ${recipientUserId}`,
					{
						amount,
						transactionId: transaction.id
					}
	...
	async processTipCommand(
		...
		try {
			logger.info(
				'TipService',
				`Processing tip command from user ${senderUserId} to user ${recipientUserId}`,
				{
					amount,
					currency,
					roomId,
			...
			// Process the tip
			const { transactionId } = await this.processTip(senderUserId, recipientUserId, amount);
	...
	private async checkCooldowns(userId: UserId, commandType: 'tip' | 'rain'): Promise<void> {
		try {
			...
			const settings = await db.query.cooldownSettings.findFirst({
				where: and(eq(cooldownSettings.isActive, true), eq(cooldownSettings.type, commandType))
			});
