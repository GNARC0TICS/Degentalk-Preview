import { db } from '@db';
import { users, xpReputationSettings } from '@schema';
import type { UserId } from '@shared/types/ids';
import { eq, sql } from 'drizzle-orm';
import { logger } from '../src/core/logger';

export class XpReputationService {
	/**
	 * Fetches the XP and Reputation values for a specific action key.
	 * @param actionKey - The unique key identifying the action (e.g., 'POST_CREATE').
	 * @returns Object containing xpValue and reputationValue, or null if not found.
	 */
	async getActionValues(
		actionKey: string
	): Promise<{ xpValue: number; reputationValue: number } | null> {
		try {
			const result = await db
				.select({
					xpValue: xpReputationSettings.xpValue,
					reputationValue: xpReputationSettings.reputationValue
				})
				.from(xpReputationSettings)
				.where(eq(xpReputationSettings.actionKey, actionKey))
				.limit(1);

			if (result.length > 0) {
				return result[0];
			}
			logger.warn('XpReputationService', `No XP/Reputation settings found for actionKey: ${actionKey}`);
			return null;
		} catch (error) {
			logger.error('XpReputationService', `Error fetching action values for ${actionKey}:`, error);
			return null;
		}
	}

	/**
	 * Awards XP and Reputation to a user for a specific action.
	 * Fetches values from xpReputationSettings based on the actionKey.
	 * @param userId - The ID of the user to award points to.
	 * @param actionKey - The key identifying the action performed.
	 * @param multiplier - Optional multiplier (e.g., for tips based on amount).
	 * @returns Boolean indicating if the points were awarded successfully.
	 */
	async awardPoints(userId: UserId, actionKey: string, multiplier: number = 1): Promise<boolean> {
		const values = await this.getActionValues(actionKey);

		if (!values || (values.xpValue === 0 && values.reputationValue === 0)) {
			// No points defined for this action or values are zero
			return false;
		}

		const xpToAdd = Math.floor(values.xpValue * multiplier);
		const reputationToAdd = Math.floor(values.reputationValue * multiplier);

		if (xpToAdd === 0 && reputationToAdd === 0) {
			// No points to add after applying multiplier
			return false;
		}

		try {
			// Update user record
			// TODO: Add logic to check for level up based on new XP total
			await db
				.update(users)
				.set({
					xp: sql`${users.xp} + ${xpToAdd}`,
					reputation: sql`${users.reputation} + ${reputationToAdd}`,
					updatedAt: new Date()
				})
				.where(eq(users.id, userId));

			logger.info(
				'XpReputationService',
				`Awarded ${xpToAdd} XP and ${reputationToAdd} Reputation to user ${userId} for action ${actionKey}`
			);

			// TODO: Trigger level up check and potential notifications/rewards
			// await this.checkLevelUp(userId);

			return true;
		} catch (error) {
			logger.error(
				'XpReputationService',
				`Error awarding points to user ${userId} for action ${actionKey}:`,
				error
			);
			return false;
		}
	}

	/**
	 * Checks if a user has leveled up after gaining XP and applies rewards.
	 * (Placeholder - needs implementation)
	 */
	async checkLevelUp(userId: UserId): Promise<void> {
		// 1. Get current user XP and level
		// 2. Query `levels` table to find the next level threshold
		// 3. If user XP >= next level threshold:
		//    a. Update user level
		//    b. Query the `levels` table for rewards (DGT, Title, Badge)
		//    c. Grant rewards (update user DGT balance, activeTitleId, activeBadgeId)
		//    d. Potentially create a notification
		//    e. Recursively check for further level ups if multiple levels gained
		logger.info(
			'XpReputationService',
			`Level up check triggered for user ${userId} - (Not yet implemented)`
		);
	}
}

// Export a singleton instance
export const xpReputationService = new XpReputationService();
