...
export async function up() {
  console.log('🚀 Applying migration: XP, Clout, Levels & Titles Enhancements');
  ...
  try {
    ...
    console.log('✅ Migration applied successfully: XP, Clout, Levels & Titles Enhancements');
  } catch (error) {
...
export async function down() {
  console.log('↩️ Reverting migration: XP, Clout, Levels & Titles Enhancements');
  const { db, pool } = getDbConnection();

  try {
    await db.transaction(async (tx) => {
      console.log('Reverting users table enhancements...');
      ...
      console.log('Reverting levels table enhancements...');
      ...
      console.log('Dropping badges table...');
      await tx.execute(sql`DROP TABLE IF EXISTS badges;`);
      
      console.log('Dropping titles table...');
      await tx.execute(sql`DROP TABLE IF EXISTS titles;`);

      console.log('Dropping xp_clout_settings table...');
      await tx.execute(sql`DROP TABLE IF EXISTS xp_clout_settings;`);

      console.log('Reverting custom_emojis table enhancements...');
      ...
      console.log('✅ XP/Clout/Levels enhancements reverted successfully.');
    });
    console.log('✅ Migration reverted successfully: XP, Clout, Levels & Titles Enhancements');
  } catch (error) {
