...
export default function LevelManagementPage() {
	...
	const createLevelMutation = useMutation({
		mutationFn: (data: LevelFormData) =>
			apiRequest({ url: '/api/admin/levels', method: 'POST', data }),
		onSuccess: () => {
	...
	const updateLevelMutation = useMutation({
		mutationFn: ({ id, data }: { id: number; data: LevelFormData }) =>
			apiRequest({ url: `/api/admin/levels/${id}`, method: 'PUT', data }),
		onSuccess: () => {
	...
	const columns = [
		{
			key: 'level',
			header: 'Level',
			render: (level: Level) => <UiBadge variant="outline">{level.level}</UiBadge>
		},
		{
			key: 'xpRequired',
			header: 'XP Required',
			render: (level: Level) => level.xpRequired.toLocaleString()
		},
		{
			key: 'description',
			header: 'Description',
		...
		},
		{
			...
			header: 'Title Reward',
			render: (level: Level) =>
				level.rewardTitle ? <UiBadge variant="secondary">{level.rewardTitle}</UiBadge> : '-'
		}
	];
