...
export default function TitleManagementPage() {
  ...
  const [sortField, setSortField] = useState<'name' | 'rarity' | 'category' | 'createdAt'>('createdAt');
  ...
  const {
    ...
    queryKey: ['/api/admin/titles', { search: debouncedSearchTerm, sort: sortField, direction: sortDirection, page, pageSize }],
    queryFn: async () => {
      const response = await fetch(`/api/admin/titles?search=${debouncedSearchTerm}&sort=${sortField}&direction=${sortDirection}&page=${page}&pageSize=${pageSize}`);
      if (!response.ok) {
  ...
  const getCategoryBadgeColor = (category: string) => {
    switch (category) {
      case 'path': return 'bg-cyan-900 text-cyan-300';
      case 'achievement': return 'bg-emerald-900 text-emerald-300';
      case 'event': return 'bg-purple-900 text-purple-300';
      case 'seasonal': return 'bg-blue-900 text-blue-300';
      case 'purchase': return 'bg-amber-900 text-amber-300';
