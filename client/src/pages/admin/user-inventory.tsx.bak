...
export default function AdminUserInventoryPage() {
  ...
  const { data: inventory, isLoading: loadingInventory } = useQuery({
    ...
    queryFn: () => apiRequest<UserInventoryItem[]>({ 
  ...
  const { data: userData } = useQuery({
    ...
    queryFn: () => apiRequest<{ username: string }>({ 
      ...
      <div className="p-6">
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>
              Add a new item to this user's inventory
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          ...
                          <Badge 
                            variant="outline" 
                            className={cn("mt-1", getRarityColor(rarity))}
                          >
                            {rarity}
                          </Badge>
                        </div>
