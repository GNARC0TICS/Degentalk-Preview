...
import { UserInventoryWithProduct, Product } from '@/types/inventory'; // Assuming these types
...
export default function AdminUserInventoryPage() {
  ...
  const { data: inventory, isLoading: isLoadingInventory, refetch: refetchInventory } = useQuery<UserInventoryWithProduct[], Error>({
  ...
  const equipMutation = useMutation<any, Error, { inventoryId: number }>({
    mutationFn: ({ inventoryId }) => apiRequest({
  ...
  const unequipMutation = useMutation<any, Error, { inventoryId: number }>({
    ...
    onSuccess: () => {
      toast({ title: 'Item unequipped', description: 'The item has been successfully unequipped.' });
      queryClient.invalidateQueries({ queryKey: ['userInventory', userId] });
    },
    onError: (error) => {
      toast({ title: 'Error unequipping item', description: error.message, variant: 'destructive' });
    },
  });

  // Mutation for granting an item
  const grantItemMutation = useMutation<any, Error, { productId: number }>({
    mutationFn: ({ productId }) => apiRequest({
      url: `/api/admin/user-inventory/${userId}/grant`,
      method: 'POST',
      data: { productId, quantity: 1 }, // Assuming quantity 1 for cosmetics
    }),
    onSuccess: () => {
      toast({ title: 'Item granted', description: 'The item has been successfully granted to the user.' });
      queryClient.invalidateQueries({ queryKey: ['userInventory', userId] });
  ...
  if (!userId) {
    return <AdminLayout><div className="p-4 text-red-500">User ID is missing.</div></AdminLayout>;
  }

  return (
    <AdminLayout>
      <div className="p-6">
        <Link href="/admin/users" className="inline-flex items-center text-sm text-zinc-400 hover:text-white mb-4">
          <ArrowLeft className="mr-2 h-4 w-4" /> Back to User List (assuming /admin/users exists)
        </Link>
        <h1 className="text-2xl font-semibold text-white mb-6">Manage Inventory for User ID: {userId}</h1>
        ...
        <div className="p-6 bg-zinc-900 border border-zinc-800 rounded-lg">
          ...
          {isLoadingInventory ? (
            ...
            <ul className="space-y-3">
              {inventory.map((item) => (
                <li key={item.id} className={`p-4 rounded-md flex justify-between items-center ${item.equipped ? 'bg-green-900/50 border border-green-700' : 'bg-zinc-800 border border-zinc-700'}`}>
                  <div>
                    <span className="font-semibold text-white">{item.product?.name || 'Unknown Item'}</span>
                    <span className="text-xs text-zinc-400 ml-2">(Inv. ID: {item.id}, Prod. ID: {item.productId})</span>
                    {item.equipped && <CheckCircle className="inline-block ml-2 h-5 w-5 text-green-400" />}
                  </div>
                  <div>
                    {item.equipped ? (
                      <Button
                        ...
                        disabled={unequipMutation.isPending && unequipMutation.variables?.inventoryId === item.id}
                        className="border-red-500 text-red-400 hover:bg-red-600 hover:text-white"
                      >
                        {unequipMutation.isPending && unequipMutation.variables?.inventoryId === item.id ? 'Unequipping...' : 'Unequip'}
                      </Button>
                    ) : (
                      <Button
                        variant="outline"
                        ...
                        disabled={equipMutation.isPending && equipMutation.variables?.inventoryId === item.id}
                        className="border-sky-500 text-sky-400 hover:bg-sky-600 hover:text-white"
                      >
                        {equipMutation.isPending && equipMutation.variables?.inventoryId === item.id ? 'Equipping...' : 'Equip'}
                      </Button>
                    )}
                  </div>
                </li>
           ...
           <Button onClick={() => refetchInventory()} className="mt-4">Refresh Inventory</Button>
        </div>
      </div>
    </AdminLayout>
  );
