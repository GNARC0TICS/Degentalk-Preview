...
const levelFormSchema = z.object({
	...
	rankName: z
		.string()
		.min(2, 'Rank name must be at least 2 characters')
		.max(50, 'Rank name must be at most 50 characters'),
	description: z.string().optional().nullable(),
	iconUrl: z.string().url('Please enter a valid URL').optional().nullable(),
	colorCode: z
		.string()
		.regex(/^#[0-9A-Fa-f]{6}$/, 'Please enter a valid hex color code (e.g. #00FF00)')
		.optional()
		.nullable()
});
...
export default function LevelsAdmin() {
	...
	const handleDeleteLevel = (id: number) => {
		if (
			window.confirm(
				'Are you sure you want to delete this level? This may affect user progression.'
			)
		) {
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormDescription>
													The title users will receive at this level
												</FormDescription>
												<FormMessage />
											</FormItem>
										)}
				...
				<Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
					<DialogContent className="sm:max-w-[500px]">
						<DialogHeader>
							...
							<DialogDescription>Update the level progression settings.</DialogDescription>
						</DialogHeader>
						<Form {...form}>
