...
export default function AdminThreadsPage() {
  ...
  const [categoryFilter, setCategoryFilter] = useState("all");
  const [prefixFilter, setPrefixFilter] = useState("all");
  const [statusFilter, setStatusFilter] = useState(forumRulesConfig.threadStatusOptions.all.key); // [CONFIG-REFAC]
  const [sortBy, setSortBy] = useState(forumRulesConfig.threadSortOptions.newest.key); // [CONFIG-REFAC]

  const pageSize = 10;

  // Fetch threads with pagination and filters
  const { data: threadsData, isLoading: isLoadingThreads } = useQuery({
    queryKey: ['/api/admin/threads', page, search, categoryFilter, prefixFilter, statusFilter, sortBy],
    queryFn: async () => {
      // Build query parameters
      const params = new URLSearchParams({
        page: page.toString(),
  ...
  // Handle filter clear [CONFIG-REFAC]
  const clearFilters = () => {
    setSearch("");
    setCategoryFilter("all");
    setPrefixFilter("all");
    setStatusFilter(forumRulesConfig.threadStatusOptions.all.key);
    setSortBy(forumRulesConfig.threadSortOptions.newest.key);
  };
