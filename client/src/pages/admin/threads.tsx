...
export default function AdminThreadsPage() {
  ...
  const [filters, setFilters] = useState<Record<string, FilterValue>>({
	...
	const { data: threadsData, isLoading: isLoadingThreads, error: threadsQueryError } = useQuery<ThreadsResponse>({
		queryKey: [
			'/api/admin/threads',
      ...
      pagination.pageSize,
		...
		queryFn: async () => {
			...
			if (filters.category && filters.category !== 'all') params.append('category', filters.category as string);
			if (filters.prefix && filters.prefix !== 'all') params.append('prefix', filters.prefix as string);
  ...
  const handleFilterChange = (filterId: string, value: FilterValue) => {
    setFilters((prevFilters: Record<string, FilterValue>) => ({ ...prevFilters, [filterId]: value }));
    setPagination((prev: SimplePaginationState) => ({ ...prev, pageIndex: 0 }));
  };
  ...
  const filtersConfig: FilterConfig[] = [
    {
      ...
      options: [
        ...
        ...(categoriesData?.map((cat: Category) => ({ value: cat.id.toString(), label: cat.name })) || []),
      ],
    },
    {
      id: 'prefix',
  ...
  const columns: ColumnDef<AdminThread>[] = [
    { key: 'id', header: 'ID', render: (thread) => <span className="font-mono">{thread.id}</span> },
    {
      ...
      render: (thread) => (
        <div className="flex items-center gap-2">
          {thread.isPinned && (
            <Badge variant="secondary" className="bg-yellow-100 text-yellow-800 border-yellow-200">Pinned</Badge>
          )}
          {thread.isLocked && (
            <Badge variant="secondary" className="bg-red-100 text-red-800 border-red-200">Locked</Badge>
          )}
    ...
    },
    {
      ...
      render: (thread) =>
        thread.prefix ? (
          <Badge style={{ backgroundColor: thread.prefix.color || '#3366ff', color: isLightColor(thread.prefix.color) ? '#000' : '#fff' }}>
            {thread.prefix.name}
          </Badge>
        ) : (
      ...
      render: (thread) => (
        <div className="flex items-center gap-2">
          {thread.user?.avatarUrl && (
            <img src={thread.user.avatarUrl} alt={thread.user.username} className="w-6 h-6 rounded-full object-cover" />
          )}
    ...
    >
      <div className="space-y-4">
        ...
        {!isLoadingThreads && threadsData?.totalPages && threadsData.totalPages > 1 && (
           <div className="mt-6 flex justify-center"> {/* Placeholder for pagination */}
             <p className="text-sm text-muted-foreground">
               Page {threadsData.page} of {threadsData.totalPages} (Total: {threadsData.total} threads)
             </p>
