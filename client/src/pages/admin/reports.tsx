...
import { Eye, CheckCircle, X, MessageSquareOff, AlertOctagon, UserX, Flag as ReportFlagIcon, AlertTriangle } from 'lucide-react'; // Removed Filter, Search
...
import type { Report, ReportStatus, ReportType } from '@/components/admin/forms/reports/ViewReportDialog';
import { ReportActionDialog, BanUserDialog } from '@/components/admin/forms/reports/ReportActionDialogs';
...
const getTypeBadge = (type: ReportType) => {
  switch (type) {
    ...
    case 'message': return <Badge variant="outline"><ReportFlagIcon className="h-3 w-3 mr-1" /> Message</Badge>;
...
export default function AdminReportsPage() {
  ...
  const reportActionMutation = useMutation({
    mutationFn: ({ reportId, action, notes }: { reportId: number; action: string; notes?: string; }) => 
      apiRequest({ url: `/api/admin/reports/${reportId}/${action}`, method: 'POST', data: { notes } }),
    onSuccess: () => {
  ...
  const banUserMutation = useMutation({
    mutationFn: ({ userId, reason, duration }: { userId: number; reason: string; duration?: string; }) =>
      apiRequest({ url: `/api/admin/users/${userId}/ban`, method: 'POST', data: { reason, duration } }),
    onSuccess: () => {
  ...
  const deleteContentMutation = useMutation({
    mutationFn: ({ type, contentId, reason }: { type: ReportType; contentId: number; reason: string; }) => {
      let endpoint = '';
      switch (type) {
        case 'post': endpoint = `/api/admin/posts/${contentId}`; break;
        case 'thread': endpoint = `/api/admin/threads/${contentId}`; break;
        case 'message': endpoint = `/api/admin/shoutbox/messages/${contentId}`; break;
        default: throw new Error('Invalid content type for deletion');
      }
    ...
    onSuccess: (_, variables) => {
      ...
      if (selectedReport) { // If deleting from view dialog, also resolve the report
        handlePerformReportAction(selectedReport.id, 'resolve', `Content deleted. Original notes: ${actionNotes}`);
      }
    },
    onError: (err: unknown) => {
  ...
  const handlePerformReportAction = (reportId?: number, action?: 'resolve' | 'dismiss', notes?: string) => {
    ...
    if (finalReportId) {
      reportActionMutation.mutate({ reportId: finalReportId, action: finalAction, notes: finalNotes });
    }
  };
  ...
  const handlePerformBanUser = () => {
    if (selectedReport) {
      banUserMutation.mutate({ userId: selectedReport.reportedUserId, reason: banReason, duration: banDuration });
    }
  };
  ...
  const columns = [
    ...
    { key: 'reason', header: 'Reason', render: (report: Report) => <span className="block max-w-[250px] truncate" title={report.reason}>{report.reason}</span> },
