					...
					<div className="flex items-center gap-2">
						{isEditing ? (
							<>
								...
								<Button onClick={handleSubmit} disabled={saveSettingsMutation.isPending}>
					...
					<TabsContent value="xp" className="space-y-4">
						<Card>
							<CardHeader>
								...
								<CardDescription>Define how users earn XP from various activities</CardDescription>
							</CardHeader>
							<CardContent>
								<form className="space-y-6" onSubmit={handleSubmit}>
									<div className="grid gap-6">
										{settings
											?.filter((s) => s.key.startsWith('xp_'))
											.map((setting) => (
												<div key={setting.key} className="grid gap-2">
													<div className="flex items-center justify-between">
														<div className="flex items-center gap-2">
															<Label htmlFor={setting.key} className="text-base">
																{settingsInfo[setting.key as keyof typeof settingsInfo]?.label ||
																	setting.key}
															</Label>
														...
														<Input
															...
															max={
																settingsInfo[setting.key as keyof typeof settingsInfo]?.max || 100
															}
															className="w-24"
														/>
													</div>
