...
export default function AdminShopItemEditPage() {
  ...
  const { data: itemData, isLoading: isLoadingItem } = useQuery<Product, Error, Product, [string, string | null | undefined]>({
  ...
  useEffect(() => {
    if (itemData && itemId) { // Editing an existing item
    ...
    } else if (!itemId) { // Creating a new item
      setName('');
      setDescription('');
      setPriceDGT('');
      setPointsPrice('');
  ...
  if (isLoadingItem && itemId) return <AdminLayout><div className="p-4">Loading item details...</div></AdminLayout>;

  return (
    <AdminLayout>
      <div className="p-6">
        <Link href="/admin/shop" className="inline-flex items-center text-sm text-zinc-400 hover:text-white mb-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-zinc-900 border border-zinc-800 p-6 rounded-lg">
          <div>
            ...
            <Input id="name" value={name} onChange={(e) => setName(e.target.value)} required className="bg-zinc-800 border-zinc-700 text-white" />
          </div>
          <div>
            <Label htmlFor="description" className="text-zinc-300">Description</Label>
            <Textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} className="bg-zinc-800 border-zinc-700 text-white" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <Label htmlFor="priceDGT" className="text-zinc-300">Price (DGT)</Label>
              <Input id="priceDGT" type="number" value={priceDGT} onChange={(e) => setPriceDGT(e.target.value)} required className="bg-zinc-800 border-zinc-700 text-white" />
            </div>
            <div>
              <Label htmlFor="pointsPrice" className="text-zinc-300">Points Price (optional)</Label>
              <Input id="pointsPrice" type="number" value={pointsPrice} onChange={(e) => setPointsPrice(e.target.value)} className="bg-zinc-800 border-zinc-700 text-white" />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <Label htmlFor="stockLimit" className="text-zinc-300">Stock Limit (leave blank for unlimited)</Label>
              <Input id="stockLimit" type="number" value={stockLimit === null ? '' : stockLimit} onChange={(e) => setStockLimit(e.target.value === '' ? null : e.target.value)} className="bg-zinc-800 border-zinc-700 text-white" />
            </div>
            <div>
              <Label htmlFor="rarity" className="text-zinc-300">Rarity</Label>
          ...
          <div>
            <Label htmlFor="cosmeticType" className="text-zinc-300 mb-2">Cosmetic Type (Quick Template)</Label>
          ...
          <div>
            <Label htmlFor="pluginReward" className="text-zinc-300">Plugin Reward (JSON)</Label>
            <Textarea
              id="pluginReward"
              value={pluginReward}
          ...
          <div className="flex justify-end">
            <Button type="submit" className="bg-emerald-600 hover:bg-emerald-500" disabled={mutation.isPending}>
