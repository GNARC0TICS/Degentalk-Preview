...
import { Product } from '@/../db/schema'; // Assuming Product type can be imported
...
export default function AdminShopItemsPage() {
  ...
  if (isLoading) return <AdminLayout><div className="p-4">Loading shop items...</div></AdminLayout>;
  if (isError) return <AdminLayout><div className="p-4 text-red-500">Error loading items: {error?.message}</div></AdminLayout>;
          ...
          <table className="min-w-full divide-y divide-zinc-700">
            <thead className="bg-zinc-800">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Name</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Price (DGT)</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Stock</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Status</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-zinc-300 uppercase tracking-wider">Plugin Reward</th>
            ...
            <tbody className="bg-zinc-900 divide-y divide-zinc-800">
              {products?.map((product) => (
                <tr key={product.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-400">{product.price}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-zinc-400">{product.stockLimit === null ? 'Unlimited' : product.stock}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${product.status === 'published' && !product.isDeleted ? 'bg-green-800 text-green-300' : 'bg-red-800 text-red-300'}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-xs text-zinc-500">
                    <pre className="max-w-xs overflow-x-auto bg-zinc-800 p-1 rounded text-[10px]">{formatPluginReward(product.pluginReward)}</pre>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <Link href={`/admin/shop/edit/${product.id}`}>
                      <Button variant="ghost" size="sm" className="text-blue-400 hover:text-blue-300 mr-2">
                        <Edit className="h-4 w-4" />
                      </Button>
                    </Link>
                    <Button variant="ghost" size="sm" className="text-red-400 hover:text-red-300" onClick={() => handleDelete(product.id!)} disabled={deleteMutation.isPending}>
