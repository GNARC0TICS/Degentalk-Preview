...
export default function CloutGrantsAdminPage() {
  ...
  const handleGrant = () => {
    ...
    grantMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: 'Clout granted', description: `User ${selectedUser.username} received ${amount} clout.` });
          setAmount(100);
          setReason('');
        },
        onError: (err: any) => {
          toast({ title: 'Error', description: err.message || 'Failed to grant clout', variant: 'destructive' });
        }
      }
    );
  };
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              ...
              {logsData?.logs?.length === 0 && <tr><td className="p-2">No logs found.</td></tr>}
            </TableBody>
          </Table>
        </CardContent>
      </Card>
