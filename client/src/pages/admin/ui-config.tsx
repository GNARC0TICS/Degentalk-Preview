...
import { Search, Plus, Edit, Trash2, Eye, EyeOff, Download, Upload, MoreHorizontal } from 'lucide-react';
import { toast } from '@/hooks/use-toast';
import { uiConfigApi, type CreateQuoteData, type QuoteFilters, type PaginationOptions } from '@/features/admin/services/uiConfigApi';
...
const UiConfigPage: React.FC = () => {
    ...
    const handleExport = async () => {
        try {
            const data = await uiConfigApi.exportQuotes(
                { type: [activeTab] },
                'json',
                true
            );
        ...
        <div className="container mx-auto p-6">
                                        ...
                                        <TableBody>
                                            {quotes.map((quote) => (
                                                <TableRow key={quote.id}>
                                                    ...
                                                    <TableCell>
                                                        <Badge
                                                            variant={quote.intensity && quote.intensity > 3 ? 'destructive' : 'secondary'}
                                                        >
                                                            {quote.intensity || 1}
                                                        </Badge>
                                                    </TableCell>
            ...
            <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
                <DialogContent className="max-w-2xl">
                    <DialogHeader>
                        <DialogTitle>Create New {activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Quote</DialogTitle>
                    </DialogHeader>
                    <QuoteForm
                        data={formData}
