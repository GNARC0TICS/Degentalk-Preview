...
import type { PostWithUser } from '@shared/types';
...
export default function ThreadPage() {
	...
	if (isError || !data) {
		return (
			<ErrorDisplay
				...
				description={error?.message || 'This thread does not exist.'}
			/>
		);
	}
	...
	console.debug('Solve permissions:', {
		isOP,
		isAdmin,
		isMod,
		canSolve,
		userId: user?.id,
		threadOwnerId: thread.user?.id
	});
