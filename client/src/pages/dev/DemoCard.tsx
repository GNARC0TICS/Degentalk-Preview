...
export function DemoCard<T extends Record<string, any>>({ id, title, initialProps, render, className = '' }: DemoCardProps<T>) {
  ...
  return (
    ...
    >
      ...
      {isActive && (
        <span className="absolute top-2 right-2 text-emerald-400 text-xs">Active</span>
      )}
    </div>
  );
} 