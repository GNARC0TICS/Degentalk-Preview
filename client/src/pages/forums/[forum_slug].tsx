...
export default function ForumDetailPage() {
	...
	const isLoading =
		isLoadingForum ||
		(!!forumSlug && !forum && !isErrorForum) ||
		(!!forum && isLoadingThreads && !threadsData && !isErrorThreads);
	...
	useEffect(() => {
		if (
			...
			(!Array.isArray(forum.forums) || forum.forums.length === 0)
		) {
			// Fetch child forums by parentId (forum.id)
			console.log(
				`[ForumPage] Container detected, fetching child forums for ${forum.name} (ID: ${forum.id})`
			);
			...
			fetch(`/api/forum/forums?parentId=${forum.id}`)
				...
				.then((data) => {
					...
					if (forumsArray.length > 0) {
						...
						console.log(
							'[ForumPage] Trying alternative endpoint /api/forum/forums/' + forum.slug + '/topics'
						);
						fetch(`/api/forum/forums/${forum.slug}/topics`)
							...
							.then((topicsData) => {
								...
								if (topicsData.topics && Array.isArray(topicsData.topics)) {
									console.log(
										`[ForumPage] Setting ${topicsData.topics.length} topics from alternative endpoint`
									);
		...
		return (
			<div className="flex flex-col min-h-screen">
				<div className="container max-w-7xl mx-auto px-4 py-6 flex-grow">
					...
					/>
					<div className="mt-4">
						<Button onClick={() => setLocation(ROUTES.FORUMS)}>Return to Forums</Button>
					</div>
				</div>
				<SiteFooter />
			</div>
							...
							{childForums.length > 0 ? (
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									{childForums.map((childForum: any) => (
										...
										>
											<h3 className="text-emerald-400 font-medium text-lg mb-1">
												{childForum.name}
											</h3>
			...
			<div className="container max-w-7xl mx-auto px-4 py-6 flex-grow">
				...
				<div className="flex flex-col lg:flex-row gap-6">
					...
					<div className="w-full lg:w-1/4 order-2 lg:order-1">
						<ForumSearch initialQuery={searchQuery} className="mb-4" onSearch={handleSearch} />
						...
						>
							<CardHeader className="pb-3">
								<div className="flex items-center justify-between">
									...
									{/* New Thread Button */}
									{isLoggedIn && (
										<CreateThreadButton categoryId={forum.id} className="hidden md:flex" />
									)}
								</div>
								{forum.description && (
							...
							<CardContent className="pb-4">
								<div className="flex items-center gap-4 text-sm">
									<span className="text-zinc-400">
										Threads: <span className="text-zinc-200">{pagination.totalThreads || 0}</span>
									</span>
									<span className="text-zinc-400">
										Posts: <span className="text-zinc-200">{forum.postCount || 0}</span>
									</span>
								</div>
								...
								{isLoggedIn && (
									<div className="mt-4 md:hidden">
										<CreateThreadButton categoryId={forum.id} className="w-full" />
									</div>
								)}
							</CardContent>
						</Card>
