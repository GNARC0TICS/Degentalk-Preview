...
import { primaryZonesArray } from '@/constants/primaryZones.tsx';
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {primaryZonesArray
              ...
              .map(zone => {
                // Helper function to get access control text
                const getAccessText = () => {
                  if (zone.accessControl.canPost.includes('admin')) return '🔒 Admin Only';
                  if (zone.accessControl.canPost.includes('token_holder')) return '💎 Token Holders';
                  return '🌍 Open to All';
                };

                return (
                  <Link key={zone.id} href={`/${zone.slug}`}>
                    <div className={`${zone.gradient} rounded-xl p-6 hover:opacity-90 transition-all duration-300 cursor-pointer group border border-zinc-700/50 hover:border-zinc-600/50`}>
                      <div className="flex items-center justify-between mb-4">
                        <div className="w-12 h-12 bg-black/20 rounded-lg flex items-center justify-center">
                          <span className="text-2xl">{zone.icon}</span>
                        </div>
                        <div className="text-xs text-zinc-300/80">PRIMARY</div>
                      </div>
                      <h3 className="text-xl font-bold text-white mb-2 group-hover:text-white/90 transition-colors">
                        {zone.label}
                      </h3>
                      <p className="text-zinc-200/80 text-sm mb-4">
                        {zone.description}
                      </p>
                      <div className="flex items-center justify-between text-xs text-zinc-300/70">
                        <span>{getAccessText()}</span>
                        <span>{zone.stats?.threadCount || 0} threads</span>
                      </div>
                    </div>
                  </Link>
                );
