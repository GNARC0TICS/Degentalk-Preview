      ...
      try {
        ...
        if (!response.ok) {
           ...
           if (response.status === 404) {
              ...
              console.error(`[ForumPage] Error fetching forum data for slug: ${slug}`, response.status, errorBody);
           }
           setForum(null);
           return;
        }
        ...
        if (!result || !result.forum) {
           ...
           setForum(null);
           console.log(`[ForumPage] Forum not found or invalid response for slug: ${slug}. Full result:`, result);
           return;
        }
        ...
        if (result.topics && Array.isArray(result.topics) && result.topics.length > 0) {
            ...
            if (result.forum.id) {
              ...
              if (!threadsResponse.ok) {
                 const errorBody = await threadsResponse.json().catch(() => ({ message: threadsResponse.statusText }));
                 const errorMessage = errorBody.message || `HTTP error! status: ${threadsResponse.status}`;
                 setError(`Failed to load threads: ${errorMessage}`);
                 console.error(`[ForumPage] Error fetching threads for forum ID ${result.forum.id}:`, threadsResponse.status, errorBody);
                 setThreads([]);
                 return;
              }
              ...
              if (Array.isArray(threadsData)) {
                 ...
                 console.log(`[ForumPage] Threads loaded for forum ID ${result.forum.id}:`, threadsData.length);
              } else {
        ...
        {childForums.length > 0 ? (
          ...
          <div className="mb-8">
            ...
            <div className="space-y-4">
              {childForums.map(childForum => (
                <Card key={childForum.id} className="bg-zinc-900/50 border border-zinc-800 rounded-lg overflow-hidden">
                  <CardHeader>
                    <CardTitle>
                      {/* Link to the child forum page */}
                      <Link href={`/forum/${childForum.slug}`} className="text-white">{childForum.name}</Link>
                    </CardTitle>
                    <CardDescription className="text-zinc-400">{childForum.description}</CardDescription>
                  </CardHeader>
                  <CardContent>
          ...
          <div className="mb-8">
            ...
            {threads.length > 0 ? (
              <div className="space-y-4">
                {threads.map(thread => (
                  <Card key={thread.id} className="bg-zinc-900/50 border border-zinc-800 rounded-lg overflow-hidden">
                    <CardHeader>
                      <CardTitle>
                        <Link href={`/forum/${slug}/threads/${thread.id}`} className="text-white">{thread.title}</Link>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
