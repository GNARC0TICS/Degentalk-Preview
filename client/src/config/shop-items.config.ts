	...
	cosmetics: {
		...
		subcategories: {
			usernameColors: {
				...
				type: 'usernameColor' as const
			},
			avatarFrames: {
				id: 'avatar-frames',
				name: 'Avatar Frames',
				type: 'avatarFrame' as const
			},
			userTitles: {
				id: 'user-titles',
				name: 'User Titles',
				type: 'userTitle' as const
			}
		}
	}
} as const;
 ...
 */
export function getItemsByType<T extends ShopItem['type']>(
	type: T
): Extract<ShopItem, { type: T }>[] {
	return defaultShopItems.filter((item) => item.type === type) as Extract<ShopItem, { type: T }>[];
}
 ...
 */
export function isUsernameColorItem(item: ShopItem): item is UsernameColorItem {
	return item.type === 'usernameColor';
}

export function isAvatarFrameItem(item: ShopItem): item is AvatarFrameItem {
	return item.type === 'avatarFrame';
}

export function isUserTitleItem(item: ShopItem): item is UserTitleItem {
