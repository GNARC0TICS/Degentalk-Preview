...
export const parseDgt = (raw: string | number | bigint): number => Number(raw) / 1e8;

export const formatDgt = (ui: number): string =>
	ui.toLocaleString(undefined, { maximumFractionDigits: 2 }) + ' DGT';

export const parseXp = (raw: string | number): number => Number(raw);

export const formatXp = (xp: number): string => xp.toLocaleString() + ' XP';
