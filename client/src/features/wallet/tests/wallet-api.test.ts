...
describe('Wallet API Service', () => {
	...
	describe('createDgtPurchase', () => {
		it('should create a DGT purchase order', async () => {
			...
			expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/api/wallet/dgt-purchase'), {
				dgtAmount: 1000,
				cryptoCurrency: 'USDT'
			});
		});
	...
	describe('transferDgt', () => {
		it('should transfer DGT to another user', async () => {
			...
			expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/api/wallet/transfer'), {
				toUserId: 2,
				amount: 100,
