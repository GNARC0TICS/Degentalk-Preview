...
export interface UseWalletReturn {
	...
	) => Promise<{ purchaseOrderId: string; paymentLink: string } | null>;
}
...
export function useWallet(): UseWalletReturn {
	...
	const getDepositAddress = useCallback(
		async (currency: string): Promise<DepositAddress | null> => {
			try {
				setIsLoading(true);
	...
	const transferDgt = useCallback(
		async (recipientId: number, amount: number, reason?: string): Promise<boolean> => {
			try {
				setIsLoading(true);
	...
	const createDgtPurchase = useCallback(
		async (dgtAmount: number, cryptoCurrency: string) => {
			try {
				setIsLoading(true);
