      ...
      if (userId && threadId) {
        ...
        try {
          ...
          if (dgtAmountToAward > 0) {
            const dgtResponse = await apiPost<{ dgtAwarded: number }>('/api/economy/transactions/create', {
              userId,
              currency: 'DGT',
              type: 'reward',
...
export const useUnsolveThread = () => {
  ...
  return useMutation({
    mutationFn: (threadId: number) => 
      forumApi.unsolveThread(threadId),
 ...
 * Posts
 */
export const usePosts = (threadId: number | undefined, params?: { page?: number; limit?: number }) => {
  return useQuery({
    queryKey: [`/api/threads/${threadId}/posts`, params],
    queryFn: () => threadId ? forumApi.getPosts(threadId, params) : Promise.reject("Thread ID is required"),
    enabled: !!threadId,
  });
};
...
export const useUpdatePost = (postId: number | undefined) => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ predicate: (query) => typeof query.queryKey[0] === 'string' && query.queryKey[0].includes('/api/threads/') && query.queryKey[0].includes('/posts') });
...
export const useThreadsByTag = (tagSlug: string | undefined, params?: { page?: number; limit?: number; sortBy?: 'latest' | 'hot' | 'staked' }) => {
  return useQuery({
    queryKey: [`/api/forum/tags/${tagSlug}/threads`, params],
    queryFn: () => tagSlug ? forumApi.getThreadsByTag(tagSlug, params) : Promise.reject("Tag slug is required"),
    enabled: !!tagSlug,
  });
};
