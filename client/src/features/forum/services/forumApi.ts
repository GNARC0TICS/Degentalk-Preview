...
export const forumApi = {
  ...
  getForumBySlug: async (slug: string): Promise<{
  ...
  getForumBySlugWithTopics: async (slug: string): Promise<{
    forum: ForumCategoryWithStats;
  ...
  getThread: async (slugOrId: string | number): Promise<ThreadWithUser> => {
    ...
    const url = isSlug 
      ? `/api/forum/threads/slug/${slugOrId}`
      : `/api/forum/threads/${slugOrId}`;
  ...
  updateThread: async (threadId: number, data: {
    title?: string;
    categoryId?: number;
    prefixId?: number;
    isLocked?: boolean;
  ...
  getThreadsByTag: async (tagSlug: string, params?: { 
    page?: number; 
    limit?: number; 
    sortBy?: 'latest' | 'hot' | 'staked' | 'popular' | 'recent';
   ...
   * Posts (Replies)
   */
  getPosts: async (threadId: number, params?: { 
    page?: number; 
    limit?: number;
  }): Promise<{ 
    posts: PostWithUser[];
  ...
  tipPost: async (postId: number, amount: number): Promise<{ 
    success: true;
    tip: {
      amount: number;
      burnAmount: number;
