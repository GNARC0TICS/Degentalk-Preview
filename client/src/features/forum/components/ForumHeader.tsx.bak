  ...
  if (isPrimaryZone) {
    return (
      <div className={`forum-header primary-zone-header ${themeClass} mb-6 ${className}`}>
        <div className={`rounded-2xl p-6 ${forum.colorTheme ? `bg-gradient-to-r from-${forum.colorTheme}-900/60 to-${forum.colorTheme}-800/40` : 'bg-zinc-900/80'} border-2 ${forum.colorTheme ? `border-${forum.colorTheme}-600/40` : 'border-zinc-700'}`}>
          <div className="flex items-center mb-4">
            {renderIcon()}
            <div>
              <h1 className="text-2xl font-bold text-white">{forum.name}</h1>
              <div className="flex items-center mt-1">
                <Badge className={`mr-2 ${forum.colorTheme ? `bg-${forum.colorTheme}-700 hover:bg-${forum.colorTheme}-600` : 'bg-zinc-700 hover:bg-zinc-600'}`}>
                  Primary Zone
                </Badge>
                {forum.hasXpBoost && (
          ...
          {forum.description && (
            <p className="text-zinc-300 mb-4">{forum.description}</p>
          )}
  ...
  return (
    <Card className={`forum-header regular-forum-header mb-6 ${className}`}>
      <CardContent className="p-6">
        ...
        {forum.description && (
          <p className="text-zinc-300 mb-3">{forum.description}</p>
        )}
