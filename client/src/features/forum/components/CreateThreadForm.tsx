...
export function CreateThreadForm({
  ...
  } = usePrefixesByCategory(selectedCategoryId);
  ...
  const { // Ensure draftData and isDraftLoadSuccess are destructured
    data: draftData, 
    isLoading: loadingDraft,
    isSuccess: isDraftLoadSuccess
  } = useQuery(queryOptions);

  const form = useForm<ThreadFormValues>({ // Moved form declaration before titleValue
  ...
  const saveDraftMutation = useMutation({
    mutationFn: async (values: ThreadFormValues & { draftId?: number }) => {
      ...
      return apiRequest<any>({ // Updated
        method, 
        url: endpoint, 
        data: {
          ...values,
          categoryId: parseInt(values.categoryId),
          prefixId: values.prefixId && values.prefixId.trim() !== '' ? parseInt(values.prefixId) : undefined,
        }
      });
    },
    onSuccess: (data) => {
  ...
  const handleSaveDraft = useCallback((html: string, json: any) => {
  ...
  const onSubmit = (values: ThreadFormValues) => {
    if (!user) {
      toast({ title: "Authentication Error", description: "You must be logged in to create a thread.", variant: "destructive" });
      return;
    }
    if (!userCanCreateThread) {
      toast({ title: "Permission Denied", description: "You do not have permission to create a thread in this category.", variant: "destructive" });
      return;
    }
  ...
  useEffect(() => {
    if (editorContent || titleValue) { // Trigger debounce only if there's content or title
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[600px] md:max-w-[800px] lg:max-w-[1000px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {hasActiveDraft ? 'Continue editing your saved draft or start fresh.' : 'Fill in the details below to start a new discussion.'}
          </DialogDescription>
        </DialogHeader>
        ...
        {!loadingPermissions && !userCanCreateThread && (
          <Alert variant="destructive">
            <AlertDescription>
              You do not have permission to create threads in this category. Please contact an administrator if you believe this is an error.
            </AlertDescription>
          </Alert>
        )}
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        ...
                        <SelectContent>
                          {loadingCategories && <SelectItem value="loading" disabled>Loading categories...</SelectItem>}
                          {categories?.map((cat: any) => ( // TODO: Type categories properly
                            <React.Fragment key={cat.id}>
                              <SelectItem value={cat.id.toString()} className="font-bold" disabled>
                                {cat.name} (Forum)
                              </SelectItem>
                              {cat.children?.map((topic: any) => ( // TODO: Type topics properly
                                <SelectItem key={topic.id} value={topic.id.toString()} className="ml-4">
                                  {topic.name}
                                </SelectItem>
                              ))}
                            </React.Fragment>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      >
                        <FormControl>
                          <SelectTrigger>
                            {field.value && Array.isArray(prefixes) && (prefixes as Prefix[]).find((p: Prefix) => p.id.toString() === field.value) ? (
                              <PrefixBadge prefix={(prefixes as Prefix[]).find((p: Prefix) => p.id.toString() === field.value)} />
                            ) : (
                        ...
                        <SelectContent>
                          ...
                          {Array.isArray(prefixes) && (prefixes as Prefix[]).map((prefix: Prefix) => (
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <TagInput
                        value={(field.value || []).map((tagName): Tag => ({ 
                          name: tagName, 
              ...
              <DialogFooter className="gap-2 sm:justify-end">
                <Button type="button" variant="outline" onClick={onClose} disabled={createThreadMutation.isPending}>
                  Cancel
                </Button>
                <Button 
                  type="button" 
                ...
                <Button type="submit" disabled={createThreadMutation.isPending || saveDraftMutation.isPending}>
