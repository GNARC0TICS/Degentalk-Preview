 ...
 */
export function invalidateQueries(client: QueryClient, keys: readonly unknown[]): Promise<void> {
...
export async function batchInvalidateQueries(
	...
	await Promise.all(keysList.map((keys) => client.invalidateQueries({ queryKey: keys })));
}
