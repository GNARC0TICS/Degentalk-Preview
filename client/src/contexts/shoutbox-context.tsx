...
export type ShoutboxPosition = 'sidebar-top' | 'sidebar-bottom' | 'main-top' | 'main-bottom' | 'floating';
...
export function ShoutboxProvider({ children }: ShoutboxProviderProps) {
  ...
  const updatePositionMutation = useMutation({
    mutationFn: async (newPosition: ShoutboxPosition) => {
      return apiRequest(
        'PUT',
        '/api/settings/shoutbox-position',
  ...
  useEffect(() => {
    ...
    if (positionChangeCount > 0) {
      console.log(`Viewport changed to ${isMobile ? 'mobile' : 'desktop'} view - Shoutbox adjusted`);
    }
  ...
  return (
    <ShoutboxContext.Provider value={value}>
      {children}
