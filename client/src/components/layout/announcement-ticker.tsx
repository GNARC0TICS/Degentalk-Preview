...
import { 
  AlertCircle, TrendingUp, Flame, Award, 
  Megaphone, Info, Bell, Star, Zap, BarChart3 
} from 'lucide-react';
...
const getIconComponent = (iconName?: string, type: string = 'info', textColor?: string) => {
  ...
  switch (iconName) {
    case 'megaphone': return <Megaphone className={iconClasses} />;
    case 'alert': return <AlertCircle className={iconClasses} />;
    case 'bell': return <Bell className={iconClasses} />;
    case 'trending': return <TrendingUp className={iconClasses} />;
    ...
    case 'chart': return <BarChart3 className={iconClasses} />;
    default:
          ...
          <div className={`ticker-content ${isHovered ? 'paused' : ''}`}>
            {displayAnnouncements.map((announcement, index) => {
              ...
              return announcement.link ? (
                ...
                <span key={`ann-${announcement.id}-${index}`}><AnnouncementContent /></span>
              );
            })}
            ...
            {displayAnnouncements.map((announcement, index) => {
              ...
              return announcement.link ? (
                ...
                <span key={`ann-dup-${announcement.id}-${index}`}><AnnouncementContent /></span>
              );
            })}
          </div>
        </div>
