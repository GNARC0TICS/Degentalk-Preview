...
export function ThreadCard({ thread, className = '' }: ThreadCardProps) {
  ...
  return (
    <Card className={`bg-zinc-900/60 border border-zinc-800 hover:bg-zinc-900 hover:border-zinc-700 transition-all duration-200 ${className}`}>
      <div className="p-4">
        {/* Thread Header */}
        <div className="flex gap-3">
          {/* User Avatar */}
          <Avatar className="h-10 w-10 border border-zinc-700">
            <AvatarImage src={user?.activeAvatarUrl || user?.avatarUrl || ''} alt={user?.username || 'User'} />
          ...
          <div className="flex-1 min-w-0">
                ...
                {prefix && (
                  <Badge 
                    className={`px-2 py-0.5 text-xs ${
                      prefix.color ? `bg-${prefix.color}-900/60 text-${prefix.color}-300 border-${prefix.color}-700/30` 
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-xs text-zinc-400">
              ...
              {/* Category */}
              {category && (
                <Link href={`/forums/${category.slug}`} className="hover:text-zinc-200 transition-colors flex items-center">
