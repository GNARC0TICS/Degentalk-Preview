...
import { useZoneMetrics } from '@/features/forum/hooks/useZoneMetrics.ts'; // Added .ts extension
import { useShopConfig } from '@/hooks/useShopConfig.ts'; // Import useShopConfig and added .ts extension
import { DgtPackage } from '@/config/economy.config.ts'; // Import DgtPackage type
...
=======
  ...
  const { currentShopItem, getCategoryTags } = useShopConfig(); // Use the hook
  ...
  const gridData: GridCardData[] = [
    ...
    // Add the shop card if included and an item is available
    ...(includeShopCard && currentShopItem ? [{
      id: 'shop-card',
      type: 'shop',
      isStatic: true,
      featuredItem: { // Map DgtPackage to ShopCardData['featuredItem']
        name: currentShopItem.name,
        price: currentShopItem.dgtAmount, // Use DGT amount as price for display
        image: currentShopItem.icon, // Use icon as image for now
      }
    } as ShopCardData] : [])
  ];
                ...
                <div className="mt-2">
                  ...
                  {Array.isArray(metrics.hotThreads) && metrics.hotThreads.length === 0 ? (
                    ...
                    <ul className="space-y-1">
                      {Array.isArray(metrics.hotThreads) && metrics.hotThreads.map((ht: { id: string | number; title: string; replies: number; views: number; }) => ( // Explicitly typed ht
