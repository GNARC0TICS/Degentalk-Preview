...
export function PostCard({
  ...
  const editedTimeAgo = post.isEdited && post.updatedAt
    ? formatDistanceToNow(new Date(post.updatedAt), { addSuffix: true })
    : null;
  
  return (
    <Card className={`mb-4 bg-zinc-900/60 border ${isSolution ? 'border-emerald-800/70' : 'border-zinc-800'} ${isSolution ? 'ring-1 ring-emerald-500/30' : ''}`}>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <Link href={`/profile/${post.user.id}`}>
              <span className="font-medium text-zinc-200 hover:text-emerald-400 transition-colors cursor-pointer">{post.user.username}</span>
            </Link>
