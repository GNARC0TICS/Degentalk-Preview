...
export interface EnhancedZoneCardProps {
	zone: {
		id: string;
		name: string;
		slug: string;
...
const EnhancedZoneCard = memo(
	({
		...
		className,
		onEnter
	}: EnhancedZoneCardProps) => {
		const { getZone } = useForumStructure();
		const derivedZone = React.useMemo(() => {
			if (zone.forums && zone.forums.length > 0) return zone;
...
EnhancedZoneCard.displayName = 'EnhancedZoneCard';

export default EnhancedZoneCard;
