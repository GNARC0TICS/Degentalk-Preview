		...
		<div className={cn('flex items-center', className)}>
			<Link href={`/profile/${username}`} className="flex items-center group">
				...
				<div className="flex flex-col">
					<div className="flex items-center gap-1.5">
						<span
							className={cn(
								...
								isAdmin ? 'text-red-400' : isModerator ? 'text-blue-400' : 'text-zinc-100'
							)}
						>
							{username}
						</span>
						...
						{/* Level Badge */}
						{showLevelBadge && (
							<LevelBadge level={level} size={getLevelBadgeSize() as 'xs' | 'sm' | 'md'} />
						)}
					...
					{/* User Title */}
					{title && (
						<span className={cn('text-xs font-medium', getTitleColorClass(title.color))}>
							{title.name}
						</span>
					)}
				</div>
