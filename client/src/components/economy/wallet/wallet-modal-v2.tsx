...
export function WalletModalV2({ open, onOpenChange }: WalletModalProps) {
  ...
  const { balance, isLoadingBalance, transactions, isLoadingTransactions, refreshBalance, refreshTransactions } = useWallet(); // Use useWallet hook
          ...
          <div className="flex-1 overflow-y-auto">
             <TabsContent value="overview" className="p-5 space-y-6 h-full">
              ...
              {wallet.pendingWithdrawals && wallet.pendingWithdrawals.length > 0 && ( <div className="bg-black/30 rounded-lg p-4 border border-zinc-800 space-y-2"> <h3 className="text-sm text-zinc-400 mb-2">Pending Withdrawals</h3> {wallet.pendingWithdrawals.map((withdrawal: any, index: number) => ( <div key={index} className="flex items-center justify-between"> <div className="flex items-center"> <span className="text-zinc-300">${withdrawal.amount.toFixed(2)}</span> </div> <span className="text-xs text-zinc-500"> {new Date(withdrawal.timestamp).toLocaleString()} </span> </div> ))} </div> )} 
               <div className="bg-black/50 rounded-lg p-4 border border-zinc-800"> <h3 className="text-sm text-zinc-400 mb-2">Quick Actions</h3> <div className="grid grid-cols-2 gap-2 mb-2"> <DepositButton variant="small" onClick={() => setActiveTab('deposit')} /> <WithdrawButton walletBalanceUSDT={wallet.walletBalanceUSDT} dgtPoints={wallet.dgtPoints} variant="small" onClick={() => setActiveTab('withdraw')} /> </div> <div className="grid grid-cols-2 gap-2 mb-2"> <TipButton variant="small" onClick={() => setActiveTab('tip')} /> <RainButton variant="small" onClick={() => setActiveTab('rain')} /> </div> <div className="grid grid-cols-1 gap-2"> <BuyDgtButton dgtPoints={wallet.dgtPoints} walletBalanceUSDT={wallet.walletBalanceUSDT} variant="small" onClick={() => setActiveTab('buy-dgt')} /> </div> </div> </TabsContent>
            <TabsContent value="deposit" className="p-5 space-y-6"> <DepositButton walletAddress={wallet.walletAddress} onCopy={copyWalletAddress} copiedAddress={copiedAddress} /> </TabsContent>
            <TabsContent value="withdraw" className="p-5 space-y-6"> <WithdrawButton walletBalanceUSDT={wallet.walletBalanceUSDT} /> </TabsContent>
            <TabsContent value="tip" className="p-5 space-y-6"> <TipButton dgtPoints={wallet.dgtPoints} walletBalanceUSDT={wallet.walletBalanceUSDT} /> </TabsContent>
            <TabsContent value="rain" className="p-5 space-y-6"> <RainButton dgtPoints={wallet.dgtPoints} walletBalanceUSDT={wallet.walletBalanceUSDT} /> </TabsContent>
            <TabsContent value="buy-dgt" className="p-5 space-y-6"> <BuyDgtButton dgtPoints={wallet.dgtPoints} walletBalanceUSDT={wallet.walletBalanceUSDT} variant="small" onClick={() => setActiveTab('buy-dgt')} /> </TabsContent>
            <TabsContent value="history" className="p-5"> <TransactionHistory /> </TabsContent>
          </div>
        </Tabs>
