...
export function WithdrawButton({
	...
	const withdrawMutation = useMutation({
		mutationFn: async (data: { amount: number; currency: 'USDT' | 'DGT'; address?: string }) => {
			...
			<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
				<TabsList className="w-full grid grid-cols-2 bg-zinc-900">
					...
					>
						...
						<Badge variant="outline" className="ml-2 bg-zinc-900 text-emerald-500">
							${walletBalanceUSDT?.toFixed(2) || '0.00'}
						</Badge>
					</TabsTrigger>
					<TabsTrigger
						value="dgt"
				...
				<TabsContent value="usdt" className="space-y-4 mt-4">
					<div>
						...
						<p className="text-xs text-zinc-500 mt-1">Minimum withdrawal: 20 USDT</p>
					</div>
				...
				<TabsContent value="dgt" className="space-y-4 mt-4">
					<div>
						...
						<p className="text-xs text-zinc-500 mt-1">Minimum conversion: 1,000 DGT</p>
					</div>
			...
			<div className="flex justify-end pt-2">
				<Button
					...
					disabled={
						...
						(activeTab === 'dgt' &&
							(parseFloat(withdrawAmount) > dgtPoints || parseFloat(withdrawAmount) < 1000))
					}
