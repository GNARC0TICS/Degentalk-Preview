  ...
  const handleReaction = async (reactionType: string) => {
    ...
    try {
      ...
      if (currentReactions[reactionIndex].active) {
        ...
        currentReactions[reactionIndex].count = Math.max(0, currentReactions[reactionIndex].count - 1);
      } else {
        // Add reaction (first remove any existing one)
        const activeIndex = currentReactions.findIndex(r => r.active);
        if (activeIndex !== -1) {
          // Remove previous reaction
          await apiRequest('DELETE', `/api/posts/${postId}/reactions/${currentReactions[activeIndex].type}`);
          
          currentReactions[activeIndex].active = false;
          currentReactions[activeIndex].count = Math.max(0, currentReactions[activeIndex].count - 1);
        }
      ...
      {reactions.map((reaction) => (
        reaction.type === 'like' ? (
          <Button
            ...
            onClick={handleLike}
          >
            <reaction.icon className={cn("h-4 w-4 transition-all", reaction.active && 'text-cyan-400')} />
            {reaction.count > 0 && (
          ...
          >
            ...
            {reaction.count > 0 && (
              <span className={cn(reaction.active && reaction.color)}>
                {reaction.count}
              </span>
            )}
          </Button>
      ...
      <Dialog open={tipModalOpen} onOpenChange={setTipModalOpen}>
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-zinc-900 rounded-lg p-6 w-80 flex flex-col items-center">
            ...
            <Button variant="ghost" className="mt-2 text-zinc-400" onClick={() => setTipModalOpen(false)} disabled={tipLoading}>
              Cancel
            </Button>
          </div>
        </div>
