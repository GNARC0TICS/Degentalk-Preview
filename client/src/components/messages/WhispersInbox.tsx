...
<<<<<<< HEAD
		...
		if (messages?.length) {
			...
			setTimeout(() => {
				const scrollContainer = document.querySelector(
					'.messages-scroll-area [data-radix-scroll-area-viewport]'
				);
				if (scrollContainer) {
	...
	const { handleClick: handleDeleteConversation, isLoading: isDeleting } = useAsyncButton(
		async () => {
			if (!selectedConversation) return;
										...
										>
											...
											<div className="flex-1 min-w-0">
												<div className="flex items-center justify-between">
													<div className="font-medium truncate">{conversation.username}</div>
						...
						<div className="flex items-center">
							...
							<div className="flex flex-1 items-center min-w-0">
								...
								<div className="truncate">
									...
									<CardDescription className="text-gray-400">Whisper conversation</CardDescription>
								</div>
							</div>
							<Button
								variant="ghost"
					...
					<CardContent className="pt-0">
						<ScrollArea className="messages-scroll-area h-[calc(70vh-14rem)] pr-4">
                          ...
                          `}
												>
													<div
														className={`text-sm ${!message.isRead && !isCurrentUser ? 'font-semibold' : ''}`}
													>
														{message.content}
													</div>
													<div className="flex items-center gap-2">
														<div
															className={`text-xs mt-1 ${isCurrentUser ? 'text-indigo-200/70' : 'text-gray-400'}`}
														>
