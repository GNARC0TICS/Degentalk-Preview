...
export const BanUserDialog: React.FC<BanUserDialogProps> = ({ isOpen, onClose, user, onConfirm }) => {
  if (!user) return null;
  return (
...
export const UnbanUserDialog: React.FC<UnbanUserDialogProps> = ({ isOpen, onClose, user, onConfirm }) => {
  if (!user) return null;
  return (
    <AlertDialog open={isOpen} onOpenChange={onClose}>
      <AlertDialogContent>
...
export const DeleteUserDialog: React.FC<DeleteUserDialogProps> = ({ isOpen, onClose, user, onConfirm }) => {
  if (!user) return null;
  return (
    <AlertDialog open={isOpen} onOpenChange={onClose}>
      <AlertDialogContent>
        ...
        <AlertDialogFooter className="flex-wrap gap-2 sm:justify-end">
          <AlertDialogCancel onClick={onClose}>Cancel</AlertDialogCancel>
          <AlertDialogAction onClick={() => onConfirm(user.id)} className="bg-red-600 hover:bg-red-700">
            Delete User
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
...
export const ChangeUserRoleDialog: React.FC<ChangeUserRoleDialogProps> = ({ isOpen, onClose, user, roles, onConfirm }) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        ...
        <DialogFooter className="flex-wrap gap-2 sm:justify-end">
          ...
          <Button type="button" onClick={handleSubmit} disabled={!selectedRole || selectedRole === user.role}>
            Save Changes
          </Button>
        </DialogFooter>
      </DialogContent>
