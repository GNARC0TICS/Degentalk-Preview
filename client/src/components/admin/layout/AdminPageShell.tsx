...
export interface TabConfig { // Export TabConfig
  value: string;
  label: string;
      ...
      {tabsConfig && tabsConfig.length > 0 && currentActiveTab ? (
        <Tabs value={currentActiveTab} onValueChange={onTabChange} className="w-full">
          <TabsList className="bg-admin-surface border border-admin-border-subtle">
            {tabsConfig.map((tab) => (
              <TabsTrigger key={tab.value} value={tab.value} className="data-[state=active]:bg-admin-bg-element data-[state=active]:text-admin-text-accent">
