...
export function StyledShoutboxMessage({ type, currency, content, sender, amount }: ShoutboxMessageStyleProps) {
  ...
  return (
    <div className={styles.wrapperClass}>
      <div className="flex items-center">
        ...
        {amount && (
          <span className={`ml-2 text-xs px-1.5 py-0.5 rounded border ${styles.amountBadgeClass} flex items-center`}>
...
export function detectMessageType(content: string): {
  ...
  const rainRegex = /^🌧️\s+@(\w+)\s+is\s+making\s+it\s+rain\s+(\d+(?:\.\d+)?)\s+(DGT|USDT)\s+on\s+(\d+)\s+lucky\s+users!$/i;
