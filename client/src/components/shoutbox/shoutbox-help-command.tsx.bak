...
import { Command, HelpCircle, CloudRain, Gift, MessageSquare, BellRing, User, Wind } from 'lucide-react';
  ...
  return (
    <div className="bg-zinc-900/80 border border-zinc-800 rounded-lg p-4 my-3">
      ...
      <div className="space-y-3">
        {commandList.map((cmd) => (
          <div key={cmd.command} className="flex items-start">
            <div className="mt-0.5 mr-3 text-zinc-400">
              {cmd.icon}
            </div>
            <div>
      ...
      <div className="mt-4 text-xs text-zinc-500 border-t border-zinc-800 pt-3">
        <p>
          <span className="text-zinc-400">Tip/Rain Limits:</span> Min DGT: 10, Min USDT: 0.1, Max Recipients: 15
        </p>
        <p className="mt-1">
          <span className="text-zinc-400">Cooldowns:</span> These commands may have cooldown periods configured by admins
        </p>
      </div>
    </div>
  );
...
export function processHelpCommand(content: string): { isHelpCommand: boolean; message?: JSX.Element } {
