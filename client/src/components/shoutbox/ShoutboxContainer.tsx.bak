...
const ShoutboxContainer: React.FC = () => {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (newMessage: string) => {
      return apiRequest({ url: '/api/shoutbox/messages', method: 'POST', data: { message: newMessage } });
    },
    onSuccess: () => {
      setMessage('');
  ...
  const lockedContent = (
    <div className="flex flex-col items-center justify-center p-8 text-center">
      ...
      <p className="text-zinc-400 mb-4 max-w-sm">
        The global shoutbox is available to members who have reached Level 3.
        Keep participating in the forum to earn XP and level up!
      </p>
      <Button 
        variant="outline" 
        onClick={() => window.location.href = '/profile/xp'}
      >
  ...
  return (
    <Card className="w-full bg-zinc-900 border-zinc-800">
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          Chat with other forum members in real-time
        </CardDescription>
      </CardHeader>
