...
export function ShoutboxWidget() {
  ...
  const { position, expansionLevel, cycleExpansionLevel, updateExpansionLevel, isMobile } = useShoutbox();
  ...
  return (
    <Card className={`bg-zinc-900/50 border border-zinc-800 overflow-hidden transition-all duration-300 flex flex-col ${
      expansionLevel === 'expanded' ? 'h-[600px]' : expansionLevel === 'preview' ? 'h-[400px]' : 'h-[200px]'
    }`}>
      <CardHeader className="pb-2 flex flex-row items-center justify-between">
        ...
        <div className="flex space-x-2">
          ...
          >
            {expansionLevel === 'expanded' ? 'Collapse' : expansionLevel === 'collapsed' ? 'Expand' : 'Full View'}
          </Button>
          <Button 
            variant="ghost" 
            size="sm" 
      ...
      <CardContent className="p-3 flex-1 overflow-hidden">
        ...
        <div 
          ...
          className={`space-y-3 overflow-y-auto mb-3 ${
            expansionLevel === 'expanded' ? 'h-[450px]' : expansionLevel === 'preview' ? 'h-[250px]' : 'h-[100px]'
          } custom-scrollbar`}
        >
          {isError ? (
            ...
            Array.isArray(messages) ? messages.map((msg: ShoutboxMessage) => (
              <div 
                key={msg.id} 
      ...
      <CardFooter className="border-t border-zinc-800 p-3 flex-col">
            ...
            <div className="absolute bottom-12 right-0 bg-zinc-900 border border-zinc-800 rounded-md p-2 shadow-lg z-10 w-64">
              ...
              <Tabs defaultValue="all" className="w-full mb-2" onValueChange={setEmojiCategory}>
                <TabsList className="grid grid-cols-5 mb-2">
                  <TabsTrigger value="all" className="text-xs py-1 px-2">All</TabsTrigger>
                  <TabsTrigger value="standard" className="text-xs py-1 px-2">Standard</TabsTrigger>
                  <TabsTrigger value="flex" className="text-xs py-1 px-2">Flex</TabsTrigger>
                  <TabsTrigger value="memes" className="text-xs py-1 px-2">Memes</TabsTrigger>
                  <TabsTrigger value="rare" className="text-xs py-1 px-2">Rare</TabsTrigger>
                </TabsList>
                  ...
                  <TabsContent key={category} value={category}>
                    {isLoadingEmojis ? (
                      ...
                      <div className="grid grid-cols-5 gap-2">
                        ...
                        {customEmojis.filter((emoji: CustomEmoji) => 
                          emoji.category?.toLowerCase() === category.toLowerCase()).length === 0 && (
        ...
        {userLoggedIn ? (
          <div className="w-full flex justify-between items-center mt-2 text-xs">
            <div className={`${message.length > 180 ? (message.length > 190 ? 'text-red-400' : 'text-yellow-400') : 'text-zinc-500'}`}>
              ...
              <div className="bg-zinc-800/50 p-4 rounded-lg w-full mt-4">
                ...
                {lockedEmojiInfo.unlockType === 'path_xp' ? (
                  <div className="space-y-2">
                    <p className="text-sm">
                      Reach <span className="text-emerald-400 font-medium">Level {lockedEmojiInfo.requiredPathXP}</span> to unlock this emoji.
                    </p>
                    <div className="bg-zinc-900 p-2 rounded text-xs text-zinc-400 mt-2">
                      Continue participating in discussions, creating threads, and earning XP to level up.
                    </div>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <p className="text-sm">
                      Purchase this emoji from the <span className="text-emerald-400 font-medium">Shop</span>.
                    </p>
                    <div className="bg-zinc-900 p-2 rounded text-xs text-zinc-400 mt-2">
