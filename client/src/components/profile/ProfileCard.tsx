...
export default function ProfileCard({ username, className }: ProfileCardProps) {
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.div key={username} variants={variants} initial="initial" animate="animate" exit="exit" className={cn(className)}>
        {isLoading && <ProfileSkeleton />}
        {isError && <ErrorDisplay title="Profile Error" error={error} variant="card" />}
        {profile && <ProfileSidebar profile={profile} isOwnProfile={currentUser?.username === username} />}
      </motion.div>
    </AnimatePresence>
  );
}
