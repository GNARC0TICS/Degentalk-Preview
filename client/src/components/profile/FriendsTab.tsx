...
const FriendsTab: React.FC<Props> = ({ profile }) => {
	...
	const renderCards = (
		...
		if (error)
			return (
				<ErrorDisplay title="Could not load list" error={error} onRetry={retry} variant="inline" />
			);
			...
			<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
				{users.map((u) => (
					<a
						...
						className="flex items-center gap-3 p-3 rounded-lg bg-slate-900 hover:bg-slate-800"
					>
	...
	return (
		<div className="space-y-6">
			...
			<div className="pt-2">
				...
				{activeTab === 'followers' &&
					renderCards(followers, loadingFollowers, followersError, refetchFollowers)}
				{activeTab === 'following' &&
					renderCards(following, loadingFollowing, followingError, refetchFollowing)}
			</div>
		</div>
	);
};
