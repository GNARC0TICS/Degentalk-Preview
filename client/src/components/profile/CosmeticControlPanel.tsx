...
interface CosmeticControlPanelProps {
  ...
  activeBadge?: { id: number; name: string; description: string | null; iconUrl: string; rarity: string } | null;
  onEditProfile?: () => void;
}
...
export function CosmeticControlPanel({
  ...
  const toggleEquipMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      toast({
        ...
        description: variables.equip ? "Your cosmetic has been equipped" : "Your cosmetic has been unequipped",
        variant: "default",
      });
  ...
  const equippedByType = {
    usernameColor: equippedItems.find(item => item.product?.pluginReward?.type === 'username_color'),
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {activeTitle ? (
                    <p className={cn(
                      'text-xs',
                      activeTitle.rarity === 'legendary' ? 'text-amber-400' :
                      activeTitle.rarity === 'epic' ? 'text-purple-400' :
                      activeTitle.rarity === 'rare' ? 'text-blue-400' :
                      'text-zinc-300'
                    )}>
                      {activeTitle.name}
                    </p>
