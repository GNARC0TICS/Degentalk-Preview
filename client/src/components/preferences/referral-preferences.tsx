              ...
              <div className="p-4 bg-zinc-800/50 rounded-lg">
                ...
                <div className="flex items-center gap-3">
                  <Avatar className="h-10 w-10">
                    <AvatarImage src={referralStats.referredBy.avatarUrl || undefined} alt={referralStats.referredBy.username} />
                    <AvatarFallback>{referralStats.referredBy.username.substring(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {referralStats.referredUsers.map((referredUser) => (
                <div key={referredUser.userId} className="flex items-center gap-3 p-3 bg-zinc-800 rounded-lg">
                  <Avatar className="h-10 w-10">
                    <AvatarImage src={referredUser.avatarUrl || undefined} alt={referredUser.username} />
                    <AvatarFallback>{referredUser.username.substring(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div>
                    <div className="font-medium">{referredUser.username}</div>
                    <div className="text-xs text-zinc-400">
                      Joined {formatDistanceToNow(new Date(referredUser.joinedAt), { addSuffix: true })}
                    </div>
                  </div>
                </div>
              ))}
