...
export function EnhancedGifPicker({
	...
	const fallbackUI = (
		<div className="flex flex-col justify-center items-center p-4 text-center">
			...
			<p className="text-zinc-400 text-sm mb-4">You can still add GIFs by entering a direct URL:</p>
		...
		<>
			<div className="flex items-center gap-2 mb-4">
				<Input
					...
					className="bg-zinc-700 border-zinc-600 text-white"
				/>
				<Button onClick={searchGifs} variant="default" disabled={searchLoading || !search.trim()}>
			...
			<Tabs defaultValue="trending" className="w-full">
				<TabsList className="w-full mb-2 bg-zinc-700">
					...
					<TabsTrigger value="manual" className="data-[state=active]:bg-zinc-600">
						Direct URL
					</TabsTrigger>
				</TabsList>
						...
						{searchLoading ? (
							...
							<div className="flex flex-col justify-center items-center h-full p-4 text-center">
								...
								<p className="text-zinc-400 text-sm">
									Try a different search term or enter a direct URL.
								</p>
								{fallbackUI}
							</div>
						) : (
	...
	if (!isDialog) {
		return (
			<div className="sm:max-w-[550px] max-h-[80vh] overflow-hidden flex flex-col bg-zinc-800 border border-zinc-700 text-white rounded-md">
				...
				<div className="p-4">{contentUI}</div>
			</div>
		);
	}
