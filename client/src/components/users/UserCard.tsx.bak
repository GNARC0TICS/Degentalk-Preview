      ...
      <Card className="bg-zinc-900/70 border border-zinc-800 hover:border-emerald-500/30 transition-all duration-300 group overflow-hidden">
        <CardContent className="p-4">
          ...
          <div className="flex items-start justify-between mb-4">
            <Link href={`/profile/${user.id}`} className="flex items-center space-x-3 group-hover:scale-105 transition-transform">
            ...
            <div className="mt-4 pt-3 border-t border-zinc-800">
              <div className="flex flex-wrap gap-1">
                ...
                {user.badges.length > 3 && (
                  <Badge 
                    variant="secondary" 
                    className="text-xs bg-zinc-800/70 text-zinc-400"
                  >
                    +{user.badges.length - 3}
                  </Badge>
                )}
