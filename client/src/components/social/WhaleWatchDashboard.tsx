...
export function WhaleWatchDashboard() {
	...
	const UserCard = ({ user, relationship, showNotificationSettings = false }: { 
		user: FollowUser; 
			...
			<div className=\"flex items-center gap-3\">
				...
				<div className=\"flex-1 min-w-0\">
					...
					<div className=\"flex items-center gap-4 text-xs text-zinc-500\">
						...
						{relationship && (
							<span>
								Followed {formatDistanceToNow(new Date(relationship.followedAt), { addSuffix: true })}
							</span>
						)}
					</div>
					...
					{showNotificationSettings && relationship && (
						<div className=\"flex items-center gap-2 mt-2\">
							...
							{relationship.notificationSettings.notifyOnThreads && (
								<Badge variant=\"outline\" className=\"text-xs\">Threads</Badge>
							)}
							...
							{relationship.notificationSettings.notifyOnLargeStakes && (
								<Badge variant=\"outline\" className=\"text-xs\">Stakes ${relationship.notificationSettings.minStakeNotification}+</Badge>
							)}
						</div>
					)}
				</div>
