				...
				<div className="divide-y divide-border">
					{hotThreads.map((thread) => (
						<Card
							...
							className="bg-zinc-900/50 border border-zinc-800 hover:bg-zinc-900/60 transition-all cursor-pointer h-full flex flex-col"
						>
							<CardHeader className="p-4 pb-2 space-y-1">
								<div className="flex items-center justify-between">
									<Badge
										variant="outline"
										className="bg-zinc-800 text-emerald-400 border-emerald-800"
									>
										Hot
									</Badge>
									<TrendingUp className={`h-4 w-4 ${getHotScoreColor(thread.hot_score)}`} />
								</div>
								<CardTitle className="text-base line-clamp-2 text-zinc-100">
									{thread.title}
								</CardTitle>
							</CardHeader>
							<CardContent className="p-4">
								<div className="flex items-center gap-2 text-xs text-zinc-400">
									...
									<Link href={`/forums/${thread.category_slug}`}>
										<Badge
											variant="outline"
											className="px-2 py-0 h-5 bg-zinc-800 text-zinc-300 border-zinc-700"
										>
			...
			{showViewMore && (
				<CardFooter className="border-t border-zinc-800 px-4 py-3">
					<Link href="/hot-threads">
						<Button
							variant="outline"
							className="w-full bg-zinc-800 text-zinc-300 border-zinc-700 hover:bg-zinc-700 hover:text-zinc-100"
						>
