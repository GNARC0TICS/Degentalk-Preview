...
export function useMissions() {
  ...
  >({
    ...
    onSuccess: (data: { success: boolean; message: string; rewards: MissionReward }, missionId: number) => {
  ...
  if (Array.isArray(missionProgress)) {
    missionsWithProgress = missionProgress.map((progress: MissionProgress): MissionWithProgressData => ({
      ...progress.mission,
      progress: {
  ...
  const getMissionTypes = () => {
    ...
    const types = missionsWithProgress.reduce<string[]>((acc: string[], mission: typeof missionsWithProgress[0]) => {
  ...
  return {
    ...
    getDailyMissions: () => Array.isArray(missionsWithProgress) ? missionsWithProgress.filter((m: typeof missionsWithProgress[0]) => m.isDaily) : [],
    getWeeklyMissions: () => Array.isArray(missionsWithProgress) ? missionsWithProgress.filter((m: typeof missionsWithProgress[0]) => m.isWeekly) : [],
  };
}
