...
export function useFeatureGates() {
  ...
  const checkAccess = (featureId: string): { 
    hasAccess: boolean;
    isLoading: boolean;
    reason?: string; 
    unlocksAtLevel?: number;
    ...
    const gate = Array.isArray(featureGates) ? featureGates.find((g: FeatureGate) => g.id === featureId) : undefined;
