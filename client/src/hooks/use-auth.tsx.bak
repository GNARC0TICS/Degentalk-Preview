...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const { data: fetchedUser, isLoading: userLoading, error: userError } = useQuery<User | null, Error>({
    queryKey: ['user'],
    queryFn: async () => {
  ...
  // Register Mutation
  const registerMutation = useMutation<User, Error, { username: string; email: string; password: string }>({
  ...
  const authContextValue = useMemo(() => ({
    ...
    error: authError || loginMutation.error?.message || registerMutation.error?.message || logoutMutation.error?.message || null,
    loginMutation,
    registerMutation,
    logoutMutation,
    // --- Dev Mode Specific ---
