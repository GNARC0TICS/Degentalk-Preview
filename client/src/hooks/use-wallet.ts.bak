...
export function useWallet() {
  ...
  const purchaseDgtMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Your DGT purchase request has been initiated. Please check your transaction history for updates."
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return {
    ...
    refreshTransactions: () => queryClient.invalidateQueries({ queryKey: ['/api/wallet/transactions'] })
  };
}
