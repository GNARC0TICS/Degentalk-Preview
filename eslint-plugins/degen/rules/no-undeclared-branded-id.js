...
module.exports = {
	meta: {
		...
		schema: [],
		messages: {
			missingImport: 'Missing import for branded ID type: {{idType}}. Add: import type { {{idType}} } from \'@db/types\';',
				...
				if (!importedTypes.has(name)) {
					context.report({
						...
						fix(fixer) {
							...
							const existingImport = sourceCode.ast.body.find(node => 
							...
							if (existingImport) {
								...
								return fixer.insertTextAt(insertPosition, `import type { ${name} } from '@db/types';\n`);
							}
						}
					});
				}
				...
				if (missingTypes.length > 0 && !hasDbTypesImport) {
					...
					if (firstNode) {
						context.report({
							...
							fix(fixer) {
								return fixer.insertTextBefore(firstNode, `import type { ${missingTypes.join(', ')} } from '@db/types';\n`);
							}
						});
					}
				}
