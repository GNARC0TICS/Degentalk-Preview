...
10. **Files: client/src/components/forum/category-card.tsx, forum-category-card.tsx, forum-card.tsx**
    ...
    - **Task:** Extract into a single `ForumEntityCard` component with a `variant` prop, standardize navigation with a `useNavigate()` hook, and deprecate legacy components.
...
11. **File: client/src/contexts/ForumStructureContext.tsx & client/src/features/forum/hooks/useForumStructure.ts**
    ...
    - **Task:** Deprecate legacy hook, memoize `mergeStaticAndApiData` with `useMemo`, code-split provider logic, and add ESLint rule for migration.
...
14. **File: server/src/domains/admin/admin.service.ts**
    ...
    - **Task:** Extract metrics to SQL views, memoize/cache stats (e.g., Redis 30s TTL), and instantiate service per-request or use DI.
...
16. **Multiple Files (e.g., client/src/components/layout/announcement-ticker.tsx, etc.)**
    ...
    - **Task:** Enforce ESLint rules for unused vars, add `eslint-plugin-boundaries` for alias groups, and create `lint:imports` script.
...
18. **File: Homepage Components**
    ...
    - **Task:** Extract `HeroBanner` from negative margins and refactor widget system for homepage scalability.
...
19. **File: scripts/auth/auth-standardize.ts**
    ...
    - **Task:** Use glob patterns (e.g., `**/*.{ts,tsx,js,jsx}`) with `fast-glob` and convert `SKIP_FILES` to CLI arguments.
...
20. **File: client/src/hooks/use-async-button.tsx**
    ...
    - **Task:** Return internal promise from `handleClick` and track `isMounted` via `useRef` or abort signal.
...
22. **File: server/src/core/logger.ts & Related Docs**
    ...
    - **Task:** Purge outdated doc references, create `@logger` alias, call `initLogger()` in `server/index.ts`, and add ESLint rule against `console.*`.
...
1. **File: server/src/domains/engagement/vault/vault.routes.ts**
   ...
   - **Task:** Centralize `getUserId` in `src/core/auth/utils.ts`, wrap unlock/transaction in a single Drizzle transaction, guard test routes with `ENABLE_TEST_ROUTES` flag, replace hard-coded address with config constant.
...
2. **File: server/utils/wallet-utils.ts**
   ...
   - **Task:** Split into `wallet-format.ts`, `wallet-db.ts`, `wallet-exchange.ts`, integrate transactional helpers into WalletService, delete `walletEngine.ts` after migration.
...
3. **File: client/src/contexts/wallet-context.tsx**
   ...
   - **Task:** Update error type to `unknown | null` or custom union, replace window events with context dispatch or Zustand, initialize state from shared store.
