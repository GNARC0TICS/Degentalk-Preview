...
/**
 * Server-Storage Domain Migration Script
 * 
 * Targeted migration for server storage and Drizzle helpers
 * HIGH RISK - core database operations and storage layer
 */
...
class ServerStorageMigration {
  ...
  async migrate(): Promise<void> {
    console.log('🚀 Server-Storage Domain Migration');
    console.log('=================================');
    console.log(`Mode: ${this.dryRun ? 'DRY RUN' : 'LIVE MIGRATION'}`);
    console.log('Risk Level: HIGH\n');

    // Get server-storage domain files
    const files = await this.getServerStorageFiles();
    console.log(`📁 Found ${files.length} server-storage files`);
    ...
    console.log('\n✅ Server-storage migration analysis complete');
  }

  private async getServerStorageFiles(): Promise<string[]> {
    const patterns = [
      // Server storage and database files
      'server/src/**/storage.ts',
      'server/src/core/storage/**/*.ts',
      'server/src/domains/**/storage/**/*.ts',
  ...
  private async analyzeFiles(files: string[]): Promise<void> {
    console.log('🔍 Analyzing server storage files...');
  ...
  private findNumericIdPatterns(content: string, filepath: string): Change[] {
    ...
    // Server-storage specific patterns
    const patterns = [
      // Profile-related patterns (most common in client types)
      { regex: /\bid:\s*number\b/g, replacement: 'id: UserId', confidence: 0.95, type: 'id_definition' as const },
...
async function main() {
  ...
  const migration = new ServerStorageMigration(isDryRun);
