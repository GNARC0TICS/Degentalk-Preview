...
/**
 * Client-Components Domain Migration Script
 * 
 * Targeted migration for the client-components domain (React components layer)
 * MEDIUM RISK - React components with branded ID parameters and returns
 */
...
class ClientComponentsMigration {
  ...
  async migrate(): Promise<void> {
    console.log('🚀 Client-Components Domain Migration');
    console.log('=================================');
    console.log(`Mode: ${this.dryRun ? 'DRY RUN' : 'LIVE MIGRATION'}`);
    console.log('Risk Level: MEDIUM\n');

    // Get client-components domain files
    const files = await this.getClientComponentsFiles();
    console.log(`📁 Found ${files.length} client-components files`);
    ...
    console.log('\n✅ Client-components migration analysis complete');
  }

  private async getClientComponentsFiles(): Promise<string[]> {
    const patterns = [
      // Client components files only
  ...
  private async analyzeFiles(files: string[]): Promise<void> {
    console.log('🔍 Analyzing client components files...');
  ...
  private findNumericIdPatterns(content: string, filepath: string): Change[] {
    ...
    // Client-components specific patterns
    const patterns = [
  ...
  private generateManualReviewChecklist(manualItems: Array<{file: string, items: Change[]}>): void {
    const markdown = [`# Client-Components Manual Review Checklist`, ``, `Generated: ${new Date().toISOString()}`, ``, `## Items Requiring Manual Review (confidence < 0.9)`, ``];
    ...
    writeFileSync('scripts/migration/output/components-manual-review.md', markdown.join('\n'));
    console.log('📄 Manual review checklist saved to scripts/migration/output/components-manual-review.md');
  }
...
async function main() {
  ...
  const migration = new ClientComponentsMigration(isDryRun);
...
export { ClientComponentsMigration };