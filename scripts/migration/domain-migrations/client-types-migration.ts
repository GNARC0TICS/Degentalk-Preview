...
/**
 * Client-Types Domain Migration Script
 * 
 * Targeted migration for the client-types domain (28 ID issues, 19 files)
 * MEDIUM RISK - simpler type definitions with clearer contexts
 */
...
class ClientTypesMigration {
  ...
  async migrate(): Promise<void> {
    console.log('🚀 Client-Types Domain Migration');
    console.log('=================================');
    console.log(`Mode: ${this.dryRun ? 'DRY RUN' : 'LIVE MIGRATION'}`);
    console.log('Risk Level: MEDIUM\n');

    // Get client-types domain files
    const files = await this.getClientTypesFiles();
    console.log(`📁 Found ${files.length} client-types files`);
    ...
    console.log('\n✅ Client-types migration analysis complete');
  }

  private async getClientTypesFiles(): Promise<string[]> {
    const patterns = [
      // Client type definition files
      'client/src/types/**/*.{ts,tsx}',
      'client/src/types/profile.ts',
      'client/src/types/forum.ts',
      'client/src/types/user.ts',
      'client/src/types/compat/**/*.{ts,tsx}'
    ];
  ...
  private async analyzeFiles(files: string[]): Promise<void> {
    console.log('🔍 Analyzing client type files...');
  ...
  private findNumericIdPatterns(content: string, filepath: string): Change[] {
    ...
    // Client-types specific patterns
    const patterns = [
...
async function main() {
  ...
  const migration = new ClientTypesMigration(isDryRun);
